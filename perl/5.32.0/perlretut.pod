...
この式の値は、C<$var> がこの文字並びを含んでいるなら真に、さもなければ
偽になります。
...
最初の正規表現 C<world> はマッチングしません; なぜなら、正規表現は大文字と
小文字を区別するからです。
...
(C<Script_Extensions> は C<Script> の改良版で、後方互換性を持っているので、
一般的に C<Script_Extensions> を使うべきです。)
例えば、英語、フランス語、および多くのその他のヨーロッパ言語は
ラテン用字で書かれます。
しかし、ギリシャ用字、タイ用字、カタカナ用字などもあります。
ある文字が (C<Script_Extensions> を基として)特定の用字に含まれるかを、
例えば C<\p{Latin}>, C<\p{Greek}>, C<\p{Katakana}> のようにしてテスト出来ます。
バリ用字に含まれないかどうかをテストするには、C<\P{Balinese}> として使います。
