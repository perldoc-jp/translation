...
トレードオフがあります。直接なハッシュのアクセスはそうでない場合より(マグニチュードの順番について、実際速く(訳註：謎))
...
$Person::Census と呼ばれるグルーバル変数でそういうことがI<できます>。
...
たぶん、メソッドは、単に、特別な引数と、それが見つけれらる場所に付いての愛好家の意味論を伴う(訳註：謎)、
