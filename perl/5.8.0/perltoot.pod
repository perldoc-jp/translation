...
マネージャーには、薄切りパンよりもむしろオブジェクトを持ちたがる人もいます(訳注：謎)。
どうしてでしょうか? オブジェクトには、何か特別なものがあるのでしょうか? 
とにかく単にオブジェクトI<である>からでしょうか?
...
オブジェクトは、複雑なふるまいを、きちんとした小さな、使いやすい包みに包んで、
遠くへやってしまう方法以外のなにものでもありません。(このことを、専門家は抽象化と呼びます。)
本当に難しい問題を片付けようと、何週間もぼーっと過ごしている、かしこい人々は、
...
カバーに肥やしをやれば、保証は無効になります、と。ですから、そんなことをしてはいけません。
...
いくつかの問題領域を扱います。クラスを、ユーザ定義の型と考えることもできます。
Perl のパッケージの仕組は、より伝統的なモジュールと同じように、クラスモジュールにも使われます。
クラス内の "live"という複数のオブジェクト は、それらがいくつかのパッケージに属していることを意味します。
...
コードのリファレンスすらありえます。ですが、持ち前の柔軟性のために、
ふつうは、オブジェクトはハッシュリファレンスです。
...
一定の距離から間接的にオブジェクトを操作することを許すメカニズムを提供すべきです。
...
上のように、、そのクラスを使う人は、
オブジェクトが一つの特殊な道具か他の何かであることを知りません(すくなくとも、その事実に注意を払うかは無関係です)。
...
そして、その誰かとは、クラスです。クラスはプログラマーが、オブジェクトにアクセスするのに使うメソッドを実装します。
ここに示すのは、標準的なオブジェクトのようにハッシュリファレンスを使うイディオムを使い、Personクラスに実装する方法です。
コンストラクタとして働くnew()というクラスメソッドを作りましょう。
そして、3っつのオブジェクトメソッド、name()と、age()と、peers() を作り、
...
オブジェクトのデータにアクセスする3っつのメソッド、name()、age()、peers()を、作りました。
これらは、実質は、すべて似たようなものです。
引数つきで呼べば、しかるべきフィールドに、(訳註：値を)セットし、引数がなければ、
...
(お望みなら、今のところ継承を安全に無視し、後になって、継承に悩むこともできます)。
...
きちんとした、小さくて、不可解なな束をユーザに返し、次のメソッドの呼出しに使われます。
...
呼ぶべきものかわかるように、Perl は、デストラクタが、DESTROYと名付けられることを、主張します。
Perlはデストラクタを呼ぶ適切な時期を考えますが、現在のところ洗練されていません。
...
にもかかわらず、プログラムが終わるときに、そのオブジェクトのデストラクタが全て正式に呼ばれることを、
残りが保証しています(訳註：謎)。
そのため、次のことが保証されます。オブジェクトが、I<最終的に>適切に、破壊され、プログラムのユニークな場合に、
オブジェクトは存在していないことが期待できます。
(他のアプリケーションに、埋め込んでPerlを走らせているなら、この徹底したガベージコレクションパス(訳註：?)は、
普段より少々頻繁に - スレッドが切れない限り - 起こります)。
...
(たぶん、L<変数ようなデータメンバ>に、下の方で記述するAliasモジュールを使わない場合は)。
...
しかし、この区別はPerl自身によって、強制されてはいません。そう振る舞うことは、プログラマしだいです。
...
メソッドか、オブジェクトかクラスに対して呼び出されるということです。
一つの特別なフィールドから値を持ってきたりセットしたりする以上のオブジェクトメソッドを示しましょう。
...
トレードオフがあります。直接なハッシュのアクセスはそうでない場合より(マグニチュードの順番について、実際)速く(訳註：謎))
...
何のためにクラスデータを望むのでしょう? たぶん、Person クラスでは、生きている人々の総数を追っておきたいでしょう。
...
$Person::Census と呼ばれるグルーバル変数でそういうことをI<できます>。
しかし、そのようにする唯一の理由は、人々がクラスデータを得られるようにしたい場合です。
$Pearson::Census と言うだけで、それをいじることが出来ます。
あなたの考えでは、そういったことは構わないのかもしれません。
ひょっとすると、変数が持ち出されることさえ、望んでいるかもしれません。
持ち出されるのなら、(パッケージの)グローバル変数でなければならない。
...
オブジェクト指向のモジュールでは、(訳註：データの)保護をするベールを設けて、実装とインターフェースを
分離します。だから、オブジェクトデータにアクセスするオブジェクトメソッドを提供するのと同じように、
...
こうたことで、もちろん、Personが壊れたときに、$Census を減らすために、
...
さもなければ、拡張的な、継承的なクラスは作らない。オブジェクトは、全てのオペレーションのために、
ランデブーポイントにいなければなりません。特に、とりわけオブジェクトメソッドからは。
グローバル変数(クラスデータ)は、ある意味で、生成したクラスの中で、"間違った"パッケージにあります。
...
その名前空間を得られるでしょうか? たぶん、無理です。よし、他のクラスが(たぶん継承されている)
...
たぶん、たいてい、最初のアンダースコアは、いつもCプログラマに不思議な強い感覚を伝えるからです。
...
(Employeeと呼ばれる)生成されたクラスが、Person 基底クラスからメソッドを継承しているなら、
...
Perlのコンパイラは、podをまったく無視します。ちょうど、翻訳者がコードを無視するように。
...
その代わりに、意味論において、全てが厳しい(訳註：謎)。それぞれのパッケージには、
...
ですが、Employeeクラスに、ちょうどPersonクラスのような振舞を当てにします。
...
これは、基底クラスから継承する以外には、なにもしないクラスを生成することです。
オリジナルの基底クラスは、適切に設計されていれば、
新しく生成されたクラスは、古いものの差し込み式の置換として、使われます。
...
メソッドの両方が結局呼んでいる関数です。
メソッド呼び出しが関数呼び出しによく似ていながら、本当に、まったく同じではなく、
二つを同じものとして扱うなら、すぐに壊れたプラグラムだけが残されることを理解してください。
...
メソッドを呼ぼうとしているときに関数呼び出しを使わないようにしてください。
...
ちょっと飽きても、落胆しないでください。後で、こういた作業を短縮するための、
いくつかの種々の便利なメカニズムについて説明します。一方で、直接なやりかたは次のものです：
...
生成されたクラスとその基底クラスの両方が同じ名前のメソッドを定義したら、
何が起きるでしょうか? たぶん、そのメソッドが生成されたクラスのもの(version)を得ます。
...
これは、不精の形です。(Getting polymorphed is also what happens
when the wizard decides you'd look better as a frog.)
...
しばしば、生成されたクラス("サブクラス"としても知られるクラス)のもの(version)と
その基底クラス("スーパークラス"としても知られるクラス)のもの(version)も同じように
...
お分かりのように、(訳註：Employeeから)向きを変えて、debug()関数を呼びます。ですが、
良い設計にとっては、こうするのは、ひじょーに、壊れやすいものです。
...
ふーむ、何かがなくなっている。給与、入社日、IDフィールドはどうでしょうか?
...
知っていることを許すことを指示しています。ですが、その逆は同様ではありません)。
...
Well, kinsa(訳註：?). 完全にリカーシブで、depth-fistの順番であることもありえます。
...
他のクラスのメソッドを借りて来ることです。このことは、幾分普及しており、
...
生成されたクラスのために、新しい索引を作り上げなければならないからです。
...
このことは、まちがいなく、プログラミングポリスと、関連する厳格な人で、たしかに口うるさい関心を満たすでしょう(訳註：謎)。
...
たぶん、メソッドは、単に、特別な引数と、それが見つけれら屡場所に付いての愛好家の意味論なので、
オートローディングをメソッドとしても使うことが出来ます(訳註：謎)。
...
gethostbyname()と、gethostbyaddr()を、C構造体のような振舞をするオブジェクトを返すように、
オーバライドしたいとしましょう。high-falutin' OO gunk は、気にしません。
やりたいことは、これらのオブジェクトがC感覚で、構造体のような振舞をすることです。
...
C<our>を宣言しなければなりませんが、Aliasがしているのが、
...
I<オブジェクト特性値>と呼ばれ、全てのクラスのメンバに共通のデータフィールドは
I<クラスデータ>や、I<クラス特性値>や、I<静的なデータメンバ>とよく呼ばれるでしょう。
...
同じ考えで記述します。ですので、I<生成されたクラス>やI<特有のクラス>や、
...
クラスメソッド(一つの文字列の引数を期待する)を呼ぶことも出来るし、その逆も出来ます。
...
ところで、このことは、メソッドが普通の組み込み関数やユーザー定義関数が出来るような、
引数リストに、関数のプロトタイプのためにチェックされない理由です。
...
クラスは、それ自身、オブジェクトの何かです。Perlのクラスは、"メタオブジェクトとしてのクラス"
(または、I<オブジェクト工場>とも呼ばれます)哲学と、"型定義としてのクラス"
(振舞をI<宣言>し、メカニズムをI<定義しない>)考えの両方を表すものとして見られます。
