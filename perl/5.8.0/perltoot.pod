...
見た目には、メソッドは、ブラケットや、ブレイス(大括弧)の代わりに、
...
(たぶん、あなたは、コンパイラにあなたの好むすべてを期待するでしょうが、起こりません。)
...
このやり方は、ほとんどの伝統的なモジュールで、期待されているのですが、
...
そのために、名前空間 - メソッド内で、目に見えるパッケージのグローバル変数の - は、継承に無関係です。
...
上で定義されているような(訳註:Personクラスの)DESTORYメソッドを、"借りた"としましょう。
これら他のクラスのオブジェクトが破壊されたら、オリジナルの $Census 変数は変化させられます。
...
適切な設計によって、そうできます。常にbless()に、二つの引数の形を使い、
...
を他のところから継承してたら、どうなるでしょう? 次のようにする方が、よりいいです。

    Person->debug($Debugging);

But even that's got too much hard-coded.  It's somewhat better to say

ですが、まだ、ハードコードしすぎです。次のようにする方が、よりいいです。
...
たぶん、それらに、何も、undefさえもセットしていません。それで、ハッシュのキーに、
