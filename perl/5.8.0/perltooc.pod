...
オブジェクトクラスを宣言するとき、共通の状態をクラスの全てのオブジェクトが共有したい状況に直面するときがあります。
...
デバッグメソッドで使われたログファイルのために、名前かファイル識別子を抽出するため。
...
集合的なデータ、たとえば、与えられた日のネットワークの、
全ATMで支払われた現金の、総金額のようなものにアクセスするため。
...
クラスによって、最後に作られたオブジェクト、または、もっともアクセスされたオブジェクトにアクセスするため、
または、すべてのオブジェクトのリストを検索するため。
...
本当のグローバル変数とは違い、クラス属性は、直接にアクセスされるべきではありません。
そうしないで、クラス属性の状態は調べられるべきですし、たぶん、変更もされるべきです。
...
クラス属性にクラスの名前か、クラスのオブジェクトのどちらを通しても、アクセスできるようにすべきです。
San_object が、Some_Class の型のものとし、&Some_CLass::population_count メソッドが
クラス属性にアクセスするとします。そうすると、2つの呼び出しが両方可能であるべきで、
もちろん、ほとんど同じであるべきです。
...
疑問は次のものです。このメソッドがアクセスする状態をどこに蓄えておくのでしょうか?
C++のように、より厳格な言語とは違って、静的なデータメンバと呼ばれる場所、
Perlは、クラス属性を宣言するのに、総合のメカニズムはまったく提供しません。
インスタンス属性を宣言する総合のメカニズムしかありません。
Perlは、開発者に、特殊な状況を要求に、うまく作られうる、
強力だが、柔軟な、大まかなセットを提供します。
...
特徴のセットでできています：外の世界とのインターフェースのためのパッケージと、
プライバシーのためのレキシカルなファイルスコープのセットです。
これらの二つのメカニズムの両方が、クラス属性の実装に使われ得ます。
...
また、これらは、作るための唯一の決定ではありません。パッケージ変数を選んだとしても、
...
この場合、Class::Data::Inheritable(お近くのCPANで利用可能です)が、
...
するのがもっとも簡単なのは、$Some_Class::CData1 や$Some_Class::CData2 のようなパッケージ変数を使い、
これらの属性を持たせることです。ですが、もちろん部外者がこれらのデータディレクトリに触ることを
勧めたりはしたくありません。ですので、アクセスをもくろむメソッドを提供します。
...
さて、%ClassDataハッシュの茄かのそれぞれのキーごとに、アクセサメソッドをクローニングします。
...
意図しないときに、あやまって、使われる可能性があるので、危なっかしいというだけです。
...
言い替えれば、シンボルテーブルを参照したい場合は、シンボリックリファレンスを使えばいい。
...
集約は、クラス属性を外部から、例えば、デバッガや持続的なパッケージから、アクセスするのに便利にします。
唯一のありそうな問題は、それぞれのクラスが持っているべきクラスオブジェクトの名前を自動的に知らないことです。
