...
glob()によって得られたファイル名のリストはcshに忠実(以前の大多数のUNIX
プラットフォームではそうなっていました)であるために現在デフォルトで
アルファベット順に格納されます。(bsd_glob()はまだGLOB_ALPHASORTを指定しない
限り、プラットフォーム固有のASCIIコードあるいはEBCDIC順になります。)[561]
...
偽の文法エラーがある一定の状況下で起こります。glob()が
最初にFile::Globをロードするときなのですが、修正されました。[561]
...
"それをすべきではない"のですが、Perlのハッシュキーの順序に依存したコードを
書くことが可能です(Data::Dumperがこれを行っています)。新しいアルゴリズムである
"One-at-a-Time"では今までと異なるハッシュキーの順序になります。より詳しい
詳細はL</"Performance Enhancements">にあります。
...
lstat(FILEHANDLE)と言う操作は意味をなさないため、現在警告を出すようになって
います。将来致命的なエラーになるかも知れません。
