...
Perl v5.30 から、Perl は振る舞いでトルコ語ロケールを検出し、
二つの型を継ぎ目なく扱います; 以前は
非トルコ語用のもののみを扱っていました。
システムに C<tr_TR.UTF-8> ロケールがあってそれがトルコ語ロケールのように
振る舞わない場合、ロケールの名前は無視され、perl はそれを非トルコ語のように
扱います。
...
これは Perl 自身によって、L<C<$!>|perlvar/$ERRNO> と
L<C<$^E>|perlvar/$EXTENDED_OS_ERROR> 経由でオペレーティングシステムの
...
Perl はこれらについても何もせず、このプラグマの引数としてこれを指定する
方法はありません。
...
読み込み専用の真偽値変数 C<${^SAFE_LOCALES}> を見ることによって、
...
スレッドセーフ操作は、Visual Studio 2005 以降の Windows と、POSIX 2008 に
互換性のあるシステムで対応しています。
一部のプラットフォームは POSIX 2008 に対応していると主張しますが、
バグっぽい実装をしているので、
それら実行するためにコンパイルするためのヒントファイルは、
それらではスレッドセーフ機能を使おうとするのをオフにします。
それらでは C<${^SAFE_LOCALES}> は 0 です。
...
通常、非スレッドビルドでは、スレッドセーフなロケール関数ではなく
伝統的な C<setlocale()> が使われます。
...
完全に等価であるはずです (
L</Multi-threaded> 説にある、いくつかの稀に遭遇する問題の場合を除きます)。
XS モジュール作者への情報は L<perlxs/Locale-aware XS code> にあります。
...
(Windows 以外で) システムの L<C<setlocale(3)>> 関数を使う
XS コードや C 言語ライブラリは、
おそらく変更なしではマルチスレッドアプリケーションからは動作しません。
L<perlxs/Locale-aware XS code> を参照してください。
...
ロケール依存の XS モジュールは、決してマルチスレッド環境で
呼び出されてないという仮定の下書かれており、
それでマルチスレッドセーフでないその他の非ロケール構文を使うかもしれません。
L<perlxs/Thread-aware system interfaces> を参照してください。
...
POSIX は現在のスレッド単位のロケール名を取得する方法を定義していません。
Darwin と NetBSD のような一部のシステムでは、これをするための
関数である L<querylocale(3)> を実装しています。
Linux のような、これのない非 Windows システムは追加でいくつかの問題が
あります:
...
組み込み perl はグローバルなロケールが有効な間に起動する
必要があります。
L<perlembed/Using embedded Perl with POSIX locales> を参照してください。
...
perl にとって、たとえあなたのプログラムで使われていなくても、
このプラットフォームでの全ての可能性のあるロケールカテゴリについて
知ることはさらに重要になってきています。
Perl は Linux での全てを知っています。
あなたのプラットフォームが他のものを持っているなら、
次のリリースで追加するために
L<https://github.com/Perl/perl5/issues> にイシューを登録してください。
現在の所、カテゴリについて教えるために Perl のソースを編集して
再コンパイルすることは可能です。
例えば、ソース中の C<LC_PAPER> を検索して、それを足りないものを
追加するためのテンプレートとして使ってください。
...
文法的には正当と認識されないけれども、実際にはシステムに対して
正当なロケールで C<POSIX::setlocale> を呼び出すことは可能です
(しかしそうするのは難しいです)。
これは組み込み perl か、自分自身でロケール名を組み立てている場合にのみ
起こります。
