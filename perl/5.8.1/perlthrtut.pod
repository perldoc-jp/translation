...
なされる。一方iスレッドはL<threads> classを使う。この変化に注意すること。
...
スカラー変数同様、配列やハッシュにもロックをかけることができるだろう。
...
リソースを持っているスレッドが自分自身のロックの開放を待つ場合は、は生じない。
...
dequeue()はブロックする。そんため、イベントループやスレッド間での
...
子スレッドをつくていた場合には子スレッドに注意を促し[（undefを送る）]、
...
昨今、最も進んだOSではプリエンティブなマルチタスクをサポートしている。
