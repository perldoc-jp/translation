...
You can change both the real uid and the effective uid at the same
time by using POSIX::setuid().
Since changes to $< require a system call, check $!
...
多くのメモリを使います。
Set it to a lower value if you want the optimisations to
be as conservative of memory as possible but still occur,
負の値を設定すると最適化は行わず、最大のメモリを使用します。
