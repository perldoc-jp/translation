...
集まったものです。L<List::Util>を参照してください。
...
PODデータをフォーマットされた overstrike 文章にコンバートします。
...
インターフェースです。インタプリタスレッド(ithreads)は Perl5.6 で導入された
...
データを共有することを可能にします。<threads::shared>を参照してください。
...
C<Tie::RefHash::Nestable>はEdward Avisによるもので、ハッシュのリファレンスを
格納することを可能にします(標準の Tie::RefHash とは異なるものです)。
本モジュールは Tie::Refhash に含まれています。
...
C<Time::HiRes>はDouglas E. Wegcheidによるもので、高精度の
...
attributes::reftype()は現在を持って tie された引数で動作するようになりました。
...
拡張されたバージョンはPerl の以前のリリースにもバックポートされ、
