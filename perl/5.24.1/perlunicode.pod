...
この文書の執筆時点 (Unicode 8.0) では、C<In_> 接頭辞の使用と
競合することはありませんが、他の二つの形式では多くの競合があります。
例えば、C<\p{Is_Hebrew}> および C<\p{Hebrew}> は、
C<\p{Script=Hebrew}> を意味しますが、C<\p{Blk=Hebrew}> と
同じものでは「ありません」。
...
L<C<qrE<sol>\b{lb}E<sol>>|perlrebackslash/\b{lb}> は
L<UAX#14 "Unicode Line Breaking Algorithm"|http://www.unicode.org/reports/tr14>
準拠のデフォルトの行分割を提供します。
...
そして、これも UAX#14 に準拠している C<L<Unicode::LineBreak>> モジュールは、
カスタマイズできる行分割を提供します。
