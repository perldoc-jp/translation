...
L<C<alarm>|/alarm SECONDS> と L<C<sleep>|/sleep EXPR> を混ぜて使うのは
普通は間違いです; なぜなら、L<C<sleep>|/sleep EXPR> は内部的に
L<C<alarm>|/alarm SECONDS> を使って内部的に実装されているかも
しれないからです。
...
L<C<alarm>|/alarm SECONDS> をシステムコールの時間切れのために使いたいなら、
...
L<C<binmode>|/binmode FILEHANDLE, LAYER> を使ってください。
...
I<L<C<binmode>|/binmode FILEHANDLE, LAYER> 関数の LAYER パラメータは
...
一般的に L<C<binmode>|/binmode FILEHANDLE, LAYER> は
L<C<open>|/open FILEHANDLE,EXPR> を呼び出した後、このファイルハンドルに対する
I/O 操作をする前に呼び出すべきです。
L<C<binmode>|/binmode FILEHANDLE, LAYER> を呼び出すと、普通はこの
...
つまり、もしこれらのシステムで L<C<binmode>|/binmode FILEHANDLE, LAYER> を
...
L<C<binmode>|/binmode FILEHANDLE, LAYER> を(いくつかのシステムで)
使うことによるその他の作用としては、特別なファイル終端マーカーが
データストリームの一部として見られることです。
Microsoft ファミリーのシステムでは、
L<C<binmode>|/binmode FILEHANDLE, LAYER> を使っていないと、
...
L<C<binmode>|/binmode FILEHANDLE, LAYER> は L<C<readline>|/readline EXPR> と
...
L<C<bless>|/bless REF,CLASSNAME> は通常、コンストラクタの最後に
置かれますので、簡便のためにそのリファレンスを返します。
派生クラスが bless されるメソッドを継承する場合は、
常に 2 引数版を使ってください。
...
L<C<caller>|/caller EXPR> が情報を得る前にオプティマイザが呼び出しフレームを
最適化してしまうかもしれないことに注意してください。
これは、C<caller(N)> が C<< N > 1 >> のとき、
あなたが予測した呼び出しフレームの情報を返さないかもしれないことを意味します。
特に、C<@DB::args> は L<C<caller>|/caller EXPR> が前回呼び出された時の情報を
持っているかもしれません。
...
使われます。)
どちらも設定されていなければ、L<C<chdir>|/chdir EXPR> は何もせずに失敗します。
...
L<perlvar>を参照してください)では、L<C<chomp>|/chomp VARIABLE> は
何も取り除きません。
...
最後の L<C<chop>|/chop VARIABLE> の値だけが返されます。
...
L<C<chop>|/chop VARIABLE> は最後の文字を返すことに注意してください。
...
L<C<chroot>|/chroot FILENAME> します。
...
B<注意:> L<C<chroot>|/chroot FILENAME> の直後に (ルートディレクトリに
L<C<chdir>|/chdir EXPR> する)
C<chdir("/")> するのはセキュリティ上の良い習慣です。
...
場合には、L<C<open>|/open FILEHANDLE,EXPR> が自動的に
L<C<close>|/close FILEHANDLE> を行ないますので、
close FILEHANDLE する必要はありません。
(L<C<open>|/open FILEHANDLE,EXPR> を参照してください。)
ただし、明示的にクローズを行なったときにのみ入力ファイルの
行番号 (L<C<$.>|perlvar/$.>) のリセットが行なわれ、
L<C<open>|/open FILEHANDLE,EXPR> によって行なわれる
...
ファイルハンドルがパイプつきオープンなら、L<C<close>|/close FILEHANDLE> は
...
L<C<close>|/close FILEHANDLE> は、そのファイルハンドルが他のスレッドで
...
BLOCK が引き続く場合、L<C<continue>|/continue BLOCK> は実際には関数ではなく、
実行制御文です。
L<C<continue>|/continue BLOCK> BLOCK が BLOCK (典型的には C<while> または
C<foreach> の中)にあると、これは条件文が再評価される直前に常に実行されます;
これは C における C<for> ループの 3 番目の部分と同様です。
従って、これは L<C<next>|/next LABEL> 文 (これは C の
L<C<continue>|/continue BLOCK> 文と似ています) を使って
...
L<C<continue>|/continue BLOCK> ブロック内に現れる可能性があります;
L<C<last>|/last LABEL> と L<C<redo>|/redo LABEL> はメインブロックの中で
実行されたのと同じように振舞います。
L<C<next>|/next LABEL> の場合は、L<C<continue>|/continue BLOCK> ブロックを
...
L<C<continue>|/continue BLOCK> 節を省略するのは、空の節を指定したのと同じで、
...
BLOCK がなければ、L<C<continue>|/continue BLOCK> は動的に囲まれた C<foreach> や
レキシカルに囲まれた C<given> で反復するのではなく、現在の C<when> または
C<default> のブロックを通り抜けるための文です。
Perl 5.14 以前では、この形式の L<C<continue>|/continue BLOCK> は
...
L<C<crypt>|/crypt PLAINTEXT,SALT> は一方向ハッシュ関数です。
...
salt で L<C<crypt>|/crypt PLAINTEXT,SALT> します。
...
これにより、L<C<crypt>|/crypt PLAINTEXT,SALT> は同じ salt で新しい文字列を
ダイジェストとしてハッシュ化できるようにします。
これによって標準的な L<C<crypt>|/crypt PLAINTEXT,SALT> や、より風変わりな
実装でも動作します。
...
ライブラリだけに依存し、Perl は L<C<crypt>|/crypt PLAINTEXT,SALT> が
...
L<C<crypt>|/crypt PLAINTEXT,SALT> 関数は大量のデータのハッシュ化には
...
含みます)に L<C<crypt>|/crypt PLAINTEXT,SALT> を使った場合、Perl は
L<C<crypt>|/crypt PLAINTEXT,SALT> を呼び出す前に与えられた
文字列を8 ビットバイト文字列にダウングレードする(文字列のコピーを作る)
ことで状況のつじつまを合わせようとします。
うまく動けば、それでよし。
動かなければ、L<C<crypt>|/crypt PLAINTEXT,SALT> は
...
HASH は、その連想配列の名前です。
(普通の L<C<open>|/open FILEHANDLE,EXPR> とは違って、最初の引数は
ファイルハンドル I<ではありません>; まあ、似たようなものですが)。
DBNAME は、データベースの名前です (拡張子の .dir や .pag はもしあっても
つけません)。
データベースが存在しなければ、MODE MASK (を L<C<umask>|/umask EXPR> で
...
L<C<dbmopen>|/dbmopen HASH,DBNAME,MASK> を実行することができます。
昔のバージョンの Perl では、DBM も ndbm も持っていないシステムでは、
L<C<dbmopen>|/dbmopen HASH,DBNAME,MASK> を呼び出すと致命的エラーになります;
...
L<C<dbmopen>|/dbmopen HASH,DBNAME,MASK> を呼び出す前にライブラリを
...
集合(ハッシュや配列)への L<C<defined>|/defined EXPR> の使用は非推奨です。
...
注意: 多くの人々が L<C<defined>|/defined EXPR> を使いすぎて、C<0> と
...
L<C<defined>|/defined EXPR> を使うべきです。
...
ハッシュの要素やスライスを指定する式を取り、L<C<delete>|/delete EXPR> は
...
L<C<delete>|/delete EXPR> は配列や配列のスライスに対しても使えますが、その
振る舞いはあまり直感的ではありません。
...
B<警告:> 配列の値に対して L<C<delete>|/delete EXPR> を呼び出すことは強く
非推奨です。
...
L<C<die>|/die LIST> は例外を発生させます。
L<C<eval>|/eval EXPR> の中で使用すると、エラーメッセージが
L<C<$@>|perlvar/$@> に入り、L<C<eval>|/eval EXPR> は
未定義値を返して終了します。
例外が全ての L<C<eval>|/eval EXPR> の外側の場合は、捕捉されなかった例外は
LIST を C<STDERR> に表示して、非 0 の値で終了します。
...
出力が空で L<C<$@>|perlvar/$@> が(典型的には前回の eval で)既に値を持っている
場合、値は C<"\t...propagated"> を追加した後再利用されます。
これは例外を伝播させる場合に有効です:
...
出力が空で、L<C<$@>|perlvar/$@> が C<PROPAGATE> メソッドを含むオブジェクトへの
...
L<C<$@>|perlvar/$@> が空の場合、C<"Died"> が使われます。
...
C<errno> の値であり、L<C<die>|/die LIST> によって使われる終了コードの値は
...
L<C<die>|/die LIST> はリファレンス引数と共に呼び出すこともでき、これが
...
マッチングするときに時々好まれます。
L<C<$@>|perlvar/$@> はグローバル変数で、L<C<eval>|/eval EXPR> はオブジェクト
実装の内部で使われることがあるので、エラーオブジェクトの解析はグローバル変数の
...
L<C<die>|/die LIST> がその行動を行う
直前に実行されるコールバックを設定できます。
結び付けられたハンドラはエラーテキストと共に呼び出され、
必要なら再び L<C<die>|/die LIST> を呼び出すことでエラーテキストを
変更できます。
...
L<C<@INC>|perlvar/@INC> ディレクトリを検索する、ファイルがあったときに
L<C<%INC>|perlvar/%INC> を更新する、といったことがあります。
...
L<C<do>|/do EXPR> をプログラム設定ファイルを読み込むのに
使いたいかもしれません。
...
すると "bar" は L<C<dump>|/dump LABEL> への引数の一部になります。
...
L<C<each>|/each HASH> がハッシュをすべて読み込んでしまった後、
次の L<C<each>|/each HASH> 呼び出しでは、リストコンテキストでは空リストが
返され、スカラコンテキストでは L<C<undef>|/undef EXPR> が返されます;
I<そのあと> もう一度呼び出すと、再び反復を始めます。
ハッシュや配列毎にそれぞれ反復子があり、L<C<each>|/each HASH>、
L<C<keys>|/keys HASH>、L<C<values>|/values HASH> でアクセスされます。
反復子は、前述したように L<C<each>|/each HASH> が要素をすべて読むことによって
暗黙にリセットされます; また、ハッシュや配列に対して L<C<keys>|/keys HASH>,
L<C<values>|/values HASH> を呼び出すことで明示的にリセットできます。
繰り返しを行なっている間に、ハッシュに要素を追加したり削除したりすると、
反復子の動作は未定義です; 例えば、要素が飛ばされたり重複したりします--
従って、してはいけません。
例外: 一番最近に L<C<each>|/each HASH> から返されたものを削除するのは常に
...
Perl 5.14 から、L<C<each>|/each HASH> がスカラ式を取ることが出来るという
実験的機能がありました。
...
Perl 5.18 から C<while> ループの中に裸の L<C<each>|/each HASH> を書けます;
...
引数を省略した L<C<eof>|/eof FILEHANDLE> は、最後に読み込みを行なった
ファイルを使います。
空の括弧をつけた L<C<eof()>|/eof FILEHANDLE> は異なります。
これはコマンドラインのファイルリストで構成され、C<< <> >> 演算子経由で
アクセスされる擬似ファイルを示すために用いられます。
通常のファイルハンドルと違って C<< <> >> は明示的にオープンされないので、
C<< <> >> を使う前に L<C<eof()>|/eof FILEHANDLE> を使うと、
入力が正常か確認するために L<C<@ARGV>|perlvar/@ARGV> がテストされます。
同様に、C<< <> >> が EOF を返した後の L<C<eof()>|/eof FILEHANDLE> は、
...
L<C<eof>|/eof FILEHANDLE> か C<eof(ARGV)> を用いるのに対して
L<C<eof()>|/eof FILEHANDLE> は最後のファイルの終わりのみを調べます。
例:
...
現実的なヒント: Perl で L<C<eof>|/eof FILEHANDLE> が必要となることは、
ほとんどありません;
...
返される値は L<C<eval>|/eval EXPR> が実行されるごとにパースされることに
注意してください。
EXPR が省略されると、L<C<$_>|perlvar/$_> を評価します。
...
L<C<"unicode_eval"> 機能|feature/The 'unicode_eval' and 'evalbytes' features>
が有効の場合(これは C<use 5.16> またはそれ以上が
宣言されている場合はデフォルトです)、EXPR や L<C<$_>|perlvar/$_> は文字単位の
文字列として扱われるので、L<C<use utf8>|utf8> 宣言は無効で、ソースフィルタは
禁止されます。
L<C<"unicode_eval"> 機能|feature/The 'unicode_eval' and 'evalbytes' features>
がなければ、内部エンコーディングに依存して
時々文字単位として扱われ、時々バイト単位で扱われます; そして
L<C<eval>|/eval EXPR> の中で有効になったソースフィルタは、まだ
コンパイル中である一部の外側のファイルスコープに影響を与えるという、
間違っているけれども歴史的な振る舞いを見せます。
入力を常にバイト列として扱い、ソースフィルタが適切に動作する
L<C<evalbytes>|/evalbytes EXPR> 演算子および L<feature> プラグマを
参照してください。
...
囲む L<C<eval>|/eval EXPR> 自身がパースされるのと同じ時点です -- 現在の
...
返り値として提供される表現は、L<C<eval>|/eval EXPR> 自身のコンテキストに
...
L<C<eval>|/eval EXPR> はスカラコンテキストでは L<C<undef>|/undef EXPR> が、
...
L<C<eval>|/eval EXPR> を、STDERR に警告メッセージを表示させない目的や、
...
L<C<eval>|/eval EXPR> は、致命的エラーとなるようなものを
トラップすることができるので、
...
L<C<eval>|/eval EXPR> でも致命的エラーになるかもしれません。
L<perlrun> を参照してください。
...
L<C<eval>|/eval EXPR> では、以下のような場合に、
...
この関数は文字列引数の L<C<eval>|/eval EXPR> と同様ですが、引数(EXPR が
...
有効か、C<CORE::> 接頭辞を使ったときにのみ利用可能です。
L<C<"evalbytes"> 機能|feature/The 'unicode_eval' and 'evalbytes' features> は、
現在のスコープで C<use v5.16> (またはそれ以上) 宣言があると自動的に
有効になります。
...
L<C<exec>|/exec LIST> 関数は、システムのコマンドを実行し、I<戻ってはきません>;
戻って欲しい場合には、L<C<exec>|/exec LIST>ではなく
...
L<C<system>|/system LIST> の代わりに L<C<exec>|/exec LIST> を使うという
よくある間違いを防ぐために、L<C<exec>|/exec LIST> が無効コンテキストで
呼び出されて、引き続く文が L<C<die>|/die LIST>, L<C<warn>|/warn LIST>,
L<C<exit>|/exit EXPR> 以外の場合、Perl は警告を出します(L<warnings> が
有効の場合 -- でもいつもセットしてますよね?)。
もし I<本当に> L<C<exec>|/exec LIST> の後に他の文を書きたい場合、以下の
...
L<C<exec>|/exec LIST> や L<C<system>|/system LIST> で間接オブジェクトを
使うのもより安全です。
...
L<C<exec>|/exec LIST> は C<END> ブロックや、オブジェクトの
...
B<警告:> 配列の値に対して L<C<exists>|/exists EXPR> を呼び出すことは強く
非推奨です。
Perl の配列要素を削除したり存在を調べたりする記法は概念的に一貫しておらず、
驚くべき振る舞いを引き起こすことがあります。
...
L<C<exists>|/exists EXPR> の引数としてサブルーチン名でなくサブルーチン
呼び出しを使うと、エラーになります。
...
サブルーチンの中断に L<C<exit>|/exit EXPR> を使わないでください。
...
L<C<exit>|/exit EXPR> 関数は常に直ちに終了するわけではありません。
...
I<LATIN CAPITAL LETTER SHARP S> (U+1E9E) の L<C<fc>|/fc EXPR> は例外です。
...
L<C<fc>|/fc EXPR> は
L<C<"fc"> 機能|feature/The 'fc' feature> が有効か C<CORE::> が
前置されたときにのみ利用可能です。
L<C<"fc"> 機能|feature/The 'fc' feature> は現在のスコープで
C<use v5.16> (またはそれ以上) が宣言されると自動的に有効になります。
...
L<C<fcntl>|/fcntl FILEHANDLE,FUNCTION,SCALAR> からの返り値のチェックに
...
L<C<fcntl>|/fcntl FILEHANDLE,FUNCTION,SCALAR> は例外を
引き起こすことに注意してください。
...
OS レベルで実際のファイル記述子がない(L<C<open>|/open FILEHANDLE,EXPR> の
第 3 引数にリファレンスを
...
ディレクトリハンドルに対する L<C<fileno>|/fileno FILEHANDLE> の振る舞いは
オペレーティングシステムに依存します。
L<dirfd(3)> のようなものがあるシステムでは、ディレクトリハンドルに対する
L<C<fileno>|/fileno FILEHANDLE> はハンドルに関連付けられた基となる
ファイル記述子を返します;
...
L<C<flock>|/flock FILEHANDLE,OPERATION> は Perl の移植性のある
ファイルロックインターフェースです;
...
これは、L<C<flock>|/flock FILEHANDLE,OPERATION> を使わないプログラムが
L<C<flock>|/flock FILEHANDLE,OPERATION> でロックされたファイルを
書き換えるかもしれないことを意味します。
...
L<C<flock>|/flock FILEHANDLE,OPERATION> は
...
ネットワーク越しにはロックできない L<C<flock>|/flock FILEHANDLE,OPERATION> も
あることに注意してください;
このためには、よりシステム依存な
...
その他の L<C<flock>|/flock FILEHANDLE,OPERATION> の例としては L<DB_File> も
参照してください。
...
チャイルドプロセスの終了を待たずに、L<C<fork>|/fork> を繰り返せば、
ゾンビをためこむことになります。
...
L<C<fork>|/fork> を Perl インタプリタでエミュレートします。
...
これは、L<C<format>|/format> が使用する内部関数ですが、直接呼び出すことも
できます。
...
C<""> に戻してもかまいません。
format は通常、1 行ごとに L<C<formline>|/formline PICTURE,LIST> を
行ないますが、L<C<formline>|/formline PICTURE,LIST> 関数自身は、PICTURE の中に
...
L<C<formline>|/formline PICTURE,LIST> は常に真を返します。
...
L<C<getlogin>|/getlogin> を認証に使ってはいけません: これは
...
POSIX 版の L<C<getpgrp>|/getpgrp PID> は PID 引数を受け付けないので、
...
L<C<gethostbyname>|/gethostbyname NAME> はスカラコンテキストで
...
L<C<getprotobynumber>|/getprotobynumber NUMBER> 関数は、一つの引数しか
...
L<C<glob>|/glob EXPR> は引数を空白で分割して、それぞれを分割された
パターンとして扱います。
...
空でない中かっこが L<C<glob>|/glob EXPR> で使われている唯一の
...
良いでしょう。
Perl の作者はこの形式の L<C<goto>|/goto LABEL> を使う必要を感じたことは、
1 度もありません (Perl では; C は別のお話です)。
(違いは、C にはループ制御と結びついた名前つきのループがないことです。
Perl にはあり、これが他の言語でのほとんどの構造的な L<C<goto>|/goto LABEL> の
使用法を置き換えます。)
...
これにより FORTRAN のような算術 L<C<goto>|/goto LABEL> が可能になりますが、
保守性を重視するならお勧めしません。
...
C<goto &NAME> の形式は、その他の L<C<goto>|/goto LABEL> の形式とはかなり
異なったものです。
...
その別のサブルーチンに伝えられます)。
L<C<goto>|/goto LABEL> のあとは、L<C<caller>|/caller EXPR> でさえも、現在の
...
L<C<grep>|/grep BLOCK LIST> で)修正すると元のリストの要素が変更されます。
...
BLOCK や EXPR の結果をリストの形にしたい場合は L<C<map>|/map BLOCK LIST> を
参照してください。
...
L<C<ioctl>|/ioctl FILEHANDLE,FUNCTION,SCALAR> コールの
3 番目の引数として渡されます。
(SCALAR が文字列値を持っておらず、数値を持っている場合には、
文字列値へのポインタの代わりに、その値が渡されます。
このことを保証するためには、使用する前に SCALAR にC<0> を足してください。)
L<C<ioctl>|/ioctl FILEHANDLE,FUNCTION,SCALAR> で使われる構造体の値を
...
L<C<join>|/join EXPR,LIST> は最初の引数にパターンは取れないことに
注意してください。
...
副作用として、L<C<keys>|/keys HASH> の呼び出しは HASH や ARRAY の反復子を
初期化します (L<C<each>|/each HASH> を参照してください)。
特に、無効コンテキストで L<C<keys>|/keys HASH> を呼び出すと
...
左辺値として使うことで、L<C<keys>|/keys HASH> を使うことで与えられたハッシュに
...
C<undef %hash> を使います。
この方法で L<C<keys>|/keys HASH> を使うことで、表の大きさを小さくすることは
できません
...
Perl 5.14 から、L<C<keys>|/keys HASH> がスカラ式を取ることが出来るという
実験的機能がありました。
この実験は失敗と見なされ、Perl 5.24 で削除されました。
...
シグナルは送られませんが、L<C<kill>|/kill SIGNAL, LIST> は、
...
L<C<last>|/last LABEL> コマンドは、(ループ内で使った) C の C<break> 文と
...
L<C<last>|/last LABEL> は C<eval {}>, C<sub {}>, C<do {}> といった
...
注意してください。
従って、L<C<last>|/last LABEL> でそのようなブロックを途中で
抜け出すことができます。
...
すると "bar" は L<C<last>|/last LABEL> への引数の一部となります。
...
L<C<local>|/local EXPR> はほとんどの人々が「ローカル」と考えるものと
違うからです。
...
EXPR が省略されると、L<C<localtime>|/localtime EXPR> は
...
スカラコンテキストでは、L<C<localtime>|/localtime EXPR> は L<ctime(3)> の値を
返します:
...
L<C<strftime>|POSIX/C<strftime>> と L<C<mktime>|POSIX/C<mktime>> 関数も
参照してください。
...
L<C<gmtime>|/gmtime EXPR> 関数と L<C<localtime>|/localtime EXPR> 関数に、
...
L<C<lock>|/lock THING> は「弱いキーワード」です; もしユーザーが(呼び出し前に)
...
与えないより、寛容な MASK でディレクトリを作り、ユーザーが自身の
L<C<umask>|/umask EXPR> で修正するようにした方がよいです。
例外は、(例えばメールファイルのような)プライベートに保つべきファイルや
ディレクトリを書く場合です。
L<C<umask>|/umask EXPR> の文書で、MASK の選択に関して詳細に議論しています。
...
返します。
L<perlipc/"SysV IPC"> および、L<C<IPC::SysV>|IPC::SysV>,
...
L<C<my>|/my VARLIST> はリストアップされた変数を、囲っているブロック、ファイル、
...
L<C<next>|/next LABEL> コマンドは、C での C<continue> 文のようなもので、
ループの次の繰り返しを開始します:
...
L<C<next>|/next LABEL> は C<eval {}>, C<sub {}>, C<do {}> のように値を返す
ブロックから抜けるのには使えません; また、L<C<grep>|/grep BLOCK LIST> や
L<C<map>|/map BLOCK LIST> 操作から抜けるのに使うべきではありません。
...
ブロック自身は一回だけ実行されるループと文法的に同一であることに
注意してください。
従って、L<C<next>|/next LABEL> はそのようなブロックから早く抜けるのに使えます。
...
すると "bar" は L<C<next>|/next LABEL> への引数の一部となります。
...
L<C<no>|/no MODULE VERSION LIST> は、その逆を行なうものです。
...
L<C<oct>|/oct EXPR> 関数は例えば、 C<644> といった文字列をファイルモードに
変換する時によく使います。
...
(L<C<oct>|/oct EXPR> は非負整数のみを扱えます; 負の整数や小数は扱えません)。
...
(以下は総合的な L<C<open>|/open FILEHANDLE,EXPR> のリファレンスです:
より親切な説明については L<perlopentut> を参照してください。)
...
L<C<open>|/open FILEHANDLE,EXPR> は安全です。
...
参照してください。
(L<C<open>|/open FILEHANDLE,EXPR> を入出力 I<両用> にパイプすることは
...
コマンドへの引数となります。
パイプモードではない L<C<open>|/open FILEHANDLE,EXPR> での三つ以上の引数の
意味はまだ未定義ですが、実験的な「層」は追加の LIST 引数の意味を与えます。
...
通常は L<open> プラグマか C<-CioD> オプションでセットされます) に保存された
...
パイプに関る L<C<open>|/open FILEHANDLE,EXPR> のときには、返り値は
サブプロセスの pid となります。
...
考えなので、L<C<open>|/open FILEHANDLE,EXPR> はしばしば
L<C<die>|/die LIST> と結び付けられて使われます。
望むものが L<C<die>|/die LIST> でない場合(例えば、CGI スクリプトのように
...
特別な場合として、3 引数の形で読み書きモードで 3 番目の引数が L<C<undef>|/undef EXPR> の場合:
...
しかし C<< open(my $A, ">>&=", $B) >> ではファイルハンドルは基礎となシステムの
同じファイル記述子を共有します。
...
1 引数 または 2 引数の形の L<C<open>|/open FILEHANDLE,EXPR>) で
(C<-|> や C<|-> というふうに) C<-> というコマンドにパイプを開くと、
暗黙の L<C<fork>|/fork> が行なわれるので、
L<C<open>|/open FILEHANDLE,EXPR> は 2 回返ります;
...
よい経験則としては、もし実行しているプラットフォームで本当の
L<C<fork>|/fork> があれば(言い換えると、プラットフォームが Linux や
MacOS X を含む Unix なら)リスト形式が使えます。
...
1 引数 と 2 引数の形の L<C<open>|/open FILEHANDLE,EXPR> に渡された
...
L<C<open>|/open FILEHANDLE,EXPR> の I<magic> と I<3 引数> 形式を誠実に
選択するべきです。
...
L<C<open>|/open FILEHANDLE,EXPR> とは違うファイルモードを持ちます)。
...
L<C<our>|/our VARLIST> は単純名を、現在のレキシカルスコープ内で使うために、
...
L<C<our>|/our VARLIST> は L<C<my>|/my VARLIST> や
L<C<state>|/state VARLIST> と同じスコープルールを持ちます; つまり
レキシカルスコープの中でだけ有効です。
新しい(レキシカル)変数を宣言する L<C<my>|/my VARLIST> や
L<C<state>|/state VARLIST> と異なり、L<C<our>|/our VARLIST> は既に
...
L<C<our>|/our VARLIST> 宣言のレキシカルスコープ内だけということです。
...
L<C<our>|/our VARLIST> 宣言はレキシカルスコープ全体に対して (たとえ
...
L<C<our>|/our VARLIST> 宣言するのは単に冗長です。
...
L<C<our>|/our VARLIST> 宣言には、それと結び付けられる属性のリストを
持つこともあります。
...
L<C<our>|/our VARLIST> は L<C<use vars>|vars> と異なります; スコープを
またぐのではなく、影響するパッケージの内側 I<のみ> で完全修飾されていない
名前を使えるようにします。
...
このようなフォーマット文字のそれぞれについて、L<C<pack>|/pack TEMPLATE,LIST> は
結果の 4 ビットを生成します。
...
L<C<pack>|/pack TEMPLATE,LIST> では I<length-item>C</>I<string-item> の
形になり、
...
L<C<pack>|/pack TEMPLATE,LIST> では、I<sequence-item> は繰り返し数を
...
これらの例は、L<C<pack>|/pack TEMPLATE,LIST>/
L<C<unpack>|/unpack TEMPLATE,EXPR> を L<Encode> モジュールの代わりとして
...
L<C<pack>|/pack TEMPLATE,LIST> や L<C<unpack>|/unpack TEMPLATE,EXPR> は、
文字列がどこへ行くかやどこから来たかを 
...
例えば、以下のような C 構造体を L<C<pack>|/pack TEMPLATE,LIST> または
...
L<C<package>|/package NAMESPACE> 宣言によって上書きされるか、
...
VERSION が指定されると、L<C<package>|/package NAMESPACE> は与えられた
名前空間の C<$VERSION> 変数に、
...
しかしこれは他の場合にも起こり得ます。
ARRAY が省略されると、L<C<shift>|/shift ARRAY> と同様に、メインプログラムでは
...
Perl 5.14 から、L<C<pop>|/pop ARRAY> がスカラ式を取ることが出来るという
実験的機能がありました。
この実験は失敗と見なされ、Perl 5.24 で削除されました。
...
L<C<pos>|/pos SCALAR> は正規表現エンジンがオフセットを保存するために使う場所を
直接アクセスするので、L<C<pos>|/pos SCALAR> への代入はオフセットを変更し、
そのような変更は正規表現における C<\G> ゼロ幅アサートにも影響を与えます。
これらの効果の両方は次のマッチングのために行われるので、
C<(?{pos() = 5})> や C<s//pos() = 5/e> のように現在のマッチング中の
L<C<pos>|/pos SCALAR> の位置には影響を与えません。
...
L<C<pos>|/pos SCALAR> からの返り値はどちらの場合も変更されません。
...
評価されます; L<C<print>|/print FILEHANDLE LIST> に渡した、リストを返す
サブルーチンも含みます。
...
L<C<printf>|/printf FILEHANDLE FORMAT, LIST> を使ってしまう
罠にかからないようにしてください。
...
適切に記述できない場合、L<C<prototype>|/prototype FUNCTION> は
L<C<undef>|/undef EXPR> を返します;
...
L<C<push>|/push ARRAY,LIST> の処理終了後の配列の要素数を返します。
...
Perl 5.14 から、L<C<push>|/push ARRAY,LIST> がスカラ式を取ることが出来るという
実験的機能がありました。
この実験は失敗と見なされ、Perl 5.24 で削除されました。
...
必ず L<C<quotemeta>|/quotemeta EXPR> か C<\Q> を使わなければなりません。
...
変更される可能性があります)。
L<C<srand>|/srand EXPR> が既に呼ばれている場合以外は、自動的に
L<C<srand>|/srand EXPR> 関数を呼び出します。
L<C<srand>|/srand EXPR> も参照してください。
...
ランダムな小数ではなく、ランダムな整数がほしい場合は、
L<C<rand>|/rand EXPR> から返された値に L<C<int>|/int EXPR> を
適用してください。
例えば:
...
例えばファイルハンドルが C<:utf8> I/O 層(L<C<open>|/open FILEHANDLE,EXPR>,
L<open> プラグマを参照してください) で開かれた場合、I/O はバイトではなく、
...
L<C<readdir>|/readdir DIRHANDLE> の返り値をファイルテストに使おうと
...
Perl 5.12 から裸の L<C<readdir>|/readdir DIRHANDLE> を C<while> で
...
L<C<readline>|/readline EXPR> はスカラコンテキスト (つまりファイル吸い込み
...
L<C<readline>|/readline EXPR> のエラーは C<ARGV> ファイルハンドルの方法では
扱えないことに注意してください。
...
文字を受信します。
デフォルトでは全てのソケットはバイトを処理しますが、例えばソケットが
L<C<binmode>|/binmode FILEHANDLE, LAYER> で C<:encoding(utf8)> I/O 層
(L<open> プラグマを参照してください) を使うように指定された場合、
I/O はバイトではなく、UTF8 エンコードされた Unicode 文字を操作します。
C<:encoding> 層も同様です:
この場合、ほとんど大体全ての文字が読み込めます。
...
L<C<redo>|/redo LABEL> コマンドは、条件を再評価しないで、ループブロックの
始めからもう一度実行を開始します。
...
L<C<redo>|/redo LABEL> は C<eval {}>, C<sub {}>, C<do {}> のように値を返す
ブロックを繰り返すのには使えません; また、L<C<grep>|/grep BLOCK LIST> や
L<C<map>|/map BLOCK LIST> 操作から抜けるのに使うべきではありません。
...
ブロック自身は一回だけ実行されるループと文法的に同一であることに
注意してください。
従って、ブロックの中で L<C<redo>|/redo LABEL> を使うことで効果的に
ループ構造に変換します。
...
すると "bar" は L<C<redo>|/redo LABEL> への引数の一部となります。
...
空文字列を返します。
EXPR が指定されなければ、L<C<$_>|perlvar/$_> が使われます。
...
L<C<ref>|/ref EXPR> は、C<typeof> 演算子のように考えることができます。
...
これは、L<C<pos>|/pos SCALAR> や
L<C<substr>|/substr EXPR,OFFSET,LENGTH,REPLACEMENT> のような関数呼び出しの
リファレンスから得られます。
...
C<Regexp> という結果は、引数が
L<C<qrE<sol>E<sol>>|/qrE<sol>STRINGE<sol>> からの結果である
正規表現であることを意味します。
...
クラスを使うかも知れないからです。
また、L<C<ref>|/ref EXPR> は
L<C<isa>|UNIVERSAL/C<< $obj->isa( TYPE ) >>> のようにサブクラスを
考慮したりはしません。
...
L<C<blessed>|Scalar::Util/blessed> を、特定のクラスのチェックには
L<C<isa>|UNIVERSAL/C<< $obj->isa( TYPE ) >>> を、型のチェックには
(これも L<Scalar::Util> にある) L<C<reftype>|Scalar::Util/reftype> を
使ってください。
(詳細と L<C<blessed>|Scalar::Util/blessed>/
L<C<isa>|UNIVERSAL/C<< $obj->isa( TYPE ) >>> の例については
L<perlobj> を参照してください。)
...
VERSION は 5.006 のような数値(L<C<$]>|perlvar/$]> と比較されます)か、
v5.6.1 の形 (L<C<$^V>|perlvar/$^V> (または L<English> モジュールでは
C<$PERL_VERSION>) と比較されます)で指定します。
...
それ以外の場合には、L<C<require>|/require VERSION> は、既に
...
L<C<require>|/require VERSION> は拡張子が F<.pm> であり、C<::> を C</> に
...
引数が裸の単語の場合、L<C<require>|/require VERSION> がどのようにファイルを
探すかを理解してください; 水面下でちょっとした追加の機能があります。
L<C<require>|/require VERSION> が拡張子 F<.pm> のファイルを探す前に、まず
...
L<C<@INC>|perlvar/@INC> 配列に直接 Perl コードを入れることで、インポート機能に
フックを挿入できます。
...
出会った場合、このサブルーチンは二つの引数と共に呼び出されます;
...
一致しないものが返されると、L<C<require>|/require VERSION> は
...
フックがオブジェクトの場合、C<INC> メソッドを提供している必要があります;
...
L<C<use>|/use Module VERSION LIST> の項と、L<perlmod> を参照してください。
...
(但し L<C<grep>|/grep BLOCK LIST> や
L<C<map>|/map BLOCK LIST> ブロックではない) から
...
L<C<return>|/return EXPR> がなければ、最後に評価された値で、
自動的にリターンします。)
...
L<C<return>|/return EXPR> への引数の一部となります。
...
スカラコンテキストで引数なしで使うと、L<C<reverse>|/reverse LIST> は
...
そのディレクトリを削除します。
成功時には真を返します; さもなければ偽を返して L<C<$!>|perlvar/$!> (errno) を
設定します。
...
L<C<print>|/print FILEHANDLE LIST> と同様ですが、暗黙に改行が追加されます。
...
L<C<scalar>|/scalar EXPR> は単項演算子なので、EXPR として括弧でくくった
...
使って)文字を操作するように設定されていたとしても、
L<C<tell>|/tell FILEHANDLE> は文字のオフセットではなくバイトのオフセットを
返すことに注意してください
(なぜならこれを実装すると L<C<seek>|/seek FILEHANDLE,POSITION,WHENCE> と
L<C<tell>|/tell FILEHANDLE> が遅くなってしまうからです)。
...
位置を指定したい場合は、L<C<seek>|/seek FILEHANDLE,POSITION,WHENCE> は
...
(おそらく) ダミーの L<C<seek>|/seek FILEHANDLE,POSITION,WHENCE> をすることで
リセットする必要があります。
L<C<seek>|/seek FILEHANDLE,POSITION,WHENCE> は現在の位置を変更しませんが、
...
L<C<seekdir>|/seekdir DIRHANDLE,POS> は同名のシステムライブラリルーチンと
...
L<C<write>|/write FILEHANDLE> や L<C<print>|/print FILEHANDLE LIST> を
...
L<C<select>|/select RBITS,WBITS,EBITS,TIMEOUT> をスカラコンテキストで
...
L<C<select>|/select RBITS,WBITS,EBITS,TIMEOUT> がシグナル (例えば、SIGALRM) の
後に再起動するかどうかは実装依存であることに注意してください。
L<C<select>|/select RBITS,WBITS,EBITS,TIMEOUT> の移植性に関する
...
エラー時は、L<C<select>|/select RBITS,WBITS,EBITS,TIMEOUT> は
...
L<C<select>|/select RBITS,WBITS,EBITS,TIMEOUT> へのよりユーザーフレンドリーな
...
L<C<select>|/select RBITS,WBITS,EBITS,TIMEOUT> を
...
L<perlipc/"SysV IPC"> と、
L<C<IPC::SysV>|IPC::SysV>, L<C<IPC::Semaphore>|IPC::Semaphore> の文書も
参照してください。
...
L<perlipc/"SysV IPC"> と、L<C<IPC::SysV>|IPC::SysV>,
L<C<IPC::Semaphore>|IPC::Semaphore> 文書も参照してください。
...
例えばソケットが L<C<binmode>|/binmode FILEHANDLE, LAYER> で
C<:encoding(utf8)> I/O 層(L<C<open>|/open FILEHANDLE,EXPR>、
L<open> プラグマを参照してください) を使うように指定された場合、
...
BSD 4.2 版の L<C<setpgrp>|/setpgrp PID,PGRP> は引数を取ることができないので、
...
L<C<@_>|perlvar/@_> を、サブルーチンの外側で、C<eval STRING>, C<BEGIN {}>,
C<INIT {}>, C<CHECK {}>, C<UNITCHECK {}>, C<END {}> で作成された
レキシカルスコープでは L<C<@ARGV>|perlvar/@ARGV> が用いられます。
...
Perl 5.14 から、L<C<shift>|/shift ARRAY> がスカラ式を取ることが出来るという
実験的機能がありました。
この実験は失敗と見なされ、Perl 5.24 で削除されました。
...
L<C<shift>|/shift ARRAY> と L<C<unshift>|/unshift ARRAY,LIST> は、
...
L<perlipc/"SysV IPC"> と L<C<IPC::SysV>|IPC::SysV> の文書も参照してください。
...
成功時には真を、エラー時には偽を返します。
L<C<shmread>|/shmread ID,VAR,POS,SIZE> は変数を汚染します。
...
L<C<sleep>|/sleep EXPR> は、L<C<alarm>|/alarm SECONDS> を使って
...
L<C<socketpair>|/socketpair SOCKET1,SOCKET2,DOMAIN,TYPE,PROTOCOL> を使って
定義しているシステムもあります;
...
スカラコンテキストでは、L<C<sort>|/sort SUBNAME LIST> の振る舞いは未定義です。
...
SUBNAME や BLOCK を省略すると、L<C<sort>|/sort SUBNAME LIST> は標準の
文字列比較の順番で行なわれます。
SUBNAME を指定すると、それは、リストの要素をどのような順番に並べるかに
応じて、負の整数、C<0>、正の整数を返すサブルーチンの名前であると解釈されます。
...
後者の場合、レキシカルに C<$a> と C<$b> を宣言するのは普通とても
逆効果になります。
...
L<C<sort>|/sort SUBNAME LIST> は元のリストへのエイリアスを返します;
for ループのインデックス変数がリスト要素へのエイリアスと同様です。
つまり、L<C<sort>|/sort SUBNAME LIST> で返されるリストの要素を(例えば、
...
このアルゴリズムは安定しておらず、2 乗の時間が掛かる I<可能性があります>。
...
速くなります。
5.8 ではソートを限定的に制御できる L<sort> プラグマがあります。
...
C<$a> と C<$b> をレキシカルとして宣言しては I<いけません>。
これはパッケージグローバルです。
...
L<C<splice>|/splice ARRAY,OFFSET,LENGTH,LIST> は、例えば、n-ary キュー処理の
実装に使えます:
...
Perl 5.14 から、L<C<splice>|/splice ARRAY,OFFSET,LENGTH,LIST> がスカラ式を
...
C<'abc'> の C<b> をセパレータとして使って出力 C<a:c> を生成します。
しかし、これは:
...
空文字列マッチングをセパレータとして使って出力 C<a:b:c> を生成します; 従って、
...
L<C<split>|/split E<sol>PATTERNE<sol>,EXPR,LIMIT> の特殊な場合として、
...
もう一つの特別な場合として、
L<C<split>|/split E<sol>PATTERNE<sol>,EXPR,LIMIT> は
...
これは(S<C<: :a:b:c>> ではなく)出力 S<C< :a:b:c>> を生成します。
...
C ライブラリ関数 L<C<sprintf>|/sprintf FORMAT, LIST> の
普通の L<C<printf>|/printf FILEHANDLE FORMAT, LIST> 記法の
整形された文字列を返します。
...
Perl は L<C<sprintf>|/sprintf FORMAT, LIST> フォーマット処理を自力で行います:
...
L<C<printf>|/printf FILEHANDLE FORMAT, LIST> と違って、
L<C<sprintf>|/sprintf FORMAT, LIST> の最初の引数に配列を渡しても
あなたが多分望むとおりには動作しません。
...
Perl の L<C<sprintf>|/sprintf FORMAT, LIST> は以下の一般に知られている変換に
対応しています:
...
通常、L<C<sprintf>|/sprintf FORMAT, LIST> は各フォーマット指定について、
使われていない次の引数を
フォーマットする値として使います。
...
設定することです。
L<C<srand>|/srand EXPR> を引数付きで呼び出すと、これを種として使います;
さもなければ(だいたい)ランダムに種を選びます。
...
L<C<srand>|/srand EXPR> が明示的に呼び出されなかった場合、最初に
L<C<rand>|/rand EXPR> 演算子を使った時点で暗黙に引数なしで呼び出されます。
しかし、最近の Perl でプログラムが L<C<srand>|/srand EXPR> を
...
共有することを避けるために、L<C<fork>|/fork> の後に L<C<srand>|/srand EXPR> を
呼び出したいかもしれません。
...
L<C<srand>|/srand EXPR> (引数なし)をプロセス中で複数回
呼び出しては B<いけません>。
...
L<C<srand>|/srand EXPR> のほとんどの実装では整数を取り、小数を暗黙に
切り捨てます。
これは、C<srand(42)> は普通 C<srand(42.1)> と同じ結果になることを意味します。
安全のために、L<C<srand>|/srand EXPR> には常に整数を渡しましょう。
...
(C<_> ではありません!)。
L<C<stat>|/stat FILEHANDLE> に失敗した場合には、空リストを返します。
普通は、以下のようにして使います:
...
下線だけの _ という特別なファイルハンドルを L<C<stat>|/stat FILEHANDLE> に
渡すと、実際には stat を行なわず、stat 構造体に残っている
前回の L<C<stat>|/stat FILEHANDLE>, L<C<lstat>|/lstat FILEHANDLE> や
ファイルテストの情報が返されます。
例:
...
スカラコンテキストでは、L<C<stat>|/stat FILEHANDLE> は成功か失敗を表す真偽値を
...
モード定数 (C<S_IF*>) と関数 (C<S_IS*>) を L<Fcntl> モジュールから
インポートできます。
...
C<S_*> 定数に関する詳細についてはネイティブの L<chmod(2)> と L<stat(2)> の
ドキュメントを参照してください。
...
L<C<state>|/state VARLIST> はちょうど L<C<my>|/my VARLIST> と同様に、
レキシカルなスコープの変数を宣言します。
...
B<Perl バージョン 5.16 からこの関数は何もしませんが、これは将来のリリースで
...
SCALAR (未指定の場合は L<C<$_>|perlvar/$_>) を学習するために追加の時間を
使います。
...
(L<C<study>|/study SCALAR> の動作方法は次のものでした: 検索される文字列の
全ての文字のリンクリストが作られるので、例えば、全ての C<'k'> 文字が
どこにあるかを知ります。
...
C<f> は C<o> よりも珍しいので、C</\bfoo\b/> を探すとき、L<C<$_>|perlvar/$_> で
C<f> を含む場所だけが探されます。
...
ループ全体を文字列として組み立てて、L<C<eval>|/eval EXPR> すれば、
いつも、すべてのパターンを再コンパイルするという事態は避けられます。
ファイル全体を一つのレコードとして入力するために、
L<C<$E<sol>>|perlvar/$E<sol>> を未定義にすれば、かなり速くなり、
多くの場合 L<fgrep(1)> のような専用のプログラムより速くなります。
...
L<C<substr>|/substr EXPR,OFFSET,LENGTH,REPLACEMENT> を左辺値として
...
文字列の内側の部分だけが返されます。
部分文字列が文字列の両端の外側の場合、
L<C<substr>|/substr EXPR,OFFSET,LENGTH,REPLACEMENT> は未定義値を返し、
警告が出力されます。
...
L<C<substr>|/substr EXPR,OFFSET,LENGTH,REPLACEMENT> を左辺値として使う
...
3 引数の L<C<substr>|/substr EXPR,OFFSET,LENGTH,REPLACEMENT> によって返された
...
L<C<syscall>|/syscall NUMBER, LIST> の引数として使うことはできません;
...
L<C<open>|/open FILEHANDLE,EXPR> 呼び出しがそれを作った場合、
PERMS の値は新しく作られたファイルの権限を指定します。
L<C<sysopen>|/sysopen FILEHANDLE,FILENAME,MODE> の PERMS 引数を省略した場合、
...
存在していた場合、L<C<sysopen>|/sysopen FILEHANDLE,FILENAME,MODE> が
失敗することを意味します。
...
めったなことでは L<C<sysopen>|/sysopen FILEHANDLE,FILENAME,MODE> の引数に
...
5.8.0 より古い Perl では、
L<C<sysopen>|/sysopen FILEHANDLE,FILENAME,MODE> は
...
Perl 5.8.0 以降では、(ほぼ確実に) PerlIO がデフォルトです。
...
L<C<sysread>|/sysread FILEHANDLE,SCALAR,LENGTH,OFFSET> を使って、
...
Unicode 文字が読み込まれます
(L<C<sysread>|/sysread FILEHANDLE,SCALAR,LENGTH,OFFSET> の LENGTH, OFFSET
および返り値は Unicode 文字になります)。
C<:encoding(...)> 層は暗黙のうちに C<:utf8> 層が導入されます。
L<C<binmode>|/binmode FILEHANDLE, LAYER>, L<C<open>|/open FILEHANDLE,EXPR>,
...
FILEHANDLE のシステム位置をバイト単位で L<lseek(2)> を使って設定します。
FILEHANDLE は、実際のファイルハンドル名を与える式でもかまいません。
...
L<C<tell>|/tell FILEHANDLE> は文字のオフセットではなくバイトのオフセットを
返します (なぜならこれを実装すると
L<C<sysseek>|/sysseek FILEHANDLE,POSITION,WHENCE> が受け入れられないほど
遅くなるからです)。
...
L<C<sysseek>|/sysseek FILEHANDLE,POSITION,WHENCE> は普通のバッファ付き IO を
バイパスしますので、
...
L<C<sysseek>|/sysseek FILEHANDLE,POSITION,WHENCE> は成功時に真を返し、
...
もし L<C<system>|/system LIST> (及び Perl のその他の多くの部分) でエラー時に
...
L<C<exec>|/exec LIST> と同様に、L<C<system>|/system LIST> でも
...
C<SIGINT> と C<SIGQUIT> は L<C<system>|/system LIST> の実行中は無視されるので、
...
L<C<system>|/system LIST> の失敗を手動で検査したいなら、以下のように
...
L<C<system>|/system LIST> の引数がシェルによって間接的に実行された場合、
...
L<C<system>|/system LIST> は L<C<fork>|/fork> と L<C<wait>|/wait> を行うので、
...
L<C<syswrite>|/syswrite FILEHANDLE,SCALAR,LENGTH,OFFSET> の LENGTH, OFFSET
および返り値は (UTF8 エンコードされた Unicode) 文字単位になります。
C<:encoding(...)> 層は暗黙のうちに C<:utf8> 層が導入されます。
...
文字を操作するように設定されていたとしても、L<C<tell>|/tell FILEHANDLE> は
文字のオフセットではなくバイトのオフセットを返すことに注意してください
(なぜならこれは L<C<seek>|/seek FILEHANDLE,POSITION,WHENCE> と
L<C<tell>|/tell FILEHANDLE> が遅くなってしまうからです)。
...
STDIN のような標準ストリームに対する L<C<tell>|/tell FILEHANDLE> の返り値は
OS に依存します:
-1 やその他の値が返ってくるかもしれません。
パイプ、FIFO、ソケットに対して L<C<tell>|/tell FILEHANDLE> を使うと、普通は
-1 が返ります。
...
ファイルハンドルに L<C<tell>|/tell FILEHANDLE>
(またはその他のバッファリング I/O 操作) を使わないでください。
これらの関数はバッファリングを無視しますが、L<C<tell>|/tell FILEHANDLE> は
違います。
...
L<C<telldir>|/telldir DIRHANDLE> は同名のシステムライブラリルーチンと同じく、
ディレクトリ縮小時の問題が考えられます。
...
L<C<tie>|/tie VARIABLE,CLASSNAME,LIST> 関数でも返されます;
...
L<C<tie>|/tie VARIABLE,CLASSNAME,LIST> 関数はモジュールを
L<C<use>|/use Module VERSION LIST> したり
L<C<require>|/require VERSION> したりしません;
自分で明示的に行う必要があります。
L<C<tie>|/tie VARIABLE,CLASSNAME,LIST> の興味深い実装については
...
スカラコンテキストでは、L<C<times>|/times> は C<$user> を返します。
...
このためには L<C<ucfirst>|/ucfirst EXPR> を参照してください。
...
L<C<umask>|/umask EXPR> の値は無効にするパーミッションビットのこのような
数値表現です。
L<C<mkdir>|/mkdir FILENAME,MASK> や
...
L<C<umask>|/umask EXPR> が C<0027> (グループは書き込めない; その他は読み込み、
書き込み、実行できない) のとき
...
C<0666> を、(L<C<mkdir>|/mkdir FILENAME,MASK> による)ディレクトリでは
C<0777> を指定しましょう。
...
エラーの場合、L<C<unlink>|/unlink LIST> はどのファイルが削除できなかったかを
知らせません。
...
L<C<unlink>|/unlink LIST> はディレクトリを削除しようとすることはありません。
この条件にあう場合にも、ディレクトリの削除は、
ファイルシステムに多大な損害を与える可能性があります。
最後に、L<C<unlink>|/unlink LIST> をディレクトリに使うのはほとんどの OS では
対応していません。
...
LIST が省略されると、L<C<unlink>|/unlink LIST> は L<C<$_>|perlvar/$_> を
使います。
...
L<C<unpack>|/unpack TEMPLATE,EXPR> は L<C<pack>|/pack TEMPLATE,LIST> の逆を
行ないます: 構造体を表わす文字列をとり、
リスト値に展開し、その配列値を返します。
...
Perl は L<C<unpack>|/unpack TEMPLATE,EXPR> に渡された値が有効なメモリ位置を
...
L<C<unpack>|/unpack TEMPLATE,EXPR> が空文字列や 0 を
返すこともありますし、例外が発生します。
...
L<C<shift>|/shift ARRAY> の逆操作を行ないます。
見方を変えれば、L<C<push>|/push ARRAY,LIST> の逆操作とも考えられます。
...
Perl 5.14 から、L<C<unshift>|/unshift ARRAY,LIST> がスカラ式を
取ることが出来るという実験的機能がありました。
この実験は失敗と見なされ、Perl 5.24 で削除されました。
...
特に C<use VERSION> の形式では、VERSION は 5.006 のような正の 10 進小数
(L<C<$]>|perlvar/$]> と比較されます)か、v5.6.1 の形
(L<C<$^V>|perlvar/$^V> (またの名を $PERL_VERSION) と比較されます) の
v-文字列で指定します。
VERSION が Perl の現在のバージョンより大きいと、例外が発生します;
Perl はファイルの残りを読み込みません。
L<C<require>|/require VERSION> と似ていますが、これは実行時にチェックされます。
...
L<C<use>|/use Module VERSION LIST> する前に、現在の Perl のバージョンを
調べたい場合に有用です。
...
C<use VERSION> は、L<feature> プラグマで定義されたように、指定された
バージョンで利用可能な全ての機能を有効にし、指定されたバージョンの機能の
束にない機能をレキシカルに無効にします。
L<feature> を参照してください。
同様に、指定された Perl のバージョンが 5.12.0 以上の場合、
制限は L<C<use strict>|strict> と同様にレキシカルに有効になります。
...
L<C<use>|/use Module VERSION LIST> は Module クラスの
...
L<C<use>|/use Module VERSION LIST> はコンパイル時に有効なので、コードが
コンパイルされる際の通常の流れ制御には従いません。
特に、条件文のうち成立しない側の中に L<C<use>|/use Module VERSION LIST> を
書いても、処理を妨げられません。
...
L<C<use>|/use Module VERSION LIST> によってインポートされたものを、
...
L<C<no>|/no MODULE VERSION LIST> の C<no VERSION> 形式を使うときには
注意を払うべきです。
...
コマンドラインから L<C<use>|/use Module VERSION LIST> 機能を
指定するための C<-M> と C<-m> の
...
副作用として、L<C<values>|/values HASH> を呼び出すと HASH や ARRAY の
内部反復子をリセットします; L<C<each>|/each HASH> を参照してください。
(特に、L<C<values>|/values HASH> を無効コンテキストで呼び出すとその他の
...
Perl 5.14 から、L<C<values>|/values HASH> がスカラ式を取ることが出来るという
実験的機能がありました。
この実験は失敗と見なされ、Perl 5.24 で削除されました。
...
L<C<vec>|/vec EXPR,OFFSET,BITS> は左辺値として、代入の
...
セットされている場合)、これは L<C<vec>|/vec EXPR,OFFSET,BITS> では無視され、
たとえ値が 256 未満の文字だけであったとしても、概念的な文字列ではなく
内部バイト文字列で操作されます。
...
L<C<vec>|/vec EXPR,OFFSET,BITS> で作られた文字列は、論理演算子 C<|>、C<&>、
C<^>, C<~> で扱うこともできます。
...
L<C<wait>|/wait> を L<C<$SIG{CHLD}>|perlvar/%SIG> のハンドラで使うと、誤って
...
ブロック内、C<DESTROY> メソッド内では L<C<wantarray>|/wantarray> の結果は
未定義です。
...
LIST の最後の要素が改行で終わっていない場合、L<C<die>|/die LIST> が行うのと
同様のファイル/行番号のテキストが追加されます。
...
出力が空かつ、(典型的には以前の eval によって) L<C<$@>|perlvar/$@> に既に値が
入っている場合、L<C<$@>|perlvar/$@> に C<"\t...caught"> を追加した値が
用いられます。
これはほとんどそのままにするときに便利ですが、
L<C<die>|/die LIST> と全体的に似ているわけではありません。
...
L<C<$@>|perlvar/$@> が空の場合は、C<"Warning: Something's wrong"> という
文字列が使われます。
...
L<C<$SIG{__WARN__}>|perlvar/%SIG> ハンドラが設定されている場合は何の
メッセージも表示されません。
メッセージをどう扱うか(例えば L<C<die>|/die LIST> に変換するか)はハンドラの
責任ということです。
従ってほとんどのハンドラは、扱おうと準備していない警告を表示するために、
ハンドラの中で L<C<warn>|/warn LIST> を再び呼び出します。
