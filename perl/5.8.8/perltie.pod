...
The default implementations of DELETE and EXISTS in
B<Tie::Array> simply C<croak>.
(TBT)
...
すべてのキーをイテレートするための関数 keys() と each() を実装する
FIRSTKEY と NEXTKEY、
...
このメソッドは keys() または each() イテレーションの間に呼び出されます。
二番目の引数として、最後にアクセスしたキーをとります。
これは、あなたが順番に取り出すとか、二度以上イテレータを呼び出したり、
