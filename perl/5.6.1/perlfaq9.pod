...
HTTPD::UserAdmin モジュールと HTTPD::GroupAdminモジュールは、
...
一引数形式のsystem()やexec()を決して使わないようにします。
...
quick-and-dirty な解決策なら、L<perlfunc/split> から派生した
以下のやり方を試してみてください。
...
Mail::InternetモジュールはMail::MailerよりUNIX的ではない
Net::SMTPを使っていますが、信頼性も低いです。
生のSMTPコマンドを無視します。
sendmailのようなmail transport agentを使う理由はたくさんあります。
その中にはキューイングも含まれますし、MXレコードやセキュリティと
いったものが含まれます。
...
(非 UNIX システムの為には、私たちはまだ良い DNS domain name-learnig 
mehtodを必要としています)

=head2 How do I fetch a news article or the active newsgroups?

(ニュースのアーティクルやアクティブなニュースグループをフェッチするには?)

Net::NNTP モジュールか News::NNTPClient モジュールのいずれかを使います。
これらは両方ともCPANから入手可能です。これらは以下のように
簡単にニュースグループのリストをフェッチするような仕事ができます。
...
(FTPファイルをフェッチしたりプットしたりするには?)

LWP::Simple (CPANで入手可能)はフェッチができますがプットはできません。
Net::FTP(これもCPANで入手可能です)はこれよりも複雑ですが、
フェッチとプットの両方ができます。
...
DCE::RPCが開発可能です(ただし、まだ入手可能にはなっていません)。
これはDCE-Perlパッケージ(CPANから入手可能です)の一部として
リリースされるようになるでしょう。
