...
フラグは新しくオープンされたファイル記述子に対して
$^F の値で決定されるものにセットされます。
...
LIST に含まれるファイルの所有者 (とグループ) を変更
します。　LIST の最初の 2 つの要素には、I<数値表現>
の uid と gid をこの順序で与えなければなりません。
...
LIST も省略すると、C<$_> を現在選択されている出力チャネルに出力することになります。
