...
L<http://unicode.org/Public/cldr/latest/> の UTF-8 ロケールとして
利用可能です。)
...
Perl 自身は、特に要求されない限りロケールを使いません (しかし再び、Perl は
これらを使うコードと相互作用するかもしれません)。
...
(less useful in v5.20 and later)この引数により、ロケールと Unicode をよりよく混ぜられるようになり、完全な
...
決定されます。
有効な環境がない場合、現在のロケールは
whatever the system default has been set to となります。
POSIX システムでは、これは、おそらく "C" ロケールになりますが、かならず
そうとは限りません。
On Windows, the default is set via the
computer's S<C<Control Panel-E<gt>Regional and Language Options>> (or its
current equivalent).
(TBT)
...
C<setlocale()> の第一引数には B<カテゴリー> を、第二引数には
B<ロケール> を与えます。
...
第二引数が省略されていて、さらにカテゴリーが C<LC_ALL> 以外の場合、この関数は
カテゴリーに対するカレントロケールの名前からなる文字列を返します。
この値を、後で行う setlocale() の呼び出しでの第二引数として
使うこともできますが、
B<but> on some platforms the string
is opaque, not something that most people would be able to decipher as
to what locale it means.
(TBT)
...
カテゴリーが C<LC_ALL> で、第二引数が与えられなかった場合には
...
C<LC_CTYPE> ロケールと C<LC_COLLATE> ロケールを無視することに
注意してください。
...
If C<set_locale()> fails for some reason (for example, an attempt to set
to a locale unknown to the system),
カテゴリーに対するロケールは変更されず、関数は C<undef> を返します。
(TBT)
...
Perl 起動時の問題は、環境変数 C<PERL_BADLANG> にゼロの値、たとえば
"0" を設定することによって回避できます。
...
C<POSIX::localeconv()> 関数は、カレントの C<LC_NUMERIC> や C<LC_MONETARY> で
指定されるロケール依存の数値書式の情報を取り出します。
(ある特定のカテゴリーのカレントロケールを知りたいだけなのなら、
...
S<C<use locale>> のスコープの内側にある(そして
C<use locale ':not_characters'> の内側でない)とき、Perl はアプリケーションで
...
また、文字列に埋め込まれているナルキャラクターにも対処します; C<strxfrm()> を
...
C<strcoll()> や C<strxfrm()> は are POSIX functions which use the standard system-supplied C<libc> functions that
常に C<LC_COLLATE> ロケールを参照するのです。
(TBT)

=head2 Category C<LC_CTYPE>: Character Types

(カテゴリー C<LC_CTYPE>: 文字タイプ)
...
S<C<use locale>> のスコープにある(そして C<use locale ':not_characters'> の
内側でない)とき、Perl は C<LC_CTYPE> ロケールの設定に従います。
...
さらに、C<LC_CTYPE> は (廃止予定の) POSIX のキャラクタクラステスト関数
C<POSIX::isalpha()>, C<POSIX::islower()> などにも影響を及ぼします。
たとえば、"C" ロケールを 7 ビットのスカンジナビアのものにしたとすると、
多分あなたは驚くことになるでしょうが、"|"  が 
C<POSIX::ispunct()> クラスから C<POSIX::isalpha()> クラスに移動するのです。
...
例えば C<\n> のような、特定の文字のためのエスケープシーケンス全ては、常に
プラットフォームにネイティブなものを意味します。
...
適切な C<POSIX::setlocale()> 呼び出しの後、
and within the scope of one
of the C<use locale> variants,
Perl は C<LC_NUMERIC> 
ロケール情報を参照します; これはアプリケーションが数値をどのように
整形するかということを
制御するものです。
...
カレントの C<LC_TIME> ロケールを参照し、ロケール依存の結果のみを生成します。
(TBT)
...
残ったロケールカテゴリーは現在のところ Perl 自身では使われません。
しかし再び、標準の Perl 配布
...
展開されたものが含まれる文字列は C<use locale> が有効な (そして
S<C<use locale ':not_characters'>> が有効でない) 場合には汚染されます。
...
配列コンテキストの結果や、C<$1> I<など>から派生した全てのサブパターンは
C<use locale> が有効である (そして S<C<use locale ':not_characters'>> が
有効でない) かつ正規表現サブパターンにロケール依存の構文を含んでいるときには
汚染されます。
これらの構文には C<\w> (英数字にマッチする)、C<\W>
(英数字以外にマッチする)、
C<\b> and C<\B> (word-boundary and
non-boundardy, which depend on what C<\w> and C<\W> match)
C<\s> (空白キャラクター)、
C<\S> (空白でないキャラクター)、
, C<\d> and
C<\D> (digits and non-digits), and the POSIX character classes, such as
C<[:alpha:]> (see L<perlrecharclass/POSIX Character Classes>) が
含まれます。
(TBT)
...
マッチング演算子と同じ振る舞いをします。
また、C<=~> の左側のオペランドは、C<use locale> が有効の
(そして S<C<use locale ':not_characters'>> が
有効でない)とき、
前に言及したもの、または
...
C<use locale> が有効な(そして S<C<use locale ':not_characters'>> が
有効でない)ときに、結果が汚染されます。
...
B<POSIX 文字クラステスト> (C<POSIX::isalnum()>,
C<POSIX::isalpha()>, C<POSIX::isdigit()>, C<POSIX::isgraph()>,
C<POSIX::islower()>, C<POSIX::isprint()>, C<POSIX::ispunct()>,
C<POSIX::isspace()>, C<POSIX::isupper()>, C<POSIX::isxdigit()>):
...
これに値が設定されている場合、C<LC_ALL> も、各カテゴリーの C<LC_I<foo>> も
...
あまりなさそうな上京ですが単に正規表現でそうしたいなら、C</l> 正規表現修飾子
...
プログラムの環境で LC_NUMERIC ロケールが指定されていて、フォーマットが
宣言されたときに C<use locale> が有効なら、書式指定された出力にある小数点の
文字は常にそこで指定されたものが使われます。
書式指定された出力は、write() が呼び出された時点での C<use locale> によって
制御することはできません。
...
呼ばれることがあります。
If you have the source for Perl, include in
the perlbug email the output of the test described above in L</Testing
for broken locales>.
(TBT)
