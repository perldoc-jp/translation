...
すべてのSV は、必須と言うわけではありませんが C<NUL> キャラクターで
終端されているべきです。
...
(offset OK) フラグを設定して、
and it moves the PV pointer (called C<SvPVX>) forward
by the number of bytes chopped off, and adjusts C<SvCUR> and C<SvLEN>
accordingly.  (A portion of the space between the old and new PV
pointers is used to store the count of chopped bytes.)
(TBT)
...
=end original
 ...
 &  PERL_MAGIC_proto          (none)         my サブルーチンプロトタイプ CV
...
この文書の以前の盤では、SV があるスクラッチパッドを、SV のフラグを
見ることによって推測することができると書いていました: レキシカルでは
C<SVs_PADMY> されていて、I<target> では C<SVs_PADTMP> がセットされています。
But this have never been fully true.
C<SVs_PADMY> could be set on a variable that no longer resides in any pad.
While I<target>s do have C<SVs_PADTMP> set, it can also be set on variables
that have never resided in a pad, but nonetheless act like I<target>s.
(TBT)
...
文字単位では、C<is_utf8_char_buf> は文字列中の現在の文字が有効な
UTF-8 かどうかを返します。
...
現在のところ、Perl は Unicode 文字列と非 Unicode 文字列を少し違う形で扱います。
...
This flag is only meaningful if the SV is C<SvPOK>
or immediately after stringification via C<SvPV> or a similar
macro.
...
SV が UTF-8 であるなら、C<SvPV> または同様のマクロで文字列化した後に
C<SvUTF8> フラグを見ることで分かります。
...
文字 C<uv> を UTF-8 文字列を書き込むときは B<常に> C<uvchr_to_utf8> を
