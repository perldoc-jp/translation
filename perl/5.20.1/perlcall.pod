...
存在しません。
この振る舞いについての例はセクション 
I<Returning a list in a scalar context> にあります。
...
セクション I<Returning a list of values> には G_ARRAY フラグを使った例と、
...
I<Returning a Scalar> というセクションでは、これら一時変数をどのようにして
陽に破棄するかを説明しており、
I<Using Perl to dispose of temporaries> というセクションではこの
