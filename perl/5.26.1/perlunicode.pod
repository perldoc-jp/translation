...
知れません;
L</Security Implications of Unicode> を参照してください。
...
用字を示します。
The C<Script_Extensions> property is an
improved version of C<Script>, as demonstrated below.  
...
複合形式で書かれることを要求します。
Prior to Perl v5.26, the single form
returned the plain old C<Script> version, but was changed because
C<Script_Extensions> gives better results.)
(TBT)
...
大きな符号位置は廃止予定です。
32 ビット ASCII システムでは、
これは、将来 C<0x7FFF_FFFF> が正当な最大になることを意味します
