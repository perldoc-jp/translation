...
または、現在のスコープで C<use v5.16> またはそれ以上を含めてください。
...
Perl v5.16 から、Perl 非 ASCII 文字をクォートするのに Unicode で定義された
戦略を採用しました; ASCII 文字のクォートは変わりません。
...
Anything in EXPR that matches PATTERN is taken to be a separator
that separates the EXPR into substrings (called "I<fields>") that
do B<not> include the separator.
