...
C<< EXPR1 // EXPR2 >> returns the value of C<< EXPR1 >> if it's defined,
otherwise, the value of C<< EXPR2 >> is returned. (C<< EXPR1 >> is evaluated
in scalar context, C<< EXPR2 >> in the context of C<< // >> itself). 
...
"c" に引き続いて上述した以外の文字を使うことは非推奨であり、将来の
...
(例えば、
in a regular expression it may be confused with a backreference;
...
望んでいるのが C<"\n8"> なら、8 進数を C<0> でパッディングすることで
この警告を回避してください: C<"\0128">。
...
C<\L>, C<\U>, C<\F>, C<\Q> はスタックできます; この場合それぞれに対して C<\E> が
必要です。
例えば:
...
制御しますが
but C</a> is the only one you are likely
to want to specify explicitly; the other three are selected
automatically by various pragmas.
(TBT)
...
使われた場合、何も読み飛ばしません。
末尾の検索中、デリミタやバックスラッシュをエスケープするバックスラッシュは
除去されます(述べたとおりに正確に、安全な場所にコピーはされません)。
...
C<\Q>, C<\U>, C<\u>, C<\L>, C<\l>, C<\F>, C<\E> の処理と展開が C<qq//> 構造と
(ほとんど)同じように起こります。
...
エミュレートするために使われます
, and any other Unix filter program
that takes a list of filenames, doing the same to each line
of input from all of them
。
