...
は C<I<X>> を引数としてオーバーロードメソッドを起動 I<しない> という
ことです。
...
C<ord("?") ^ 64> は 127 なので C<\c?> は DELETE で、"@" は 64 のために
...
あるということです。
C<use locale> の基では、C<\F> は C<\L> と同じ結果を生成します。
