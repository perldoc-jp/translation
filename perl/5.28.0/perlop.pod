...
定義します: それらが左側と結びつくか右側と結びつくかです。
For example, in C<9 - 3 - 2>, subtraction is left associative,
so C<9 - 3> is grouped together as the left-hand operand of the second
subtraction, rather than C<3 - 2> being grouped together as the right-hand
operand of the first subtraction. It is as if the expression were written
C<(9 - 3) - 2>, not C<9 - (3 - 2)>. So the expression yields C<6 - 2 == 4>,
...
If its operand is a single sigilled
thing, it creates a reference to that object.  If its operand is a
parenthesised list, then it creates references to the things mentioned
in the list.  Otherwise it puts its operand in list context, and creates
a list of references to the scalars in the list provided by the operand.
...
In that case it supplies scalar
context to the left operand, and returns a string consisting of the
left operand string repeated the number of times specified by the right
operand.  If the C<x> is in list context, and the left operand is either
enclosed in parentheses or a C<qw//> list, it performs a list repetition.
...
置換した数を返します。
見つからなければ、偽 (a value that is both an empty string (C<"">)
and numeric zero (C<0>) を返します。
(TBT)
...
1 つの文字列が戻ってきます; コマンドが失敗したときは C<undef> を返します。
...
I<SEARCHLIST> を括弧類で括った場合には、I<REPLACEMENTLIST> 用に、もう一組の
区切り文字を用意しなければなりません;
...
あります。
この例では、結局これらは ASCII の表示文字であることが分かります。
...
しかし、移植性のある範囲であっても、何が調べる必要のないものに含まれるのかは
一般的に明らかではありません。
...
C</c> 修飾子が指定されると、I<SEARCHLIST> には補集合が指定されたものと
解釈されます。
So for example these two are equivalent (the exact
maximum number will depend on your platform):
(TBT)
...
解釈されます。
C</d> が指定されない場合で、I<REPLACEMENTLIST> が I<SEARCHLIST> よりも短いと、
同じ長さになるまで、最後の文字が繰り返されているものとして扱われます。
I<REPLACEMENTLIST> が空文字列でのときには、I<SEARCHLIST> と同じになります。
...
終端文字列に C<~> を前置することで、L</Indented Here-docs> (後述)を
使いたいことを指定します。
...
ヒヤドキュメント修飾子 C<~> は、コードをより読みやすくするために
ヒヤドキュメントをインデントできるようにします:
...
全ての行は(改行のみの行を除いて)少なくとも同じだけの先頭の空白が
B<なければならず>、さもなければ perl は croak します。
タブとスペースは混ぜることが出来ますが、正確に
マッチングしなければなりません。
一つのタブは 8 スペースと同じではありません!
...
そして空白は C<~> とクォートされた区切り文字の間にも使えます:
...
L</Indented Here-docs> に記述されている C<<< <<~FOO >>> 構文を使ってください:
...
Furthermore, if the input symbol or an explicit assignment
of the input symbol to a scalar is used as a C<while>/C<for> condition,
then the condition actually tests for definedness of the expression's
value, not for its regular truth value.
(TBT)
...
ヌルファイルハンドル C<< <> >> は特別で、B<sed> や B<awk> および、ファイル名の
...
CPAN にある C<ARGV::readonly> モジュールか、二重山かっこが使えます。
