...
=end original
   ...
   (?=...)           ゼロ幅の肯定先読み表明
   (?*pla:...)       同じ; 5.28 から実験的に利用可能
   (?!...)           ゼロ幅の否定先読み表明
   (?*nla:...)       同じ; 5.28 から実験的に利用可能
   (?<=...)          ゼロ幅の肯定後読み表明
   (?*plb:...)       同じ; 5.28 から実験的に利用可能
   (?<!...)          ゼロ幅の否定後読み表明
   (?*nlb:...)       同じ; 5.28 から実験的に利用可能
   (?>...)           出来うる限りマッチし、バックトラックしない
   (?*atomic:...)    同じ; 5.28 から実験的に利用可能
   (?|...)           分岐のリセット
   (?<name>...)      名前付き捕捉
   (?'name'...)      名前付き捕捉
...
(L<http://oreilly.com/catalog/9780596528126/>)
