...
C<!> や C<not> ｎよって真の値を否定すると、特別な偽の値を返します。
...
C<Inf> を C<Inf> で割った場合は C<NaN> です (一方 C<Inf> に C<Inf> を足すと
...
配列の値に対して L<C<delete>|perlfunc/delete EXPR>
を呼び出すことは強く非推奨であることに注意してください。
