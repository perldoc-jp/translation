...
MANIFEST ファイルを適宜編集し、C<make manisort> を使用して
...
使用します(Perl を動作させることができない場合は、アドレス
...
L<http://search.cpan.org/dist/CPAN-YACSmoke/> または
L<http://search.cpan.org/dist/POE-Component-CPAN-YACSmoke/> または
...
Gisle Aas の図解 perlguts (別名:illguts) は素晴らしいですが、
細部に関しては少し古くなっています;
さまざまな SV 構造体はメモリフットプリントを小さくするために
再設計されています。
しかし、基本的なことは正しく、絵は非常に役立ちます。
...
Perl APIの文書では、内部関数の機能や、ソースで使用される多くのマクロについて説明しています。
(TBR)
...
文書のメンテナンスには、F<pod/>ディレクトリのすべての管理(新しい文書の提供も含む)と、coreのモジュールへの文書が含まれます。
(TBR)
...
できました。
ではパッチを作りましょう。
F<Porting/patching.pod> によれば、大きな変更を行う場合には、
いじり始める前にディレクトリ全体を安全な場所に
コピーしてから行う必要があり、それから次のようにします:
...
そして最後に、私たちの理論的根拠とともに、それを perl5-porter に
提出します。
仕事完了!
...
文書を変更する場合は、新しい文書に記載されている内容をテストするとよいでしょう。
(TBR)
...
テストに影響を与える環境変数については、TestおよびTest::Harnessモジュールの文書も参照してください。
(TBR)
...
ASCIIプラットフォームでは、上位ビットセットを持たない文字(つまり、その序数が厳密なASCII、0-127)のみが不変であり、コード内の文書とコメントは、しばしばC<hibit>のようなものを参照していると想定することができます。
...
Compaqの"Tru64 UNIX Programmer's Guide"の"Debugging Programs with Third Degree"の章に、Third Degreeの最も広範な文書があります。
(TBR)
...
使用可能なコマンドと出力フォーマットの詳細については、gprofのローカル文書を参照してください。
(TBR)
...
非常に単純な問題を取り上げ、文書、回帰テスト、そして最後にp5pに提出するためのパッチを使って、問題を完全に解決する方法を説明しました。
