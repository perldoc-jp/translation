...
Robert Spier が管理する B<RT> バグトラッカーシステムを使用して、
...
Perl API の文書では、内部関数の機能や、ソースで使用される多くのマクロについて
説明しています。
...
Perl コアの一部として出荷されるモジュールは、F<lib/> サブディレクトリと
F<ext/>サブディレクトリにあります:
F<lib/> はピュア Perl モジュール用で、F<ext/> はコア XS モジュールを
含んでいます。
...
設定プロセスは、Perl がサポートする無数のオペレーティングシステムに対して
Perl が移植性があるようにするための方法です。
設定、構築、インストールプロセス、およびコアコードの全体的な移植性に対する
責任は、設定パンプキンにあります -
...
Perlの C<-D> オプションを使用して、Perl が何をしているのかを詳しく
...
ここでは例として C<gdb> を使用します;
この原則はどのデバッガにも適用されます(多くのベンダーではデバッガを
C<dbx> と呼んでいます)が、使用しているデバッガのマニュアルを
確認してください。
...
保護されている場合に限り、I64 と U64 を使用してください。
...
これは C99 または C++ です。
Perl は C89 です。
//-コメントを使用することは多くの C コンパイラで暗黙的に許可されていますが、
