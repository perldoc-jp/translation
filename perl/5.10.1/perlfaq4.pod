...
With earlier versions of Perl, you have to do a bit more work.
この問い合わせを多くのアイテムに対して
...
to use a scalar that always has the
next element of the circular array
...
(which would mean that you're wasting a lot of space). To convert an
array of bits to a string, use C<vec()> to set the right bits.
