...
これはこのプログラムの全てのスレッドに同時に影響するので、
全てのスレッドで単一のロケールを受け付けるのでない限り、
スレッド化アプリケーションでロケールを使うのは問題があるかもしれません。
