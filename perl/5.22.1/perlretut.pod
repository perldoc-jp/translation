...
あります。
まず始めに、すべての文字がマッチングにおいて'あるがまま'(as is) に
使われるのではないということです。
...
前置することによってマッチングさせることができることを知っておくことが
重要です:
...
\w は単語を構成する文字(英数字 と _)にマッチングします; 単に
...
修飾子なし (//): デフォルトの動作です。
C<'.'> は C<"\n"> 以外の任意の文字にマッチングします。
C<^> は文字列の先頭にのみマッチングし、C<$> は文字列の末尾もしくは終端にある
改行の直前にのみマッチングします。
...
マッチングさせることができ、文字列の末尾はアンカー C<\Z> (C<$> と同じように、
末尾と末尾にある改行の直前にマッチングします) や C<\z> (末尾にのみマッチング)で
マッチングさせることができます:
...
正規表現中の 'd' は文字列中の 'c' にマッチングしません - 行き止まりです。
...
正規表現中の 'f' は文字列中の 'e' にマッチングしません; 行き止まりです。
...
文字列 "abcde" に対して 'abcd' がマッチングしました。
...
第一に、二番目のグループの三番目の選択肢 'de' もまたマッチングしますが、
そこに行く前に停止しました - 与えられた文字の位置で、最も左のものが
優先されるからです。
第二に、文字列の最初の文字が 'a' であったのでマッチングすることができました。
もし最初の位置でマッチングに成功しなければ、Perl は二番目にある文字 'b' へと
移動して同じことを繰り返します。
...
グループ化は値を \g1 にセットするので、同じ三文字の並びが両方のパーツで
使われます。
...
'd', 'm', 'y' をそれぞれ日付の要素を捕捉するグループの名前として使って、
3 つの適合するパターンを書けます。
...
選択の中で最も左にあるeがマッチングすると考えた人がいるかもしれませんが、
...
考えるかもしれませんが、^アンカーが単語の先頭にマッチングすることを
強制していません。
法則 0 がここで適用されます。
...
正規表現要素 'at' の 'a' は文字列の末尾にマッチングしない。
一文字後戻りする。
...
正規表現要素 'at' の 'a' は文字列の最後の文字 't' にマッチングしないので、
更に一文字後戻りする。
...
C<\G> アンカーは直前のマッチングで残った部分にマッチングします。
...
これらは C<\p{name}> エスケープシーケンスで表されます。
強い関係があるのは C<\P{name}> 特性で、C<\p{name}> の反対の意味を持ちます。
...
Unicode の C<\p{用字名}> 特性は、全ての Unicode 文字を、それが書かれる
言語用字に分類します。
...
これらは C<\p{name=value}> や C<\p{name:value}> のような形です (等号と
コロンは交換可能です)。
これらは単一形式よりより一般的で、実際ほとんどの単一形式は単に
共通複合形式の Perl 定義のショートカットです。
例えば、以前の段落でのスクリプトの例は C<\p{Script=Latin}>,
C<\p{Script:Greek}>, C<\P{script=katakana}>, C<\P{script=balinese}> と等価に
書けます (大文字小文字は C<{}> の中では無意味です)。
...
複数で表現されるものです。
例えば、S<C<A + COMBINING RING>> のような Unicode の完全な名前を使うと、
基底文字 C<A> と結合文字 S<C<COMBINING RING>> による書記素クラスタで、
...
定義します。
これらは C<[:name:]> の形式で、C<name> は POSIX クラス名です。
...
C<\C> は戻り読みでサポートされていません; なぜなら、C<\C> の定義がすでに
当てにならないもので後ろに戻るときにはより一層当てにならないからです。
...
もし C<condition> が真であれば、C<yes-regexp> がマッチングの対象となります。
C<condition> が偽であった場合、C<yes-regexp> はスキップされて、
Perl は次の正規表現要素へと進みます。
二番目の形式は Perl の S<C<'if () {} else {}'>> 文のようなものです。
C<conditon> が真であれば C<yes-regexp> がマッチングの対象となり、偽であれば
C<no-regexp> がマッチングの対象となります。
...
グループに名前を付けたいなら、そのグループを再帰させるために C<(?&name)> が
使えます。
...
ふーむ。
何が起きたのでしょう?
先のパターンが効果としては最後のものと (おとんど) 同じであることをあなたは
...
ドイツ語の 'der|die|das' にマッチングさせるものです:
