...
速度の低下のほとんどを取り除けているはずです。
Signals should still be dispatched within the same statement as
they were previously - if this is not the case, or it is possible to
create uninterruptible loops, this is a bug, and reports are encouraged
of how to recreate such issues.
(TBT)
...
op が二つ少なくなりました。
この変更により、C<shift @_;> と比べて C<shift;> はスレッドなし perl で
...
比較を行う foldEQ_utf8 API 関数は大幅にリファクタリングされ
...
状況によっては O(N^2) (N は参照の数) の時間がかかることがありました。
...
xhv_fill は struct xpvhv から削除され、ハッシュ毎に 1 IV 節約される
ことにより、システムによっては struct xpvhv がキャッシュ境界に
一致するようになりました。
このメモリ節約が他の場所での低速化を引き起こすことを避けるために、
HvFILL の真偽値での使用は代わりに (等価である) HvTOTALKEYS を
...
@EXPORT_FAIL AV は要求されるまで作成されなくなり、従ってどちらもそれに
もどる型グロブではありません。
...
要求されたときにだけ行うようになりました(ただしこれはデバッグビルドでない
場合にのみ可能です)。
...
削除するようになりました; except where the first carries a label, since labels
must not be eliminated by the optimizer and label usage isn't conclusively known
at compile time.
(TBT)
...
C<attributes> はバージョン 0.12 から 0.14 に更新されました。
...
C<Archive::Extract> はバージョン 0.38 から 0.48 に更新されました。
...
C<Archive::Tar> はバージョン 1.54 から 1.76 に更新されました。
...
C<Attribute::Handlers> はバージョン 0.87 から 0.89 に更新されました。
...
C<autodie> はバージョン 2.06_01 から 2.1001 に更新されました。
...
C<AutoLoader> はバージョン 5.70 から 5.71 に更新されました。
...
C<B> はバージョン 1.23 から 1.29 に更新されました。
...
C<B::Concise> はバージョン 0.78 から 0.83 に更新されました。
...
C<B::Debug> はバージョン 1.12 から 1.16 に更新されました。
...
C<B::Deparse> はバージョン 0.96 から 1.03 に更新されました。
...
The C<no 5.13.2> や似たような形式を B::Deparse で正しく扱えるように
なりました (5.12.3)。
...
C<B::Lint> はバージョン 1.11_01 から 1.13 に更新されました。
...
C<base> はバージョン 2.15 から 2.16 に更新されました。
...
C<Benchmark> はバージョン 1.11 から 1.12 に更新されました。
...
C<bignum> はバージョン 0.23 から 0.27 に更新されました。
...
C<Carp> はバージョン 1.15 から 1.20 に更新されました。
...
引き起こされる C<おかしな ARRAY のコピー> が起きる場合を
修正します (5.12.2)。
...
C<CGI> はバージョン 3.48 から 3.52 に更新されました。
...
C<Compress::Raw::Bzip2> はバージョン 2.024 から 2.033 に更新されました。
...
C<Compress::Raw::Zlib> はバージョン 2.024 から 2.033 に更新されました。
...
C<CPAN> はバージョン 1.94_56 から 1.9600 に更新されました。
...
C<CPANPLUS> はバージョン 0.90 から 0.9103 に更新されました。
...
C<CPANPLUS::Dist::Build> はバージョン 0.46 から 0.54 に更新されました。
...
C<Data::Dumper> はバージョン 2.125 から 2.130_02 に更新されました。
...
C<DB_File> はバージョン 1.820 から 1.821 に更新されました。
...
C<DBM_Filter> はバージョン 0.03 から 0.04 に更新されました。
...
C<Devel::DProf> はバージョン 20080331.00 から 20110228.00 に更新されました。
...
C<Devel::Peek> はバージョン 1.04 から 1.07 に更新されました。
...
C<Devel::SelfStubber> はバージョン 1.03 から 1.05 に更新されました。
...
C<diagnostics> はバージョン 1.19 から 1.22 に更新されました。
...
C<Digest::MD5> はバージョン 2.39 から 2.51 に更新されました。
...
C<Digest::SHA> はバージョン 5.47 から 5.61 に更新されました。
...
C<DirHandle> はバージョン 1.03 から 1.04 に更新されました。
...
C<Dumpvalue> はバージョン 1.13 から 1.16 に更新されました。
...
C<DynaLoader> はバージョン 1.10 から 1.13 に更新されました。
...
C<Encode> はバージョン 2.39 から 2.42 に更新されました。
...
C<Env> はバージョン 1.01 から 1.02 に更新されました。
...
C<Errno> はバージョン 1.11 から 1.13 に更新されました。
...
C<Exporter> はバージョン 5.64_01 から 5.64_03 に更新されました。
...
C<ExtUtils::CBuilder> はバージョン 0.27 から 0.280203 に更新されました。
...
C<ExtUtils::Command> はバージョン 1.16 から 1.17 に更新されました。
...
C<ExtUtils::Constant> はバージョン 0.22 から 0.23 に更新されました。
...
C<ExtUtils::Install> はバージョン 1.55 から 1.56 に更新されました。
...
C<ExtUtils::MakeMaker> はバージョン 6.56 から 6.57_05 に更新されました。
...
C<ExtUtils::Manifest> はバージョン 1.57 から 1.58 に更新されました。
...
C<ExtUtils::ParseXS> はバージョン 2.21 から 2.2210 に更新されました。
...
C<Fcntl> はバージョン 1.06 から 1.11 に更新されました。
...
C<File::Basename> はバージョン 2.78 から 2.82 に更新されました。
...
C<File::CheckTree> はバージョン 4.4 から 4.41 に更新されました。
...
C<File::Copy> はバージョン 2.17 から 2.21 に更新されました。
...
C<File::DosGlob> はバージョン 1.01 から 1.04 に更新されました。
...
C<File::Fetch> はバージョン 0.24 から 0.32 に更新されました。
...
C<File::Find> はバージョン 1.15 から 1.19 に更新されました。
...
C<File::Glob> はバージョン 1.07 から 1.12 に更新されました。
...
C<File::Spec> はバージョン 3.31 から 3.33 に更新されました。
...
空ディレクトリを返します;
C<abs2rel()> は Unix 型の入力を正しく扱います (5.12.2)。
...
C<File::stat> はバージョン 1.02 から 1.05 に更新されました。
...
C<Filter::Simple> はバージョン 0.84 から 0.86 に更新されました。
...
C<GDBM_File> はバージョン 1.10 から 1.14 に更新されました。
...
C<Hash::Util> はバージョン 0.07 から 0.11 に更新されました。
...
C<Hash::Util::FieldHash> はバージョン 1.04 から 1.09 に更新されました。
...
C<I18N::Collate> はバージョン 1.01 から 1.02 に更新されました。
...
C<I18N::Langinfo> はバージョン 0.03 から 0.08 に更新されました。
...
C<I18N::LangTags> はバージョン 0.35 から 0.35_01 に更新されました。
...
C<if> はバージョン 0.05 から 0.0601 に更新されました。
...
C<IO> はバージョン 1.25_02 から 1.25_04 に更新されました。
...
C<IPC::Cmd> はバージョン 0.54 から 0.70 に更新されました。
...
C<IPC::Open3> はバージョン 1.05 から 1.09 に更新されました。
...
C<IPC::SysV> はバージョン 2.01 から 2.03 に更新されました。
...
C<lib> はバージョン 0.62 から 0.63 に更新されました。
...
C<Locale::Maketext> はバージョン 1.14 から 1.19 に更新されました。
...
C<Log::Message> はバージョン 0.02 から 0.04 に更新されました。
...
C<Log::Message::Simple> はバージョン 0.06 から 0.08 に更新されました。
...
C<Math::BigInt> はバージョン 1.89_01 から 1.994 に更新されました。
...
C<Math::BigInt::FastCalc> はバージョン 0.19 から 0.28 に更新されました。
...
C<Math::BigRat> はバージョン 0.24 から 0.26_02 に更新されました。
...
C<Memoize> はバージョン 1.01_03 から 1.02 に更新されました。
...
C<MIME::Base64> はバージョン 3.08 から 3.13 に更新されました。
...
C<Module::Build> はバージョン 0.3603 から 0.3800 に更新されました。
...
C<Module::CoreList> はバージョン 2.29 から 2.47 に更新されました。
...
C<Module::Load> はバージョン 0.16 から 0.18 に更新されました。
...
C<Module::Load::Conditional> はバージョン 0.34 から 0.44 に更新されました。
...
C<mro> はバージョン 1.02 から 1.07 に更新されました。
...
C<NDBM_File> はバージョン 1.08 から 1.12 に更新されました。
...
C<Net::Ping> はバージョン 2.36 から 2.38 に更新されました。
...
C<NEXT> はバージョン 0.64 から 0.65 に更新されました。
...
C<Object::Accessor> はバージョン 0.36 から 0.38 に更新されました。
...
C<ODBM_File> はバージョン 1.07 から 1.10 に更新されました。
...
C<Opcode> はバージョン 1.15 から 1.18 に更新されました。
...
C<overload> はバージョン 1.10 から 1.13 に更新されました。
...
C<Params::Check> はバージョン 0.26 から 0.28 に更新されました。
...
C<parent> はバージョン 0.223 から 0.225 に更新されました。
...
C<Parse::CPAN::Meta> はバージョン 1.40 から 1.4401 に更新されました。
...
C<PerlIO::encoding> はバージョン 0.12 から 0.14 に更新されました。
...
C<PerlIO::scalar> はバージョン 0.07 から 0.11 に更新されました。
...
C<PerlIO::via> はバージョン 0.09 から 0.11 に更新されました。
...
C<Pod::Html> はバージョン 1.09 から 1.11 に更新されました。
...
C<Pod::LaTeX> はバージョン 0.58 から 0.59 に更新されました。
...
C<Pod::Perldoc> はバージョン 3.15_02 から 3.15_03 に更新されました。
...
C<Pod::Simple> はバージョン 3.13 から 3.16 に更新されました。
...
C<POSIX> はバージョン 1.19 から 1.24 に更新されました。
...
C<re> はバージョン 0.11 から 0.18 に更新されました。
...
C<Safe> はバージョン 2.25 から 2.29 に更新されました。
...
C<SDBM_File> はバージョン 1.06 から 1.09 に更新されました。
...
C<SelfLoader> はバージョン 1.17 から 1.18 に更新されました。
...
C<sigtrap> はバージョン 1.04 から 1.05 に更新されました。
...
C<Socket> はバージョン 1.87 から 1.94 に更新されました。
...
C<Storable> はバージョン 2.22 から 2.27 に更新されました。
...
C<Sys::Hostname> はバージョン 1.11 から 1.16 に更新されました。
...
C<Term::ANSIColor> はバージョン 2.02 から 3.00 に更新されました。
...
C<Term::UI> はバージョン 0.20 から 0.26 に更新されました。
...
C<Test::Harness> はバージョン 3.17 から 3.23 に更新されました。
...
C<Test::Simple> はバージョン 0.94 から 0.98 に更新されました。
...
C<Thread::Semaphore> はバージョン 2.09 から 2.12 に更新されました。
...
C<Thread::Queue> はバージョン 2.11 から 2.12 に更新されました。
...
C<threads> はバージョン 1.75 から 1.83 に更新されました。
...
C<threads::shared> はバージョン 1.32 から 1.36 に更新されました。
...
C<Tie::Hash> はバージョン 1.03 から 1.04 に更新されました。
...
C<Tie::Hash::NamedCapture> はバージョン 0.06 から 0.08 に更新されました。
...
C<Tie::RefHash> はバージョン 1.38 から 1.39 に更新されました。
...
C<Time::HiRes> はバージョン 1.9719 から 1.9721_01 に更新されました。
...
C<Time::Local> はバージョン 1.1901_01 から 1.2000 に更新されました。
...
C<Time::Piece> はバージョン 1.15_01 から 1.20_01 に更新されました。
...
C<Unicode::Collate> はバージョン 0.52_01 から 0.73 に更新されました。
...
C<Unicode::Normalize> はバージョン 1.03 から 1.10 に更新されました。
...
C<Unicode::UCD> はバージョン 0.27 から 0.32 に更新されました。
...
C<version> はバージョン 0.82 から 0.88 に更新されました。
...
C<warnings> はバージョン 1.09 から 1.12 に更新されました。
...
C<warnings::register> はバージョン 1.01 から 1.02 に更新されました。
...
C<XSLoader> はバージョン 0.10 から 0.13 に更新されました。
...
C<VMS::DCLsym> はバージョン 1.03 から 1.05 に更新されました。
...
C<Win32> はバージョン 0.39 から 0.44 に更新されました。
...
C<XS::Typemap> はバージョン 0.03 から 0.05 に更新されました。
...
C<Class::ISA> は Perl コアから削除されました。
以前のバージョンは 0.36 でした。
...
C<Pod::Plainer> は Perl コアから削除されました。
以前のバージョンは 1.02 でした。
...
POD 文書のための新しいスタイルガイドで、ほとんどは pod2man man ページの
NOTES の節から分離されました。
...
Perl 5.12.0 の perlmodlib ページは、リストを生成するスクリプトのバグにより、
多くのモジュールが漏れていました。
...
L<perlrun> において、C<-0NNN> オプションのうち C<-0400> 以上のものの
...
C<File::Copy> の setuid でのテストを省略するようになりました (5.12.2)。
...
出力中に PerlIO 層によって誤ったレコード境界が発生することは
なくなりました (5.12.3)。
...
二重割り当てによる C<Perl_rename()> でのメモリリークが
修正されました (5.12.2)。
...
C<keys>, C<values>, C<each> 組み込み関数を配列に対しても使えるように
...
C<split> はスカラコンテキストや無効コンテキストで呼び出されたときに
C<@_> を変更されなくなりました。
