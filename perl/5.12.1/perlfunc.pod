...
If EXPR is undefined, returns C<undef>.
(TBT)
...
C<@> を使うと、繰り返し数は一番内側の C<()> グループの先頭からのオフセットを
表現します。
...
そして整数 I<n> なら、オフセットは一番内側から n 番目の C<()> グループの
...
スカラコンテキストと無効コンテキストでは、配列 C<@_> に分割結果を
設定します。
しかし、スカラコンテキストや無効コンテキストでの split の使用は
推奨されません; サブルーチンの引数を上書きしてしまうからです。
