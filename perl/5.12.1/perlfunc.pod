...
C<"switch"> 機能が有効なら、C<continue> は現在の C<when> (あるいは
C<default>) のブロックを終了して、次のものに移動するための文となります。
...
(単純な真偽値テストでは、C<undef>、0、空文字、C<"0"> のいずれも偽を返すので、
区別することができません。)
...
C<eval> はスカラコンテキストでは未定義値が、リストコンテキストでは
空リストが返され、C<$@> にエラーメッセージが設定されます。
...
しかし、これは使ってはいけません)。
SUBSTR が見つからなかった場合には、C<index> はベースよりも 1 小さい値、
通常は C<-1> が返されます。
...
コマンドへの引数となります。
パイプモードではない C<open> での 3 つ以上の引数の意味はまだ未定義ですが、
...
Digital/Compaq はリトルエンディアンモードで使っています(いました)が、
...
含みます)が、C<my> (あるいは C<our> (あるいは C<state>)) で作成された
レキシカル変数には I<影響しません>。
...
誤って解釈されることがあります。)
FILEHANDLE を省略した場合には、デフォルトでは標準出力か、
最後に選択された出力チャネルに出力します; L</select>を参照してください。
LIST も省略すると、C<$_> を現在選択されている出力ハンドルに
出力することになります。
...
書き込んで 1 を返し、それからファイル終端で 0 を返すものと想定されます。
...
Unix の中には、実際に利用可能なデータがないために引き続く読み込みが
ブロックされる場合、select(2) が、ソケットファイル記述子が
...
最後の例では、Perl が 64 ビット整数を理解しない場合は例外が発生します。
...
syswrite() の LENGTH, OFFSET および返り値は (UTF-8 エンコードされた
Unicode) 文字単位になります。
C<:encoding(...)> 層は暗黙のうちに C<:utf8> 層が導入されます。
L</binmode>, L</open>, C<open> プラグマ, L<open> を参照してください。
