...
C<use feature 'unicode_strings'> プラグマは、プラットフォームに関わらず常に
特定のレキシカルスコープで Unicode セマンティクスを強制することを
意図しています。
リリース 5.12 では、これは部分的に実装されていて、大文字小文字変更にのみ
適用されます。
...
任意の文字とマッチングし、C<\P{UpperCase}>は UpperCase 特性値 が False である
任意の文字とマッチングします; そしてこれらはそれぞれ
...
より長いけれども説明的で意味が理解しやすいものです。
したがって、前述の "L"および "Letter" は同等であり、
交換可能です。
...
Unicode Script特性は、指定された文字の中にある用字を示し、
C<\p{Script=Hebrew}> (短縮: C<\p{sc=hebr}>) のような複合形式で
マッチングさせることができます。
...
ブロック特性は Unicode の基本的な部分で動作させるのに有用です。
...
(あるいは最終的にコードを読む人々が知っているか自信がない)という理由で、
...
これらの多くは Unicode のもの同義語にすぎませんが、いくつかは
...
追加しました。
しかしこの文字は、文字 "LATIN CAPITAL LETTER E" と文字
...
Perl は便利なように C<Non_Canonical> 型を追加しています;
これは任意の互換分解を意味します。
...
これは ASCII の範囲の任意の英数字にマッチングします; つまり
C<[A-Za-z0-9]> です。
...
これは ASCII の範囲の任意の英字にマッチングします; つまり
C<[A-Za-z]> です。
...
これは ASCII の範囲の任意の空白文字にマッチングします; つまり
C<S<[ \t]>> です。
...
これは ASCII の範囲の任意の制御文字にマッチングします; つまり
C<[\x00-\x1F\x7F]> です。
...
これは ASCII の範囲の任意の数字にマッチングします; つまり
C<[0-9]> です。
...
これは ASCII の範囲の任意の図形文字にマッチングします; つまり
C<[\x21-\x7E]> です。
...
これは ASCII の範囲の任意の小文字にマッチングします; つまり
C<[a-z]> です。
...
これは ASCII の範囲の任意の表示文字にマッチングします; つまり
C<[\x20-\x7E]> です。
これは図形文字に SPACE を加えたものです。
...
これは ASCII の範囲の任意の句読点文字にマッチングします; つまり
C<[\x21-\x2F\x3A-\x40\x5B-\x60\x7B-\x7E]> です。
これらは単語文字でない図形文字です。
Posix 標準は句読点の定義を含んでいて、Unicode はこれらの文字を
「シンボル」と呼んでいることに注意してください。
...
これは ASCII の範囲の任意の大文字にマッチングします; つまり C<[A-Z]> です。
...
この符号位置が割り当てられたということではなく、
符号位置の意味が決定されたということです。
...
これを修正する作業が行われていますが、5.12 リリースに間に合うように
完了したものはその一部だけです。
完了したのは、大文字小文字変換の重要な部分です。
懸念があり、いくつかの証拠がありますが、
古いコードが既存の振る舞いに依存しているかもしれないという
懸念およびいくつかの証拠により、
例え新しい構文が関与していなくても、
新しい振る舞いは L<feature> プラグマの C<unicode_strings> 機能によって
明示的に有効にされている必要があります。
...
プラグマは 5.12 リリースでは大文字小文字操作にしか影響しませんが、
後のリリースではすべての問題のある振る舞いに影響することが計画されています:
これらすべてを使わずに一つだけ使うことはできません。
...
これらのファイルは、C<\$Config{privlib}>/F<unicore> の既存のファイルを
置き換える必要があります。
(C<\%Config> は、Config モジュールから利用できます。)
一部の名前を変えるには、そのディレクトリにある F<README.perl> の指示に従って、
それから F<make> を実行してください。
...
大文字小文字を無視したマッチングには問題があります;
(大かっこで囲まれた) 文字クラスに関するもの、
畳み込まれる文字が複数の文字になるもの (単一の文字 LATIN SMALL LIGATURE
FFL が 3 文字の文字列 C<ffl> に大文字小文字を無視してマッチングするようなもの)
Latin-1 Supplement にある文字に関するものなどです。
