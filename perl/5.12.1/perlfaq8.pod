...
perl の実行ファイルがビルドされたオペレーティングシステムの名前
...
CPAN にある C<Term::ANSIColor> モジュールを使うことができます。
...
CPAN にある C<Term::ReadKey> モジュールは、B<stty> をキー毎にシェルに
...
以下の例は、標準の C<POSIX> モジュールを使った解決策で、
...
そして以下の例は、C<HotKey> モジュールを使ったものです。
...
最も簡単な方法は、CPAN にある C<Term::ReadKey> に対して
...
CPAN にある C<Term::ReadKey> モジュールをインストールしているのなら、
...
あるいはほとんどのシステムで、CPAN にある C<Term::ReadKey> を使って
行うこともできます。
...
参照)でオープンする必要があるでしょう。
C<sysopen()> と C<Fcntl> モジュールにある C<O_RDWR|O_NDELAY|O_NOCTTY> とを使って
...
C<IPC::Open2>, C<IPC::Open3>, C<IPC::Run>, C<Parallel::Jobs>,
C<Parallel::ForkManager>, C<POE>, C<Proc::Background>, C<Win32::Process>
...
これは C<atexit()> をシミュレートするのに使うことができます。
...
C<END> ブロックは、トラップされないシグナルが
プログラムを強制終了させた場合には呼び出されません。
したがって、C<END> ブロックを使う場合には同時に以下のようにするべきです:
...
例外処理そのものに興味があるのなら、C<exception.pl> ライブラリ
...
Windows では、C<Win32::API> を試してください。
Mac では、C<Mac::Carbon> を試してください。
C 関数へのインターフェースを持つモジュールがなければ、
C<Inline::C> を使って Perl のソースコードにちょっとした C のコードを
インライン化できます。
...
伝統的に、これらのファイルは標準配布に含まれる C<h2ph> というツールによって
生成されるものです。
...
移植性と健全性(sanity)のために、C<h2xs> を使うべきでしょう
(これも標準の perl 配布キットに含まれています)。
このツールは、C のヘッダーファイルを Perl のエクステンションに変換します。
C<h2xs> の使い方は L<perlxstut> を参照してください。
...
C<IPC::Open2> モジュール(標準の perl の配布に含まれています)は内部的に
...
C<IPC::Open3> モジュール(標準配布パッケージに含まれています)も使えますが、
これは C<IPC::Open2> とは引数の順序が違うということに注意してください
...
command?"> を参照するか、C<IPC::Open3> モジュールを使ってください。
...
C<Net::FTP>, C<TCP::Client>, C<Net::Telnet> といったモジュール(CPAN で入手可能です)を
試してみてください。
http://www.cpan.org/scripts/netstuff/telnet.emul.shar も telnet プロトコルを
エミュレートする助けになるでしょうが、C<Net::Telnet> は使うのがとても簡単です。
...
昔々、L<chat2.pl> と呼ばれたライブラリがありました(これは標準の perl
配布キットに含まれます)。
もしこれをどこかで見つけてもB<使ってはいけません>。
今日では、CPAN にある C<IO::Pty> や C<IO::Stty> といった
ライブラリを探すのが最善でしょう。
...
そのほかもっと良いのは、C<POSIX::setsid()> 関数を使うことです。
...
CPAN で入手できる C<Proc::Daemon> モジュールはこれらの操作を
行ってくれる関数を提供しています。
...
いかないかもしれません。
C<Expect> では、他のプログラムは人間である振りをします。
...
C<IO::Interactive> モジュールが答えとして最良のものです。
...
C<Sys::AlarmCall> モジュールを使うこともできます。
...
C<DBI> モジュールはほとんどのデータベースサーバと型に対する抽象
...
その他のモジュールはより具体的なアクセスを提供します: C<Win32::ODBC>,
C<Alzabo>, C<iodbc> などは CPAN Search (http://search.cpan.org) で
見つけられます。
...
現在のディレクトリからディストリビューションをインストールしたいなら、
C<CPAN.pm> に C<.> をインストールするように知らせることができます (完全停止):
...
なぜなら、より古いバージョンの C<ExtUtils::MakeMaker> が推奨していた
...
考慮するべきです。
さらなる詳細については C<ExtUtils::Makemaker> の文書を参照してください。
...
プラグマ的モジュール(pragmatic module) C<lib.pm> は Perl 5.002 で
最初に導入されました。
...
値を定義しているものです。
これは Perl がインストールされたときに C<h2ph> を使って作成されることも
ありますが、そうでないときもあります。
