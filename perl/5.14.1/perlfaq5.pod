...
一つの裏技は C<File::ReadBackwards> を使って、ファイルの最後から
始めることです。
...
C<File::ReadBackwards> モジュールは、入力レコードセパレータを
...
行に対して tie された配列を使ってアクセスできる C<Tie::File> モジュールも
使えます。
...
C<sysopen> を使っているものはすべて C<Fcntl> から定数を引いているものと
仮定します:
...
Perl 5.6.0 から、C<glob> 外部リソースに頼ることなく内部で実装されています。
当然として、最近の perl では C<glob> ではメモリの問題はありません。
...
標準の C<Tie::File> モジュールか C<DB_File> モジュールの C<$DB_RECNO>
...
Perl と共に配布されている C<File::Find> モジュールが、ディレクトリ構造を
辿るための大変な作業の全てを行います。
...
CPAN からダウンロードできる C<File::Find::Closures> は、
C<File::Find> と共に使える、たくさんの「すぐに使える」サブルーチンを
提供します。
...
CPAN からダウンロードできる C<File::Finder> は、
...
CPAN からダウンロードできる C<File::Find::Rule> モジュールは、
...
Perl と共に配布されている C<File::Path> モジュールには、大変な作業の全てを
...
C<File::Path> モジュールには、より古い C<rmtree> サブルーチンへの
レガシーなインターフェースもあります。
