...
　B<-x> スイッチを
指定した場合には、まず、最初に C<#!> と "perl" という文字列を
含む行を探し、そこから解析を始めます。
...
C<#!> 行に "perl" という語が含まれていなければ、C<#!> の後に指定された
...
I/O 操作) において暗黙的に C<:utf8> PerlIO 層が適用されます。
...
指示します。
I<sed -n> や I<awk> のようにファイル名引数上で繰り返しを
行なうことになります:
...
Perl に指示します。
I<sed> のようにファイル名引数上で繰り返しを行なうことになります:
...
渡さなければなりません。
これは通常コマンドラインの最初の方につけるか、C<#!> 構造に対応する
システムではここに書くかです。
...
なぜなら(C<:encoding()> のような)外部読み込みの層はロードするために
IO が必要だからです。
...
提供する場合は"stdio"実装を使うよう設定します。
そうでなければ "unix perlio" 実装を使います。
...
ハッシュからハッシュの単位で、ハッシュキー挿入中に異常なデータが検出されると、
ハッシュは代替のランダムハッシュの種に切り替えます。
...
ランダム化されます。
perl が B<-DUSE_HASH_SEED_EXPLICIT> 付きでコンパイルされた場合、
デフォルトの振る舞いは PERL_HASH_SEED がセットされない限り
ランダム化 I<されません>。
...
(Perl 5.8.1 から) これを C<"1"> に設定すると、実行開始時にハッシュの種の
値を画面(STDERR)に出力します。
