...
サブルーチンの内部では、配列 C<@_> はサブルーチンに渡されたパラメータです。
Inside a subroutine, C<@_> is the default array for
the array operators C<push>, C<pop>, C<shift>, and C<unshift>.
(TBT)
...
The value you set in  C<nasty_break()> is still there when you
return.
...
フォーマット名。
デフォルトは、ファイルハンドル名に _TOP を続けたもの。
For example, the default format top name for the C<STDOUT>
filehanlde is C<STDOUT_TOP>.
(TBT)
...
デフォルトは、ファイルハンドル名。
For example, the default format name for the C<STDOUT>
filehandle is just C<STDOUT>.
(TBT)
