...
大文字と小文字の両方に含まれるので C<Cased> であるけれｄも、
...
これらのほとんどは Unicode のもの同義語にすぎませんが、いくつかは
...
C<\p{\p{Present_In:3.1} 以上もマッチングします。
...
サブルーチンのナメが汚染されたデータによって決定された時点で
die することに注意してください。
...
対応関係を定義することができます。
The mappings are currently only valid
on strings encoded in UTF-8, but see below for a partial workaround for
this restriction.
(TBT)
