...
存在する Date::ICal のテストを探し回って、F<t/01sanity.t>に、次のものを見つけました[7]。
...
Date::ICalのmanページに急いで目を通していて、次のものに遭遇しました:
...
おぉー! 実行されないようだ。さて、これを修正しないと想定ましょう。[11]
ふつう、テストをコメントにして、このテストを、ToDoリストのどこかにおくでしょう。
...
以下のようなコードに直す前にテストを書くことができます。

TODOテストがパスすると、Test::Harness は、テストが、「思いがけず成功した」と報告します。
これが起きれば、C<local $TODO>でTODOブロックを外して、本当のテストに変えればいい。

=head2 汚染モードでのテスト

汚染モードは楽しいものです。全ての全体的な特徴のなかでも、もっとも全体的なものです。
汚染モードを付けると、汚染モードは、プログラムの全てのコードとI<全ての>モジュール(と、それらが使っているI<全ての>モジュール)に影響します。
コードの一つでも、清潔でなければ、全てが爆発します。
...
MacOSのエポックは、1904年の1月1日の真夜中です。VMSのエポックは、1858年の11月17日の深夜です。
