...
B<待った!>もし、今初めてテストを書こうとしているのなら、Test::Simpleをまず見なさい。
...
B<待った!>もし、今初めて、テストを書き始めようと言うのなら、Test::Simpleをまず見なさい。
Test::Moreは、基本的なテストのコツを得て、置き換え可能なTest::Simpleの差込式の置換です。
...
いくつかのケースで、あるテストscript全てを完全にスキップしたいでしょう。
...
モジュールが利用できなかったりとか。
こういったケースでは、テストをスキップしたり、
失敗することが予想されるけれど、
将来的に動く(a todo test)であろうということを宣言したりしなければいけません。
