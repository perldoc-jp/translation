...
B<待った!>もし、今、テストを書き始めようと言うのなら、Test::Simpleをまず見なさい。
...
以下のような様々な、「ok」と示す方法があります。よりよい診断や、テストを簡単にスキップしたり、
将来の主眼点をテストし、複雑なデータ構造を比較する機能などです。
...
C<use Test::More> の時に、この保護を行う好ましい方法は、計画を宣言することです。
...
(テストを弱めるので、これを使うのは避けるようにしてください)

  use Test::More qw(no_plan);

In some cases, you'll want to completely skip an entire testing script.

いくつかのケースで、全部のテストscriptを完全にスキップしたいでしょう。
...
Test::Moreがエクスポートする機能をコントロールしたければ、
...
script中のテストを見つけやすくなり、「簡単な指数関数」を楽に探せます。
...
「ok 番号」か、「not ok 番号」のどちらかを出力することです。他の全ては、ただのグレービーです。
...
それに、他の人に、あなたの意図する考えを与えます。$test_nameは、は、オプショナルですが、
使うことが強く勧められています。
...
is()とisnt()は、テストが何で、テストがなぜ失敗したか、を知っています。
たとえばこのテスト：
...
このコードは、1を返すかどうかをチェックします。まったく違います。
...
ok()を超える、この利点は、is() と isnt()の利点に似ています。
失敗に関して、よく診断します。
...
ok()を超える cmp_ok の 利点は、テストが失敗したときに、
...
$module や $object を確かめるチェックは、
複数の @methods を実行できます。 (関数でも動きます).
...
かを使える。これらは、ok(1)と、ok(0)の同意語です。

Use these very, very, very sparingly.

pass() と fail() を、使うのをひじょーに慎重にしてください。
...
何が間違っているかについてよい考えを得るでしょう。ですが、時に、
...
B<注意> 正確に診断の出力を整形することは、まだ、変化します。
しかし、それに何を投げても、テストに干渉しないことが保証されています。
...
普通、テストしているモジュールのロードが失敗したかどうかをただ、吐くよりも、
むしろ、 ok をロードしたかどうかをテストしたいとでしょう。
そのような目的のために、C<use_ok>と、C<require_ok>がある。
...
これらは、単純に、与えられた $module とテストを使って、
ロードが ok したかを確かめる。BEGIN ブロック内で、use_ok() を走らせることを推奨します。
これにより、この関数は、コンパイル時にexportされ、プロトタイプに、適切な名誉が与えられる。

If @imports are given, they are passed through to the use.  So this:

@import が与えれた場合、use を通って、通過する。次のように :
...
ある条件のもとでテストを動かすことで、テストスクリプトが死ぬ時があります。
特定の関数やメソッドは(MacOSでのfork()のように)実装されていない。
いくつかのリソースが利用できない(ネット接続)とか、モジュールが利用できないとか。
こういったケースでは、テストをスキップするしなければならないか、
失敗することが予想されるけれど、
将来的に動く(a todo test)であろうということを宣言しなければいけません。
...
これは、次のSKIPブロックのために完全に安全です。それぞれのSKIPブロックは、
...
Test::Harness は、失敗を ok であると解釈します。
なんでも成功にし、予期しない成功と、報告します。
todoがなされた分かったら、TODOフラグを外すことが出来ます。
...
また、それらのテストが修正されたばあい、即座に知ることが出来るでしょう。

Once a todo test starts succeeding, simply move it outside the block.
When the block is empty, delete it.

一度、todoテストが成功し始めると、単純に、ブロックの外側にtosoテストをを移します。
ブロックが空なら、削除します。
...
Test::Harness は、合格しているとして、テストを解釈します。

=item When do I use SKIP vs. TODO?

=item SKIP 対 TODO を いつ使うのでしょう？
...
機能(fork()やsymlinksなどの)を持っていないOSのもとで走ることや、
インターネット接続を必要としているのに、ユーザが利用できないことも含みます。
...
is()と似ているが、$this と $thatが、ハッシュか、配列のリファレンスです。
...
eq_array() とにていますが、要素の順番が重要ではありません。
...
このことは、両方とも、Test::Builderを使っている、二つのテストライブラリーが、
...
基礎にあるTest::Builderオブジェクトにアクセスできます:
...
Test::Moreを拡張しようとするなら、やめておきなさい。Test::Builder を代わりに使いなさい。
...
no_plan と todo は、新しい Test::Harness の主眼点に依存し、修正しています。
...
その依存はTest::Harnesをアップグレードさせる原因になるでしょう。
...
自分のok()ルーチンを最初に書いたとき、Pritikinのテストモジュールの存在にまったく気づきませんでした。
...
L<Test::Simple> もし、Test::Moreがまったく混乱させるものであり、
