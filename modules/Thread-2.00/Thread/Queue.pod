...
C<Thread::Queue>によって実装されるキュー（待ち行列）はリストによくにたスレッドセーフなデータ構造である。かなりのスレッドが安全に、リストの最後に要素を付け加えたり、先頭から要素を取り除いたりすることができる（リストの中に対して加えたり取り除いたりするとこは出来ない）。
...
C<enqueue>メソッドはキューの最後にスカラーのリストを付け加える。リストに応じてキューは成長する。
...
C<dequeue>メソッドはキューの先頭からスカラーを取り除き、それを返す。キューが空のとき、C<dequeue>は他のスレッドがスカラーをC<enqueue>するまでブロックする。
...
C<dequeue_nb>メソッドはC<dequeue>メソッドのように、キューの先頭からスカラーを取り除く。だがC<dequeue>と違って、C<dequeue_nb>はキューが空でもブロックしない。代わりにC<undef>を返す。
