...
App::Packer::Backend::PAR - PAR用App::Packerのバックエンド

=head1 説明

This module implements an B<App::Packer> backend based on B<PAR>.

このモジュールはB<PAR>に基づいたB<App::Packer>のバックエンドを実装する。
...
現在のところ、このモジュールはC<pp>の機能のうち最小限のサブセットしか
持っていない。だが作者らはこのモジュールが、最終的にC<pp>の仕事を
プログラム可能なモジュール風に引き継げるようになると期待している。
