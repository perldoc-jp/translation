...
単純なシンタックスは、ほとんどのプロジェクトのために必要なものです。
...
このスタイルは手軽です。このようなファイルで仕事をしながら、Config::Simple が
デフォルトブロックにそれらを割り当てるなら、デフォルトで、'デフォルト'と呼ばれます:-)
(訳註:謎)
...
最初の例のように、read() は guess_syntax()ファイルに呼びます。
...
更新しようとするキーが存在しなければ、作成されます。たとえば、
iniファイルに、新しい"[session]"ブロックを追加するには、そのブロックがなくても:
...
たまには、プログラムが自分自身の設定ファイルをオンザフライに、
可能ならユーザの入力から、生成したいかも知れません。
...
ですので、何をしているのか分かっているなら、これを使ってください。
...
切に、クォートされます。値の一部にあるダブルクォートは、全てバックスラッシュでエスケープされます。
