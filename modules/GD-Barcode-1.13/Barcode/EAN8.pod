...
GD::Barcode::EAN8 - GDでEAN8(JAN8)バーコード・イメージを作成します
...
I<エラーチェック付き>
  ...
  die $GD::Barcode::EAN8::errStr unless($oGdBar);   #長さが不正


=head1 説明

GD::Barcode::EAN8はGD::Barcodeのサブクラスで、
GDでEAN8(JAN8)バーコードイメージを作成することを可能にします。
...
コンストラクタ。
I<$sTxt>のためのGD::Barcode::EAN8 のオブジェクトを作成します。
I<$sTxt>は7あるいは8桁の数字([0-9])です。
もしI<$sTxt>が7桁であれば、あなたに代わってCDを計算します。
（訳者注:ドキュメント中は12桁になっていますが、7桁の間違い）
