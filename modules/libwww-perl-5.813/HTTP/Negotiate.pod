...
HTTP::Negotiate - サービスするドキュメントの変数を選択する
...
コンテントネゴシエーションはネゴシエートできる変数とさまざまな
...
変数は関数 choose() の呼び出しにより、優先順序が並べられます。
...
HTTP::Request オブジェクトのどちらで、"Accept*" ヘッダが探されます。
...
計算されたクォリティが 2 つの変数で同じであれば、サイズにより
ソートされます(小さい方が先)。
I<例>:
...
クライアントに決して提供されなくても、ゼロのクオリティ変数が返される
リストに含まれることに注意してください。
...
スカラコンテキストでは最も高いスコアの変数の識別子を返すか、何もクオリティが
ゼロでなければ、C<undef> を返します。
...
変数は以下の値のリストにより説明されます。
属性がおかしいか、その変数には当てはまらなければ変わりに C<undef> を
使ってください。
...
これはその変数のための名前として使う文字列。
この選択された変数への識別子は choose() により返されます。
...
劣るかもしれません。
qs の値はコンテントプロバイダにより各変数が設定されなければなりません。
...
その変数に適用される一つまたは複数のコンテントエンコーディング。
