...
いくつかの入力で構成されます。
フォームの状態は can be tweaked and it can then be
asked to provide C<HTTP::Request> objects that can be passed to the
request() method of C<LWP::UserAgent>.
(TBT)
...
parse() クラスメソッドは will parse an HTML document and build up
C<HTML::Form> objects for each <form> element found.
...
Calling this method turns
this input on without having to know the value name.
入力が既にオンなら、何も起きません。
(TBT)
