...
あなたの秘密鍵が暗号化されているならば、Net::SSLey からのデフォルトのパスワード
プロンプトが望ましくないかもしれません。
...
certificateZ<> が入っているファイルの場所を示すため、このオプションを
使わなければなりません。
...
コンテキストを再 利用したければ、SSL_reuse_ctxオプションを使ってください
