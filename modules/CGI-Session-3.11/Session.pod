...
L<CGI::Session::Tutorial|CGI::Session::Tutorial> - CGI::Sessionに関して広範囲に記述されたマニュアルです。ライブラリの構成やドライバの特性についての説明を含んでいます。

=item *

L<CGI::Session::CookBook|CGI::Session::CookBook> - 実生活での問題に対する実用的な解決法
...
"FreezeThaw"及び"Default"です。デフォルトは"Default"なのですが、
...
Note: 全ての有効期限はセッションの最終アクセス時間に関するものであって、作成された時間ではありません。ただちにセッションを期限切れにするにはC<delete()>を呼び出して下さい。特定のセッションパラメータをすぐに期限切れにするにはそのパラメータに対してC<clear()>を呼び出して下さい。
...
もしあれば、セッションの有効期限を示します。expire()メソッドを通してアクセスできます。
...
CGI::SessionはいくつかのL<dirvers|"ドライバ">、L<serializers|"シリアライザ">及びL<id generators|"IDジェネレータ">のようなモジュール方式のコンポーネントから構成されます。このセクションでは使用可能なものについてリストしています。
...
L<File|CGI::Session::File> - セッションデータをファイルにそのまま格納するデフォルトのドライバです。フルネーム: B<CGI::Session::File>

=item *

L<DB_File|CGI::Session::DB_File> - BerkelyDBにセッションデータを格納するためのドライバです。L<DB_File>を必要とします。フルネーム: B<CGI::Session::DB_File>

=item *

L<MySQL|CGI::Session::MySQL> - MySQLのテーブルにセッションデータを格納するためのドライバです。L<DBI|DBI>及びL<DBD::mysql|DBD::mysql>を必要とします。フルネーム: B<CGI::Session::MySQL>
...
L<Default|CGI::Session::Serialize::Default> - デフォルトのデータシリアライザです。標準のL<Data:Dumper|Data::Dumper>を使用します。フルネーム: B<CGI::Session::Serialize::Default>。

=item *

L<Storable|CGI::Session::Serialize::Storable> - L<Storable>を使いデータをシリアライズ化します。L<Storable>を必要とします。フルネーム: B<CGI::Session::Serialize::Storable>

=item *

L<FreezeThaw|CGI::Session::Serialize::FreezeThaw> - L<FreezeThaw>を使って
データをシリアライズ化します。L<FreezeThaw>を必要とします。フルネーム: B<CGI::Session::Serialize::FreezeThaw>
...
L<Incr|CGI::Session::ID::Inct> - 自動的にインクリメントされるidを生成します。フルネーム: B<CGI::Session::ID::Incr>

=item *

L<Static|CGO::Session::ID::Static> - 静的にセッションidを生成します。 B<CGI::Session::ID::Static>

=back

=head1 クレジット

以下はCGI::Sessionの開発に対してpatcheを提供したり、提案出したりして
...
L<CGI::Session::Tutorial|CGI::Session::Tutorial> - さらに幅広く記述されたCGI::Sessionのマニュアル
