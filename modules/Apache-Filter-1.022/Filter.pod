...
=head1 概要 (SYNOPSIS)
  ...
  my $fh = $r->filter_input(); # 任意 (あなたは input FH が不要かもしれない)
  while (<$fh>) {
...
ファイルハンドルは Filter1 が STDOUT に書き出したどんなものも含みます。Filter3 へは
それは Filter2 が STDOUT に書き出したどんなものも含みます。Filter3 の出力は
直接ブラウザに行きます。

その Filter1, Filter2, そして Filter3 モジュールは B<前進方向> の順で並び、
...
チェーン内でたった一つの要素の時、これを実行するのに、オーバーヘッドは殆どない
べきです。

現在、下記の公開モジュールはフィルタ認識します。他にもご存知であれば
どうか私に教えてください。

 Apache::Registry (ここに含まれている Apache::RegistryFilter を利用すること)
 Apache::SSI
 Apache::ASP
 HTML::Mason
...
Apache::Filter は Apache のサブクラスで全ての Apache メソッドが利用可能です。
...
このメソッドはあなたに、ユーザによりリクエストされたファイル ($r->filename)
または前のフィルタの出力のどちらかのファイルハンドルを与えるでしょう。
もしスカラーコンテキストで呼び出したら、あなたは $r->filename がうまく見つかり、
開かれたのかどうかをあなたに知らせる Apache ステータスコード (OK, NOT_FOUND,
または FORBIDDEN) もまた得られるでしょう。もしそうでなければ返された
ファイルハンドルは undef でしょう。

=item * $r->changed_since($time)

現在の入力が C<$time> から変化したと思われるかどうかという事に基づき、
真か偽を返します。現在、これを理解するための判断基準はこれです:
...
危険でした。なぜなら、前/後のフィルタもまたヘッダを送出するかもしれなく、
その場合、あなたは送信を取得すると重複したヘッダを持つでしょう。
...
あなたは、概要 (SYNOPSIS) で私が C<"PerlSetVar Filter On"> と言った事に
気付くでしょう。この情報は実際はこのモジュールで使われず、それら自身のフィルタ
...
幾つかのフィルタが存在したら、殆ど確実に不適当になります。これは
もしあなたがコンテンツ長をセットするモジュールを書いた場合、それらは
$r->filter_register 呼び出しの B<後に> それをすべきであることを意味します。
...
何が起こるかまでは考慮していません。だからそうしないで下さい。
