...
コンパイルされた形式は、ディスクのjit_path ディレクトリに保存され、
ロード段階にコントロールが返します。

=back

これは、HTML::Templateのキャッシュモードの働きによく似ているかもしれませんが、
いくつかの、重要な違いがあります：
...
コンパイル段階に、長時間かかります。システム依存であり、
大きなテンプレートをコンパイルするのに、数秒かかることもあります。

=item *

結果として、コンパイルされたテンプレートは、普通の、キャッシュされたテンプレートよりも、
だいぶ速いです。作者のベンチマークによると、HTML::Template::JITは、
既にコンパイルされたテンプレートは、4から8倍、HTML::Templateのキャッシュモードよりも速く実行します。

=item *

結果として、コンパイルされたテンプレートは、普通のキャッシュテンプレートよりも、
少ないメモリを使います。もしも、全てのテンプレートが既にコンパイルされているのなら、
...
使いかたは、普通の、HTML::Templateの使いかたと同じですが、new()のオプションにいくつか追加
...
STDERRに漠然としたデバッグの束を吐き出します。次のことに、留意してください。
