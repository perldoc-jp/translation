...
Requestオブジェクトに翻訳します。出力側では、HTTP::Responseオブジェクト
...
ック(ポストバック)からのイベントのポスティング; セッション状態の再定義、
リソースマネージメント; セッションエイリアス; シグナルのタイプ、ハンド
ラ、落とし穴; シグナルウォッチャ; 同期イベントvs非同期イベント; 指定時
刻に動作するイベント(アラームとディレイ)についての情報を含んでいます。
