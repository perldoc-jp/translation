...
上記の例では、両方のクラスの初期化メソッドが S<C<'foo' =E<gt> 'bar'>> 
を受け取ります。C<My::Class>の初期化メソッドは S<C<'param' =E<gt> 'value'>> 
も受け取り、C<Parent::Class>の初期化メソッドは S<C<'data' =E<gt> 'info'>> 
も受け取ります。この仕掛けでは、特定のクラスに対するパラメータは、
...
1つのハッシュリファレンスにI<平坦化>されるでしょう。例を示します:
...
するため、共有データに変換する必要があります。C<-E<gt>set()>メソッドは
あなたに代わってこれらの作業を行います。
...
アプリケーションコードでは使えません。オブジェクトメソッド内で、
オブジェクトフィールド配列/ハッシュにデータを設定する際に、
このメソッドを使ってください。
...
変更可能な値を返します。この変更可能な値は例えば、
代入文や置換演算子の左辺として使うことができます
(これ故C<LVALUE> (左辺値) と呼ばれます)。

Perl 5.8.0以降では、Object::InsideOutはC<LVALUE>コンテキストで使われた場合に
オブジェクトのフィールドに値を設定する、
...
エクスポートします。クラスに付加的な破棄処理
 (例: ファイルハンドルのクローズ) が必要なら、
...
上記のように、累積メソッドはC<:Cumulative>
 (または S<C<:Cumulative(top down)>> )
属性でタグ付けされ、クラス階層をI<トップダウンで>伝搬します
...
判断したとします。単一の親クラスを持つ場合は通常、
単に C<$self->SUPER::format_name($name)>
...
サブルーチンを用意しなければなりません。
C<:Automethod>サブルーチンへの引数は、オブジェクトおよび
オリジナルのメソッド呼び出し時に与えられた引数となります
 (C<AUTOLOAD>と同じです)。 C<:Automethod>サブルーチンは、
要求されたメソッドが分かる場合はその機能を実装した
サブルーチンリファレンスを返し、
そうでない場合は要求の処理方法が不明であることを示すため単にC<return;>で
終了してください。
...
C<Object::InsideOut::pump()>に代わって
ダンプされたデータからオブジェクトを作成する、
C<:Pumper>ラベルを付けたサブルーチンを用意する必要があるでしょう。
...
シリアル化もサポートしています。
クラスがL<Storable>を使ってシリアル化できることを
指定する方法が2つあります。
...
これにより、オブジェクトをシリアル化するためのC<-E<gt>store()>および
C<-E<gt>freeze()>メソッドが、逆シリアル化するためのC<retrieve()>および
C<thaw()>サブルーチンが使えるようになります。
...
Object::InsideOutは通常、
これらのサブルーチンをクラスおよびアプリケーションコード
から呼び出すことができなくなるようにします (隠してしまいます)
...
メソッド名が衝突した場合は、
完全修飾名を使ってC<-E<gt>heritage()>を呼び出せます:
...
分離 (つまり削除) します。外部オブジェクトは、
クラスまたは実際に使っている継承オブジェクト
...
アクセスできます。継承オブジェクトは強くカプセル化されており、継承を
...
(カプセル化した外部オブジェクトが含まれる場合)。従って、次の例は動作しません:
...
メソッドをI<オーバーライド>したときです (例えば、 C<My::Class> 自身が
C<-E<gt>bar()>メソッドを持つ場合)。
このようにオーバーライドされたメソッドを直接呼ぶことはできません。
このようなオーバーライドを意図して行っているのであれば、
これは問題にならないでしょう: オーバーライドをバイパスするコードは
誰も書くべきではありません。
...
使うことを強く推奨します。もしこれが動作しない場合は、
S<C<use base>>を試してください。
...
属性でラベル付けして下さい。あなたのハンドラは、入力引数に対して
...
同様に、上記の様な動作するにもかかわらずC<$SIG{'__DIE__'}>をローカル化しない、
他モジュールのコードを呼び出す場合は、
