...
現在MIDIトラックはタイプ、イベント、データという三つの属性を持っている。あなたが扱うほとんど全てのトラックは"MTrk"というタイプだ。これはデフォルトのタイプである。イベントはMTrkを構成している。もしあるトラックがMTrkでないか、あるいはMTrkとして解析されない場合、それはデータ属性ということだ。
...
MTrkタイプのトラックがエンコードされる時、そこに定義済みのデータがあるなら、それがエンコードされたものである（「データをエンコードする」とは、）。データが""で定義されていてもエンコードになることに注意（データがundefになっているならエンコードされない）。しかし、定義済みデータがMTrkにない場合、トラックイベントがエンコードされる。これはC<MIDI::Event::encode>の呼び出しを通じて行われる。
...
非MTrkタイプのトラックがエンコードされるなら、トラックのデータがエンコードされる。トラックにデータが無い場合は、0サイズのトラックとして振舞う。
...
つまり、(1)イベントはMTrkタイプのトラックの内部でのみ意味を持つ。(2)定義されたデータとイベントの両方を欲しがりはしないだろう。(3)あなたの時間の99.999%はMTrk内のイベントに気をまわすことになる。なぜなら、それがあなたが関わる全てだからだ。
...
新しいトラックオブジェクトを返す。デフォルトでMTrkタイプになる。これはたぶんあなたが望むものだろう。任意のオプションとして無名ハッシュを渡せる。モジュールは四つのオプションを認識する。：C<data>は新規のトラックデータを与えられた文字列に設定する。C<type>は新規のトラックのタイプを与えられた文字列に設定する。C<events>は新規のトラックのイベントを与えられたリストリファレンスの内容にセットする（配列の配列へのリファレンス。正確な情報はL<perllol>を参照すること）。そして C<events_r>はC<events>と同じ。
...
このメソッドはトラックの内容を複製し、オブジェクトを返す。もしこの機能がなぜ必要なのかよくわからない場合は、次の例を考えてみよう。
...
だから、あなたはfunk_it_upとturn_it_outというルーチンをつくり、何らかの方法でそれぞれのトラックを修正することになる。しかし問題は、上記のコードではあなたの望むことをやってくれないだろうということだ。$funkの最後尾のトラックオブジェクトと$sambaの最後尾のトラックオブジェクトがI<同じオブジェクト>だからだ。驚くかもしれないが、一つのオブジェクトが別のオプスの中に同時に存在しうる（上のようなケースの除けば、これはこれで良い）。だからオブジェクトをコピーする必要がある。上記のコードを変更すると：
...
ついでにいうと、このようなコピーの必要性が、オプスにおいても生じる（そして実際、リファレンスを基にしたどんなデータ構造でも生じる。オプスとトラックはMIDI要素のほとんどの全てのケースをカバーしなければならないにもかかわらず）。これがオプスを丸ごとコピーするために$opus->copyが存在する理由である。
...
（一つだけのトラックをコピーする必要があるなら、単に$new = [@$old]でよい。何らかの理由で一つのトラックを越えてイベント構造（リストのリスト）をコピーする必要が出たらMIDI::Event::copy_structureを使うこと。）
...
トラックのイベントリストを返す。あるいは、@eventsを指定し、
それが空でなければセットされる。
...
つまり、$track->events(@events)は（@eventsは空ではない）イベントリストの
セットの仕方で、@events = $track->eventsはイベントリストの取得方法である。
...
トラックのイベントリストへのリファレンスを返す。あるいは、$events_rを指定するとリファレンスがセットされる。実際のところ、"$events_r"は、配列の配列(LoL)へのリファレンスなら何でもよくて、C<$some_events_r>からだろうと、C<[@events]>からだろうと、古き簡素なC<\@events>からだろうと構わない。
...
もともと$track->eventsがイベントを扱う唯一の方法だった。だが私が$track->events_rを付け加えたのは、(1)イベントリストを何らかの有意義な理由で()にセットできるようにするため。(2)（10000要素の長さにもなるような）イベントリストを前後でI<コピー>せずに、直接トラックイベントを操作できるようにするためである。そこでこのように書くことができる：
...
$trackのタイプを返す。'MFoo'を与えれば、それがセットされる。たぶん次のような文脈以外では、このメソッドを使う必要が生じることはないだろう：
