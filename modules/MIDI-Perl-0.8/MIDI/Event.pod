...
これはMIDI通信で使われるものとしてMIDIイベントを記述しているようにみえることに注意。リアルタイムデータ挿入は、MIDIファイルで生じうるようなものではないと思う。
...
実際、このライブラリは、これまで書いてきたように、メッセージにバイトがアルタイム挿入されるようなMIDIデータを正しくパースすることはI<できない>。
