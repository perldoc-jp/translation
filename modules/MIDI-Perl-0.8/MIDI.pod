...
このイベントは命令、トラック開始からのタイムオフセット、そしていくつかの
パラメータからなるリストである。
...
MIDIエンコードの内幕を扱うためにいくつかの選択肢があるなかで、
"標準"MIDIと、配列の配列を操作する基礎を理解することによって、
...
B<メタイベント>：
ほとんどのメタイベントは、イベントと違って全てのチャンネルに適用される。
イベントの方は大概一つのチャンネルにだけに適用される。

B<note>: my oversimplistic term for items in a score structure.

B<ノート>：私によって過度に簡略化されたスコア構造体に含まれる項目を表す言葉。
