...
shared_cache - 1 に設定すると、モジュールは(CPANから利用できる)
...
これらがどのように機能するかは、IPC::SharedCache をご覧下さい。
...
double_cache - 1 に設定されると、モジュールは可能な限り
一番よいキャッシュのために、
...
blind_cache - 1 に設定されると、通常のキャッシュと全く同じように動きますが、
...
shared_cache_debug - 1 に設定するとモジュールは
IPC::SharedCache での debug オプションをオンにします。
...
素晴らしい候補は CGI.pm query オブジェクトです。例:
...
Q: HTML::Templateについて議論したり、助けを得るための場所はありますか？
...
/[Tt]/は/t/iにくらべてより速く、よりスペースを有効に使います -
文字列の長さの倍程度。
...
A: 短く言えば: それはできません。 長く答えると、
HTML::Templateの背後にある基本的な概念 - デザインとプログラムは分けるべき - に
反するのでするべきではありません。
