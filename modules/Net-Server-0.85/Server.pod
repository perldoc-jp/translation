...
（C<Net::Server::Fork>）、予めフォークされた子プロセスを一定数維持する
...
行う際に、flockによるserialized acceptという考えも利用している
（PreForkはflock、IPC::Semaphore、そして直列化を制御するためのパイプから
選ぶことができる）。
...
フォークし、子プロセスがクライアントの処理を制御し、exitする。
...
IO::Multiplexを利用する。このパーソナリティはフォークを使わないで一つの
...
フォークする。サーバは常にC<max_servers>個のクライアントリクエストを
受け付けられることを保証する。このタイプはアクセスの激しいサイトに適して
おり、負荷に関わりなくmax_server個のプロセスを実行できる。
これはほとんどのアプリケーション用に適しているといえる。複数のポートを
受け付けるために、flock、IPC::Semaphore、ないしは子プロセスを直列化する
ためのパイプのいずれかを利用する。同時受け付けのための複数の子プロセスを
許可しないOSに対処するため、直列化を単一ポート用に切り替えるかもしれない。
...
フォークする。サーバは常に、少なくともC<min_spare_servers>個以上、
...
使ったNet::Serverが起動する。Net::Serverの上にサーバを構築する際、
...
C<<Net::Server->run(key1 => 'val1')>> 。予め作成しておいたオブジェクトを
...
bindメソッドがコマンドラインから自らを解放した後、サーバが
forkするかどうかを設定する。それ後サーバはC<POSIX::setsid()>
...
フォークされた子プロセスがサーバプロセスを完全にシャットダウンする
...
先に挙げた全てのC<ARGUMENTS>は同じ名前でサーバオブジェクトの
...
http://perlmonks.orgのI<traveler>とI<merlyn>[旅人と魔法使い]は
