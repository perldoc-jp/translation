...
B<-encoding> 引数を、XHTML のためのキャラクタセットを指定するために使うことが
出来ます。
...
この宣言の唯一の目的は文字セットエンコーディングを宣言することです。
...
文字セット ISO-8859-1 (デフォルト)を指定すると、標準の HTML エスケープの
ルールが使われます。
"<" 文字は "&lt;"、">" は "&gt;", "&" は "&amp"、クォート文字は
"&quot" になります。
...
守るため、
信用できないデータに escapeHTML() を呼ぶべきです。
文字セットを変更するには、charset() を使ってください。
