...
これは I<通常の接頭辞に追加されます>。
例えば、コードが /usr/local/lib/perl に行く場合、
DESTDIR=/tmp/ に設定すると、インストールは
/tmp/usr/local/lib/perl に行きます。
...
Makefile.PLを含むサブディレクトリの配列への参照。
例:ext/SDBM_File内の['sdbm']
(TBR)
...
INST_SCRIPTからこのディレクトリにファイルをコピーする'make install'によって使用されます。
(TBR)
...
perlプログラムとして処理されるファイルのハッシュを参照します。
MakeMakerは、デフォルトで、見つかった*.PLファイル(Makefile.PLを除く)をキーとし、ファイルのベース名を値とします。
例:
(TBR)
...
*.PLファイルは、ターゲットファイル自体への出力を生成することが想定されています。
同じ*.PLファイルから複数のファイルを生成できる場合、ハッシュの値はターゲットファイル名の配列への参照になります。
次に例を示します。
(TBR)
...
この変数は、コマンドラインで構築されたPerlプログラムのコンテキスト内で補間されるため、追加の\\が必要です。
二重引用符は、そのコマンドラインを構築するために-eスイッチとともに使用されます。
#はMakefile用にエスケープされます。
...
ブール。
このパラメータがtrueの場合、必要なモジュール(またはその正しいバージョン)がないと致命的になります。
perl Makefile.PLは適切なメッセージで終了します。
(TBR)
...
注:依存関係がない場合にテストを早期に停止するためのショートカットについては、L<Test::Harness>を参照してください。
(TBR)
...
このパラメータは、後で解決される可能性のある単純な要件には使用<しません>。
たとえば、モジュールの<make test>が失敗した後などです。
(TBR)
...
C<PREREQ_FATAL>を使用する必要があるのは<極めて>まれです。
(TBR)
...
Hashref:この拡張を実行するために使用可能である必要があるモジュールの名前(たとえば、SDBM_FileのFcntl)はハッシュのキーであり、必要なバージョンは値です。
必要なバージョン番号が0の場合、すでにインストールされているバージョンがあるかどうかをチェックするだけです。
(TBR)
...
それでも別のソリューションが必要な場合は、ニーズに合った別のサブルーチンを開発し、F<makemaker@perl.org>に差分を送信します。
(TBR)
...
さらに、META.ymlモジュールのメタデータファイルが作成され、これがMANFESTに追加されます。
この動作をオフにするには、NO_METAフラグを使用します。
(TBR)
...
このコマンドはデフォルトでNULLコマンドに設定されます。
その後に続く$(TOUNIX)は、UNIXではデフォルトでNULLコマンドに設定され、配布ディレクトリ内のファイルをUNIX形式に変換します。
次に、そのディレクトリでC<tar>を実行してtarfileに変換し、ディレクトリを削除します。
