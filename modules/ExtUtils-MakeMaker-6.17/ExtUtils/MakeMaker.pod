...
AFS ユーザーは will have to specify the installation directories as these most
...
デフォルトは $(SITEPREFIX)/man/man$(MAN*EXT)。
(TBT)
...
'make install' で使われ、which copies files from INST_ARCHLIB to this
directory if INSTALLDIRS is set to vendor.
(TBT)
...
'make install' で使われ、which copies files from INST_BIN to this
directory if INSTALLDIRS is set to vendor.
(TBT)
...
'make install' で使われ、which copies files from INST_LIB to this
directory if INSTALLDIRS is set to vendor.
(TBT)
...
These directories get the man pages at 'make install' time if
INSTALLDIRS=vendor.
デフォルトは $(VENDORPREFIX)/man/man$(MAN*EXT)。
(TBT)
...
これを使うかは makefile 次第です。
(L<Config/lddlflags> 参照)
(TBT)
...
or C<-noprototypes>.
詳しくは xsubpp の文書を参照してください。
MakeMaker は空文字列がデフォルトです。
(TBT)
...
then
何もしない F<Makefile> を作るけれども、全ての「通常の」ビルドターゲットを
成功するようにできます。
そうするためには、次のようなものを
(TBT)
