...
ユーザがスーパーユーザー権限を持っていてかつ、AFS や相対的に
...
perl を実行したいときには $(PERL) ではなくてこれを使ってください。
必要なフラグが設定されています。
...
インストールする *.pm 及び *.pl ファイルをハッシュリファレンスで指定します。
例:
...
ライブラリファイルを含んでいるサブディレクトリを配列リファレンスで
指定します。
デフォルトでは ['lib',$(BASEEXT)] です。
