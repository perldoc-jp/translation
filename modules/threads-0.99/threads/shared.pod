...
もし新たに生成したリファレンスを共有したい場合、残念ながら、Perlの設計段階の問題のせいで、C<&share([])>とC<&share({})>という構文を使う必要がある。
...
C<bless>は共有リファレンスに対してはサポートしていない。現行のバージョンでは、C<bless>はスレッドローカルなリファレンスをblessするだけである。
blessは他のスレッドにまで広がらない。将来のPerlのバージョンでは実装されることが期待されている。
...
C<share $hashref->{key}>しても、share()はエラーメッセージを出さない。しかし、C<$hashref->{key}>は共有されB<ない>。
C<lock $hasref->{key}>しようとすれば"locking can only be used on shared values"（ロックは共有変数に対してのみ使用できる）というエラーが発生する。
