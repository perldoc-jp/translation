...
Moose::Cookbook::Basics::Recipe12 - Mooseを使っていないベースクラスを拡張する
...
このレシピではMooseを利用してMooseベースではない親クラスをサブクラスするやり方を説明します。このレシピでうまく行くのは親クラスがblessされたハッシュリファレンスの場合のみです。親クラスがちょっと変わったことをしている場合はL<MooseX::InsideOut>を試してみてください。
