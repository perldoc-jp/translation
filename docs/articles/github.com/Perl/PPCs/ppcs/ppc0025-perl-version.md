...
これはまた永遠に、"Perl&#160;5"を指定するのではなく、単に"Perl"と言うときに人々が意味する言語でもあります。
より古いPerlのバージョンもありましたが、それらは古代の歴史です。
...
Perl&#160;4(それ自体は、最初のラクダ本の利便性のために再バッジされたPerl&#160;3のリリースにすぎません)まで、Perlは、そこで書かれたコードが異なる構造になるという意味で、異なった、独特の初期の言語でした。
...
そうすることでまた、いくつかの小さな点で互換性を破壊し、いくつかの古いPerlの詩を壊したことで有名です。
...
Perl&#160;6がPerl&#160;5によって設定された先例を、より大きな規模で繰り返すことが明らかであると感じられたことは容易に理解できます。
...
その意図は、バージョンバンプを、必要なだけ広く、一度だけ互換性を破る機会として捉え、より深い問題を修正し、言語自体の観点から定義できるように言語を正規化し、さらなる根本的な破壊なしに進化できる強固な基盤を築くことでした。
...
これは明らかに非常に密接に関連した言語ですが、多くの点で大きく異なります。
...
Perl&#160;5は、5.10の時代にPerl&#160;6の一部の初期の設計を逐語的に採用しようとした試みから大きな被害を受けました。
当時でさえ、これを機能させるには言語はあまりにも離れすぎていました。
その結果、これらの機能の多くは、苦労して言語から再び削除されなければなりませんでした(一部は現在でもまだ存在しています)。
...
Perl&#160;6が独自のものになったとしても、Perl&#160;5が近い将来に進化することに関心があり続けました。
...
言語ファミリーのフレームワークでさえ、Perl&#160;5はもはや最終的に時代遅れになるバージョンではありませんでしたが、より古く、より不格好で、時代遅れのフレーバーとして現れました(実際、Perlの将来についての両方の概念は、Perl&#160;6の外部認識にも同様に問題を引き起こしました)。
...
突然、それはもはやいかなる形であってもPerlの未来ではなく、より高度なフレーバーでさえもなくなりました。
...
Perl&#160;5には現在と30年の長い歴史がありますが、今は再び未来が必要で、外部認識を大きく修正する必要があります。
...
それから、振り返って、Perlが決して立ち止まっていなかったことを認識する機会があります。
...
変化のペースは5.0以降、以前と比較して減少しましたが、その理由は、その新しい機能の可能性がはるかに大きいことと、CPANを通じてその可能性が実現されたことであり、CPANはそのコアを変更することなく新しいニッチを埋めることができました。
Perl&#160;4からPerl&#160;5への移行が完全に成功したときに、Perl&#160;5からPerl&#160;6への移行が実現しなかった理由は、それぞれの連続性と不連続性の組み合わせにあります。
Perl&#160;5は、Perl&#160;4の開発が終了した時点でかなり早く実際に実装され、真に新しくより強力な言語であるにもかかわらず、Perl&#160;5はほぼ完全にPerl&#160;4のスーパーセットでした。
...
その後の試みは、言語の継続性に対するこの要件を実証しました:
[Kurila](https://github.com/ggoossen/kurila)、[Moe](https://github.com/MoeOrganization/moe)、そしてそのはるかに小さな野心にもかかわらず、Perl&#160;7も、最終的にはすべて牽引力を得ることができませんでした。
...
将来の機能の束でオフにされる機能フラグで名前を付けることです。
...
この移行の技術的な詳細はここで提案されているものとは異なりますが、Java&#160;5が内部的にまだほぼ1.5であったという手法と、この提案がXSコードを扱う方法との間には類似点があります。
教訓は、最もありそうもない情報源から得ることができます。
* 一部の読者にとっては、Solaris&#160;7とSunOS&#160;5.7以降との関係が思い浮かぶかもしれません(ただし、Solarisと以前のバージョンのSunOSの関係はより複雑です)。
* この変更を受けて、私たちのバージョン管理のテンポはNode.jsに続くものと一致します; 毎年新しい奇数バージョンと偶数バージョンがあり、それぞれ新しい開発リリースと安定リリースを示します。
...
バージョン42は、特にこれを行うのに特に説得力のある時点です; これはPerlの部外者にさえ明らかになるダグラスアダムズの参照のためだけではありません;
Perl文化の中にも、特にバージョンに関しては、さらに重要なポイントがあります;
42はたまたま6&times;7です;ASCII文字42(したがってv-string`v42`)はたまたま`*`です。
技術的な観点からも、バージョン48よりもかなり前にこの移行を行うのが適切です。
...
他のアプローチは、それが引き起こす可能性のある他の破壊に加えて、すべてのXSコードを壊します; これには、ほぼ確実に、この提案の破損のすべてが含まれます。
...
理想的には、これは42.0になりますが、後で行うことができます。
...
しかし、この破壊はある時点まで実際に広く現れることはありません;
また、おそらくほぼ完全に`PERL5OPT`と`PERL5LIB`に依存しており、CPANで見ることができ、おそらくエコシステムの一部に依存しているので、修正可能です。
...
インストールされたモジュールへのパスを気にするコードは、これが実現すると、新しい種類の`INSTALL_BASE`とその結果のディレクトリ構造を認識する必要があります。
...
ここでは、重要な部分は、より顕著なリビジョンである5ではなく、よりノイズが多い、つまり、バージョンである40です。
...
なぜなら、機能の束はサブバージョン固有ではなく、Perlのポイントリリースは5.6/5.8/5.10時代のような寿命と知名度を失って久しいからです。
...
1. Perl&#160;5.xを無期限に使い続ける
   ...
   しかし、これは認識の問題に対処するものではなく、こアプローチの非変更の性質により、バージョン番号の問題は将来の新たな課題に対して開かれたままになります。
...
2. すぐにPerl&#160;7に移行する
   ...
   私たちがこのオプションを決定するかもしれないのは、私たちがとにかく言語を進化させ続けているので、Perl&#160;6がもはや機能しなくなった今、私たちはメジャーバージョンをバンプするだけで自由になり、新しいメジャーバージョンを正当化するためにPerl&#160;5.0とは多くの違いがあるからです。
   これは実行可能なオプションですが、技術的なコスト(主にXSのバージョンチェックによる)はいくらか大きく、私たちは特に何も得ることなしにそれらを引き受けます。
...
3. 目玉機能(新しいオブジェクトシステムなど)に基づいてPerl&#160;7に移行する
   ...
   また、バージョンがバンプされる時期の先例を作るものではなく、バンプを正当化するのに十分な大きさの変更があるかどうかについての将来の議論を開き、停滞の新たな認識を避けるためにバンプを正当化するために、時々大きな目玉機能を導入する間接的な圧力を生み出します。
   同じように、以前に経験したように、このタイプのバージョンバンプでは、新しいものが発生するたびに、その範囲内に破壊的な変更を含める強い誘惑があります;
   これは、家をきれいにする機会があるように見えるからだけでなく、特定のものをより簡単に破壊する必要性が認識されているため、バージョンをバンプする理由にもなるからです。
...
XSレベルではオプション1と本質的に区別できないため、オプション1の技術的なコストのかからなさの多くを維持しています。
外見上はオプション2に最も近く、特定の目玉機能によって動機付けられることなくバージョンが変更されますが、オプション2とは異なり、バージョンを変更するという観点に対処します。
オプション3とはまったく異なる方法でこれを行います: 言語に大きな変更を加えてバンプを正当化するのではなく、重要な変更がすでに発生しており、言語がすでに進化しているという事実を強調します。
...
しかし、これらのうち最後のものはすでに主に歴史的なものです。
   ...
   その場合、これら2つの変数がある程度相補的であることを考えると、`PERL5OPT`に対応するものは`PERL_OPT`ではなく`PERLOPT`と名付けられるべきです。
...
数週間の熟考の後、2024年11月13日の冷たい早朝の散歩の間に、動機のセクションは最終的に具体化し、ライン川を見下ろす川沿いのベンチで文章になりました。
