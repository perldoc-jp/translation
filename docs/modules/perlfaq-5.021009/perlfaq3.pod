...
GUI エディタです。
UI のための狙いは、TIMTWTDI や「簡単なことは簡単に、難しいことも可能」の
ような Perl の原則に基づいています。
...
Cygwin パッケージ ( L<http://cygwin.com/> ) から。
...
始めるにはよい場所は L<perlootut>です; リファレンスとして L<perlobj> が
使えます。
