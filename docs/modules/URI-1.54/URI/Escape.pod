...
文字のいくつかは区切り文字として、ある URI 構成要素の一部として
使うように予約されています。
通常のデータとして取り扱いたければ、これらはエスケープされなければなりません。
さらなる詳細については RFC 3986 を読んでください。
...
エスケープされるデフォルトの文字セットは上記で示した
C<unreserved> 文字 I<ではない> 部分のすべてと、予約されている文字です。
つまり、デフォルトは:
...
JavaScript の encodeURIComponent() 関数は uri_escape_utf8() と似ています。
