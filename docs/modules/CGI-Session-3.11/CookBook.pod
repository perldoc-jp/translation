...
CookBook - cgiアプリケーションのセッション管理におけるチュートリアル
...
C<CGI::Session::CookBook>はB<CGI::Session>ディストリビューションに付属する
チュートリアルです。webアプリケーションにおけるライブラリの使用方法および
いくつかの問題に対する実用的な解決手段を示します。L<CGI::Session|CGI::Session>
およびその文法に慣れ親しんでいないのであれば、本チュートリアルを読むことを
お奨めできません。
...
しかしながら我々は例においてデフォルトのB<DSN>を用いていますが、
あなたが望むどのような設定を自由に思索してもらって構いません。

After initializing the session, we should "mark" the user with that ID.
We use HTTP Cookies to do it:

セッションを初期化した後、我々はIDを用いてユーザーを"区別"しなければ
なりません。我々はHTTPクッキーをそのために使用します:
...
最初の行ではB<CGI.pm>のC<cookie()>メソッドを用いてクッキーを作成しています。
その次の行ではB<CGI.pm>'s C<header()>メソッドを用いてユーザーのブラウザに
送信しています。

After the above confessions, we can move to some examples with a less 
guilty conscious.

下記の告白の後、我々は罪の意識をさほど伴わないいくつかの例を提示することが
可能です。
...
我々はユーザーの名前と電子メールアドレスを尋ねるフォームをサイト内に
抱えています。我々は訪問者が次回(ひょっとすると数日後や、数週間後でさえ
あることも)サイトを訪れた際、あいさつできるようにデータを格納しようと
考えています。
...
しかしながら極めて単純でストレートであるように思いますが、この種の例は
より強力なセッション管理のトリックが用いられます。
...
入力するためのフォームフィールドの名前がそれぞれ"first_name"および
"email"であると仮定すると、我々は第一にcgiパラメータからこの情報を
取得することが可能です。
...
フォームから上記の2値を手軽に取得した後、我々はこのようにセッションを
用いて、直ちに保存することが可能です:
...
もし上記の4行の解決法があなたにとって(それは私に取ってもなのだが)
長いように思えるのであれば、単一行のコードで実現することができます:
...
上記の文法では"first_name"および"email"パラメータをB<CGI.pm>から取得し、
B<CGI::Session>オブジェクトにそれらを保存します。さて、別の時間になったり、
別の場所であっても、あなたは単純にこう書くことができます
