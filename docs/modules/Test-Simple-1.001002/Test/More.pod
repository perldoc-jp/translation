...
スクリプトが、なぜスキップするのかの理由を宣言すると、即座に0(成功)で
終了します。
...
これは "no_plan" な計画より安全で、"no_plan" を置き換えるものです。
...
このコードは、C<exists $brooklyn{tree}> が真であるかどうかを
チェックしません。
このコードは、1 を返すかどうかをチェックします。これらはまったく違います。
似たような警告は、偽 と 0 にもあります。こういうケースでは、ok() を
使います。
...
正規表現に見える文字列(代替の区切りは現在対応していません)として
与えられます。
...
サブテストがどのように動くかによって、望むなら計画を省くことができます。
これはサブテストの末尾に暗黙の C<done_testing()> を追加します。
以下の二つのサブテストは同じです:
...
あるモジュール、あるいは複数のモジュールの読み込みに成功するかを
テストしたい場合があります。
例えば、より複雑なテストの実行に進む前に、単純に配布の全ての
モジュールが読み込めるかをまずテストしたい場合がしばしばあります。
...
指定された $module または $file を C<require> しようとします。
読み込みに成功すれば、テストは成功します。
さもなければ失敗し、読み込みエラーが表示されます。
...
C<require_ok> は、入力がモジュール名かファイル名かを推測します。
...
C<require_ok> と同様ですが、指定された $module を C<use> し、
ファイルではなくモジュールだけを読み込みます。
...
単にモジュールが読み込めるかをテストしたいなら、
C<require_ok> を使ってください。
...
単にテストでモジュールを読み込みたいだけなら、直接 C<use> を
使うことを勧めます。
これはテストの停止を引き起こします。
...
C<use Foo ()> の同等物がほしい場合は、モジュールを使うけれども
...
たとえば、二つデータ構造が等価であるかどうかを見る必要があるときもあります。
こういった場合のために、Test::More は、ちょっとした有益な関数を
提供しています。
...
is() と似ていますが、$got と $expected はリファレンスです;
それぞれのデータの構造を見てまわり、それぞれが等価かどうか、深い比較を
します。
...
is_deeply() は、リファレンスのでリファレンスされた値の違いを比較します、
リファレンスそれ自身(その型を除く)は無視されます。
つまり、bless や tie のような側面は、「異なる」と考えられません。
...
is_deeply() は、今のところ、関数リファレンスと glob の扱いは
非常に限定的です。
単に、同じリファレンス先を持っているかをチェックします。
これは将来改善されるかもしれません。
...
L<Test::Differences> と L<Test::Deep> は、より掘り下げた機能を
提供しています。
...
そのために、C<print STDERR> よりも安全に、自分で自分自身の
診断メッセージを書く方法があります。
...
C<print> と同様、@diagnostic_message は単純に一緒に結合されます。
...
C<ok() or diag()>を、C<open() or die()> のように覚えると
覚えやすいかもしれません。
...
B<注意> 診断の出力のための正確フォーマットは、まだ流動的です;
...
diag()と似ていますが、ハーネスでテストが動いている場合には表示されません。
冗長な TAP ストリームでのみ見られます。
...
人が読みやすい形式で、リファレンスの内容をダンプします。
通常 C<note> や C<diag> に与えたいと思うでしょう。
...
ある条件下でテストを動かすことによって、テストスクリプトが
die することがあります。
(MacOS での fork() のような)特定の関数やメソッドは実装されていなかったり、
(ネット接続のような)いくつかのリソースが利用できなかったり、
モジュールが利用できなかったりとか。
こういったケースでは、テストをスキップしなければならないか、
そうでなければ、失敗することが予想されるけれど、
将来的に動くであろう(todo テスト)ということを宣言しなければなりません。
...
Test::More がこれを扱う方法は、名前付きブロックによるものです。
...
ネストする SKIP ブロックは完全に安全です。
それぞれの SKIP ブロックには、C<SKIP> ラベルがなければなりません;
そうしないと、Test::More は、その魔法をうまく使えません。
...
todo ブロックでは、その中のテストは、失敗すると予期されます。
Test::More は、普通にテストを行いますが、特別なフラグを出力し、
それのテストが「todo」であることを示します。
Test::Harness は、この失敗を ok であると解釈します。
何かが成功すると、予期しない成功と報告します。
そうすると、あなたは todo が解消されたと分かり、
TODO フラグを外すことが出来ます。
...
コメントアウトすることではなく、
プログラム的な todo リストであるようになることです。
どれくらいするべき仕事が残っているのか分かるし、どのようなバグがあるのかも
気付きます;
また、それらのテストが修正された場合、即座に識別することが出来ます。
...
一度、todo テストが成功し始めると、単純に、ブロックの外側にそれを移します。
ブロックが空なら、削除します。
...
時々、これが可能でない場合があります。
失敗するテストは全てのプログラムが die するか、
ハングする原因になることがよくあります;
C<eval BLOCK> の内側で、C<alarm> を使ってもです。
...
B<もし、ユーザが出来ないかもしれないときには>、SKIP を使ってください。
これには、インストールされていないオプションのモジュールや、
...
これは、テストスクリプトにテストを置きたい(常によい考えです)けれども、
...
is_deeply() ができるより前に書かれた関数で、二つの任意のデータ構造の
...
eq_array() と似ていますが、要素の順番は重要では B<ありません>。
これは、深いチェックですが、順番の無視はトップレベルにしか
適用されません。
...
L<Test::Deep> には、よりよい集合比較関数があります。
...
Test::Builder はあらゆるテストライブラリーのための、一つの統合された
バックエンドを提供しています。
このことは、両方とも Test::Builder を使っている、二つの
テストライブラリーならば、B<同じプログラムで同時に使える> ことを
意味します。
...
Test::More は 5.8.1 以降の Perl で動作します。
...
スレッド対応は 5.10.1 より前ではあまり安定していませんが、
これはスレッドが 5.10.1 より前ではあまり安定していないからです。
...
Test::More は 5.6.2 以降の Perl でコアモジュールになっていますが、
Test::More はそれから進化していて、
あなたが使っている全ての機能が同梱されているバージョンの Test::More に
ないかもしれません。
あなたがモジュールを書いているなら、
あなたが必要な Test::More の最小バージョンをパッケージのメタデータに
指定することを忘れないでください。
例えば、もしあなたが C<done_testing()> を使っているけれども、
あなたのテストスクリプトが Perl 5.10.0 で動作してほしいなら、,
明示的に Test::More > 0.88 を要求する必要があります。
...
Subtests did not implicitly call C<done_testing()> until 0.96;
...
これは Test::More 0.88 でリリースされ、最初に同梱されたのは Perl 5.10.1 に
Test::More 0.92 の一部としてです。
...
C<cmp_ok()> は 0.40 で導入されましたが、
0.86 でオーバーロードされたオブジェクトに対しても安全にするための
重要なバグが修正されています;
修正されたものが最初に同梱されたのは Perl 5.10.1 に
Test::More 0.92 の一部としてです。
...
これらは Test::More 0.82 でリリースされ、
最初に同梱されたのは Perl 5.10.1 に
Test::More 0.92 の一部としてです。
...
完全なバージョン履歴は Changes ファイルにあり、
コアに含まれている Test::More のバージョンは L<Module::CoreList> を
使うことでわかります:
...
utf8 やその他の non-ASCII な文字を Test::More と一緒に使う場合、
"Wide character in print" の警告が出るかもしれません。
C<binmode STDOUT, ":utf8"> を使っても直りません。
(Test::More を駆動している) Test::Builder は STDOUT と STDERR を
複製しています。
そのため、それらの出力の方法の変更も含む、それらへのどんな変更も、、
Test::More にはわかりません。
...
一つの回避法は、できるだけ早く、Test::More (およびその他の Test
モジュール) を読み込むより前に、STDOUT と STDERR にエンコーディングを
適用することです。
...
より直接的な回避法は、Test::Builder で使われているファイルハンドルを
変更することです。
...
オーバーロードされたオブジェクトは B<文字列として> (または cmp_ok() では、
比較する演算子に従って、文字列か数字として)比較されます。
...
これは、Joshua Pritikin の Test モジュールをまとめて進化させたものです。
自分が ok() ルーチンを最初に書いたとき、この存在に
まったく気づいていませんでした。
このモジュールが在るのは、簡単にテストの名前を Test のインターフェースに、
...
ここでのゴールは、既存の Test.pm より、柔軟性を提供しつつ
...
L<Test::Harness> は、テスト実行器であり、Perl の出力インタプリタです。
C<make test> を駆動しているものであり、C<prove> ユーティリティが
由来するところです。
...
L<Test::Legacy> は、オリジナルのテストモジュールである、
他のテストライブラリと一緒にはうまく動かない、
Test.pm と共に書かれたテストを行います。
...
L<Test::Class> は、xUnit に似ていますが、より perl っぽいです。
...
L<Test::Deep> は、より強力で複雑なデータ構造のテストができます。
...
L<Test::Inline> は埋め込みテストというアイデアを示します。
...
L<Bundle::Test> は、便利なテストモジュールを全てインストールします。
