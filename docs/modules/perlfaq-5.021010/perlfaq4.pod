...
FAQ のこのセクションでは、数値、日付、文字列、配列、ハッシュその他の
...
(標準 Perl 配布キットの一部である)L<POSIX> モジュールは C<ceil()>、
...
perl の 5.000 から 5.003 では、三角関数は L<Math::Complex> モジュールの中で
実行されていました。
5.004 では、L<Math::Trig> モジュール(標準 Perl 配布キットの一部です)が
三角関数を実装しています。
内部的にはこれは L<Math::Complex> を使っていて、一部の関数は実数値を
複素数領域へ変化させることができます; 2 の inverse sine がその一例です。
...
L<perlfaq4> の後の方での例では CPAN にある L<Bit::Vector> を使っています。
perl 組み込みの関数よりも L<Bit::Vector> を選択する理由は、
...
そして L<Bit::Vector> は半端なビット数にも対応しています:
...
ただし、この形式では、C<..> 演算子がその範囲にあるすべての整数のリストを
生成するということに注意すべきです;
これによって大きな範囲を使った場合に大量のメモリを消費することになります。
しかし、この問題は C<for> ループの中で C<..> を使うときには起こりません;
なぜならこの場合は範囲演算子はリスト全体が作られることはなく、範囲内の
I<反復> に最適化されるからです。
従って
...
このようにしても 500,000 個の整数の中間リストが生成されたりはしません。
...
二度以上 C<srand> を呼び出してはいけません--乱数の質を落としてしまいます。
...
引き起こされることですが :-)。
L<http://www.cpan.org/misc/olddoc/FMTEYEWTK.tgz> の
"Far More Than You Ever Wanted To Know" の中の F<random> という記事で
...
Perl は C<rand> と C<srand> の実装に基となるシステムに依存しています;
生成された数値は十分にランダムではありません (特に Windows では:
L<http://www.perlmonks.org/?node_id=803632> を参照してください)。
C<Math> 名前空間にあるいくつかの CPAN モジュールはよりよい疑似乱数生成器を
実装しています; 例えば
L<Math::Random::MT> (「メルセンヌツイスター」; 高速) や
L<Math::TrulyRandom> (乱数を生成するのにシステムタイマーの不完全性を
使います; やや遅いです) を参照してください。
乱数に関するさらなるアルゴリズムは L<http://www.nr.com/> の
"Numerical Recipes in C" に記述されています。
...
L<POSIX> モジュールも日付をその年の何日目か、または何週目かに整形します。
...
任意の日付に対してその年の何日目かを得るには、C<localtime> の引数から
紀元からの秒数を求めるために、L<POSIX> の C<mktime> を使います。
...
L<Date::Calc> モジュールもこれらを計算する二つの関数を提供します:
...
システムによっては、L<POSIX> モジュールの C<strftime()> 関数が
...
Perl と共に配布されている L<Time::Piece> モジュールは、C<localtime> を
オブジェクトを返すものに置き換えます。
...
差分を引き算で得ることもでき、結果は L<Time::Seconds> オブジェクトに
なります:
...
フォーマットされた日付に対して作業したい場合は、L<Date::Manip>,
L<Date::Calc>, L<DateTime> といったモジュールが助けになるかもしれません。
...
その部分部分を標準の L<Time::Local> モジュールの C<timelocal> に渡せます。
さもなければ、CPAN にある L<Date::Calc>, L<Date::Parse>, L<Date::Manip>
モジュールを見るべきでしょう。
...
L<DateTime> モジュールで同じことをすることもできます:
...
CPAN にある L<Time::JulianDay> モジュールが使えます。
しかし、本当にユリウス日がほしいのか確認してください; 多くの人々が
ユリウス日に関して異なる考え方を持っています; 例としては、
L<http://www.hermetic.ch/cal_stud/jdn.htm> を参照してください。
...
L<DateTime> モジュールは単純で、夏時間に関わらず、前日の同じ時刻を返します:
...
L<Date::Calc> モジュールの C<Today_and_Now> 関数を使うこともできます。
...
Perl 5.10 から、L<Time::Piece> と L<Time::Seconds> は標準配布の
...
L<Time::Piece> モジュールは、オブジェクトを返す新しい C<localtime> を
エクスポートし、L<Time::Seconds> は秒数を示す C<ONE_DAY> 定数を
エクスポートします。
...
L<Time::Local> にも同じ問題があり、同じ特別な状況では間違った答えを
返します:
...
モジュールや、L<Regexp::Common> のようなその他のモジュールを
見ることもできます。
...
L<Business::ISBN>, L<Business::CreditCard>, L<Email::Valid>,
L<Data::Validate::IP> のように、特定の種類の入力を検査するための
モジュールもあります。
...
バックスラッシュ (C<\>) によるシェルエスケープは以下のようにして
取り除きます:
...
L<Interpolation> モジュールもまたあなたのために多くの魔法を使います。
...
ネストしたパターンやバランスされた式については、(perl 5.10 から利用可能な)
L<< (?PARNO)|perlre/C<(?PARNO)> C<(?-PARNO)> C<(?+PARNO)> C<(?R)> C<(?0)> >>
のような構文を参照してください。
CPAN モジュール L<Regexp::Common> はこのような正規表現を構築する
助けになります (特に L<Regexp::Common::balanced> と
L<Regexp::Common::delimited> を参照してください)。
...
より複雑な場合には、おそらくは L<Regexp::Grammars>, L<Parse::RecDescent>,
L<Parse::Yapp>, L<Text::Balanced>, L<Marpa::XS> のような CPAN にある
パースモジュールを使ってパーサを書く必要があります。
...
あるいは、ただ単に L<Text::Tabs> モジュール(標準 Perl 配布キットの一部です)を
使ってもできます。
...
L<Text::Wrap> (標準 Perl 配布キットの一部です)を使います。
...
L<Text::Wrap> に与える段落には埋め込みの改行があってはいけません。
L<Text::Wrap> は行を均等割り付けしません(左寄せします)。
...
または CPAN の L<Text::Autoformat> モジュールを使ってください。
...
その多くの能力を評価するために、L<Text::Autoformat> の文書を
参照してください。
...
しかし、大きな文字列中の、複数キャラクターから構成される部分文字列の数を
...
いくつかのモジュールがこのようなパースを扱います-- L<Text::Balanced>,
L<Text::CSV>, L<Text::CSV_XS>, L<Text::ParseWords> などです。
...
考えてみましょう。
ここで C<split(/,/)> を使うことはできません; なぜなら、クォートの内側にある
カンマで分割すべきではないからです。
...
あるいは、L<Text::PaserWords> モジュール(標準 Perl 配布の一部です)を
使ってこうします:
...
しかし、CSV のパースと生成のためには、自分自身で実装するのでなく
L<Text::CSV> を使うことを強く勧めます; 既に何年も試されテストされている
コードを単に使うことで、後でつまらないバグが起こることから守れます。
...
これはしないで下さい。
L<Text::CSV>, L<Text::CSV_XS>, L<Text::CSV_PP> のような、この
...
C<Text::Soundex> モジュールが使えます。
あいまいマッチングや近傍マッチングを行いたいなら、
L<String::Approx>, L<Text::Metaphone>, L<Text::DoubleMetaphone> といった
モジュールを試すのも良いでしょう。
...
あるいは、L<Text::Template> や L<Template> ツールキットのような
...
モジュールを使いたいなら、L<List::MoreUtils> の C<uniq> 関数を
試してみてください。
...
"in" という単語は I<in>dication で、おそらくデータを保管するのにリストや
配列ではなくハッシュを使うべきです。
...
一度だけテストする場合、標準モジュール L<List::Util> がこの目的のために
...
考えるかもしれません。
ここでは CPAN モジュールの L<FreezeThaw> を使っています:
...
L<List::Util> モジュールの C<first()> 関数が使えます。
...
L<List::Util> が使えない場合は、同じことをするために自分でループを書きます。
...
配列の添え字がほしい場合は、C<List::MoreUtils> の C<firstidx()> 関数を
使います:
...
または自分で書くなら、添え字に順番に変えていって、それぞれの添え字の
配列要素をチェックして、条件を満たすものが見つかるまで繰り返します:
...
Linked Lists" ( L<http://www.slideshare.net/lembark/perly-linked-lists> ) で
詳細について話していますが、単に彼の L<LinkedList::Single> モジュールを
使えます。
...
L<Array::Iterator::Circular> は循環配列のための反復子オブジェクトを
作ります:
...
良くありません; つまりこれは O(N**2) のアルゴリズムです。
...
CPAN にある L<List::Permutor> モジュールを使ってください。
リストが実際には配列なら、L<Algorithm::Permute> モジュール(これも CPAN に
あります)を試してください。
...
L<Algorithm::Loops> モジュールも the C<NextPermute> と C<NextPermuteNum> の
...
最初にその関数で使う部分を取り出します; なぜなら、sort BLOCK は同じ要素に
対して何度も何度も呼び出される可能性があるからです。
...
この手法に関する更なる情報については
L<http://www.cpan.org/misc/olddoc/FMTEYEWTK.tgz> にある
...
または CPAN の L<Bit::Vector> モジュールを使います:
...
L<Bit::Vector> は、ビットベクタ、小さい整数の就業、「大きな整数」の
...
ハッシュに対して C<keys>, C<values>, C<each> を混ぜると、イテレータを
リセットしてしまって処理が無茶苦茶になる危険があります。
...
例えば、大文字小文字を無視した順序のレポートを作るには、比較する前のキーを
小文字にするために C<lc> を使います:
...
L<DB_File/"In Memory Databases"> にあるように、L<DB_File> モジュールと
C<tie()> を使った、C<$DB_BTREE> ハッシュ束縛を使うことができます。
CPAN の L<Tie::IxHash> モジュールも有益かもしれません。
...
真を返します。
与えられたキーに対する値は C<undef> とすることができます; これは
C<$hash{$key}> を C<undef> にして、C<exists $hash{$key}> が真を
返すという状態です。
...
そしてその状態は以下のようになります; 大文字になっているのが変わった
場所です:
...
まず最初にハッシュからキーを取りだして、それをリストに格納します; そして、
先に説明した「重複の削除」問題の解決を行います。
例:
...
構造体とオブジェクト指向クラスの例が L<perlootut> にあります。
...
CPAN にある L<Data::Diver> モジュールもあなたのために動作します。
...
これは本質的には L<Data::Diver> がしてくれることです:
...
バージョン 5.8.0 から、ハッシュは与えられたキーの固定された数に
I<制限> できます。
制限ハッシュの作成と操作のためのメソッドは L<Hash::Util> モジュールで
エクスポートされます。
...
3 つめとして、L<Regexp::Common> には様々な種類の数値にマッチングする
正規表現があります。
...
POSIX システムを使っているのなら、Perl は文字列を double に変換する
...
より一般的には、CPAN にある L<FreezeThaw>, L<Storable> といった
モジュールをあたってみるべきでしょう。
Perl 5.8 から、L<Storable> は標準配布の一部となりました。
以下に L<Storable> の C<store> と C<retrieve> を使った例を挙げます:
...
CPAN にある L<Data::Dumper> モジュール(5.005 以降では Perl のリリースに
含まれています)はデータ構造を出力するのに向いています。
CPAN にある L<Storable> モジュール(5.8 以降では Perl のリリースに
...
CPAN から L<Business::CreditCard> モジュールを入手してください。
...
CPAN にある L<PGPLOT> モジュールにある arrays.h/arrays.c というものが
それをします。
倍精度実数や単精度実数を大量に扱うのであれば、CPAN にある
L<PDL> モジュールを使うことを考えてみるとよいでしょう--これは
number-crunching を簡単にしてくれます。
