...
GD::Barcode::ITF - GDでITF(Interleaved2of5)バーコード・イメージを作成します
...
I<エラーチェック付き>
  ...
  die $GD::Barcode::ITF::errStr unless($oGdBar);    #文字が不正
  $oGdBar->plot->png;


=head1 説明

GD::Barcode::ITFはGD::Barcodeのサブクラスで、
GDでITFバーコードイメージを作成することを可能にします。
...
コンストラクタ。
I<$sTxt>のためのGD::Barcode::ITFのオブジェクトを作成します。
I<$sTxt>は数字([0-9])です。
