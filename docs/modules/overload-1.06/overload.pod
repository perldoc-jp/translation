...
伝統的に Perl 関数 L<int> は 0 に丸めるので、浮動小数点風の型は同じ
意味論に従うべきです。
...
キー C<"~~"> は switch 構文で使われるスマートマッチングを
オーバーロードすることを可能にします。
L<feature> を参照してください。
...
与えます。
もしリファレンスのアドレスを得るためにこれを使っている
(2 つのリファレンスが同じものを指しているかをチェックするのに便利です)なら、
より速い C<Scalar::Util::refaddr()> を使った方が良いでしょう。
