...
送信されるキーストロークは KEYS で指定します。
いくつかの特別な意味を持つ文字があります。
これにより制御コードや修飾子も送信できます:
...
中かっこは特殊文字をクォートするために使われます(SendKeys("{+}{{}") は
は '+' と '{' を送信します)。
またそれらを使って名前が付けられたアクションを使うことができます:
...
または、VK コードとして扱われる数値を指定します。
一桁の数字は文字として扱われるので、その前に '0' を付けてください。
...
C<$keys> の中の Unicode 文字は、ALT+NUMPAD キーストロークに変換されます。
全てのアプリケーションが Unicode 入力を理解できるわけではないことに
注意してください。
...
この関数はマウス入力をエミュレートします。
COMMAND パラメータは一つあるいはそれ以上の文字列が入った文字列です:
...
ウィンドウが指定されたスタイルを持っているかどうかを判定します。
さらなる詳細についてはサンプルスクリプトを参照してください。
...
ウィンドウが指定された拡張スタイルを持っているかどうかを判定します。
さらなる詳細についてはサンプルスクリプトを参照してください。
...
対応するライブラリ関数を使って(MSDN 参照) MenuID 番号を返します。
...
$curr は MenuId で、$menu は
ホットキーを含むメニューの(ローカル化された!)名前です: "Rep&eate"
メニューアイテムのインデックスを返します (見つからない場合は -1)。
...
(2007 年) 現在、Perl の Unicode 入力に関する合意はないので、
このモジュールは、情報を Windows に問い合わせるときに A と W のどちらの
システムコールを使うかを設定する C<UnicodeSemantics> 関数を宣言します。
この違いに対応し、C<UnicodeSemantics> に設定された値に依存して
異なった結果を返す関数は:
...
C<GetWindowText> およびその呼び出し元 - FindWindowLike, WaitWindow,
WaitWindowLike
...
真偽値引数が設定されると、A と W のどちらかのシステムコールの結果を
関数の意味論フラグを変更します。
引数が設定されないと、現在のフラグの値を返します。
...
Jarek Jurasz (jurasz@imb.uni-karlsruhe.de), http://www.uni-karlsruhe.de/~gm07 は
DibSect とその他の一部分を書きました (詳しくは C<Changes> 参照)。
...
Dennis K. Paulsen (ctrondlp@cpan.org) はさまざまな部分を書きました
(詳しくは C<Changes> 参照)。
...
Dmitry Karasik (dmitry@karasik.eu.org) は Unicode と
cygwin/mingw 対応を追加しました。
