...
これは Emacs バックアップファイル (ファイル名がチルダ '~' で終わるもの) を
ファイル一覧から取り除きます。
...
使いましょう;
B<DBD::Informix> のものは、問題を回避するのに非常に成功しているので、
一見の価値があります。
...
B<DBD::Driver> パッケージの C<driver()> メソッドであって、
サブパッケージ B<DBD::Driver::dr>, B<DBD::Driver::db>,
B<DBD::Driver::db> のものではないことに注意してください。
...
同じ引数を取ります。
I<Name> 属性は B<DBI> 標準属性です
(L<DBI/Database Handle Attributes> を参照してください)。
...
再び、スカラコンテキストでは、外側のハンドルのみが返されます。
このように I<Statement> 属性はキャッシュされるべきです。
...
この文書ですでに言及しているように、B<DBI> は既知のドライバの接頭辞の
レジストリを持っていて、登録されていない接頭辞を持つ
不明な属性について、将来警告するようになるかもしれません。
...
注意してください。
図示された方法は、信頼性があまりありません;
SQL 分の一部としてクォートされた文字列、区切り文字、SQL コメントの中に
疑問符が現れると混乱する可能性があります。
...
この文書の他の場所に記述されている、C<type_info_all()> と
C<get_info()> メソッドも必要でしょう。
...
最も難しいメソッドです。
これに加えて、正しく動作するために継承された B<DBI> メソッドに
設定しなければならない多くの文属性があります
(後述する L</Statement attributes> を参照してください)。
...
メタデータの作成が利用可能です。
これらは、C<install_method()> を使ってインストールできるように、
ドライバが登録した接頭辞で始まる名前を持たなければならないことを
忘れないでください。
...
テストの一部はトランザクションログのないデータベースのためのものです;
残りはトランザクションログのあるデータベースのためのものです;
サーバはブログ、ストアードプロシージャ、ユーザー定義データ型に
対応しているものといないものがあります。
...
分けるというのは地獄です。
[1996 年に継承したバージョンは B<DBD::Oracle> のみに基づいていましたが、
B<DBD::Informix> を分解した経験に基づき、賛成します。]
...
インクルードされます。
I<DBISTATE_INIT> は、C<dbistate()> から呼び出されるために、
C<dbd_init()> への引数の名前が必要であることに注意してください。
...
初めてです。
できるだけ多くこれを使ってください; しかしトレースレベルが 3 未満の場合は
何も出力しません。
レベル 1 と 2 は B<DBI> のために予約されています。
...
一般的に、接続が失敗すると、ハンドルが個別に破壊する必要が内容に、
全ての割り当てられたリソースが解放されるようにするのが最良です。
...
典型的な、単純な可能性は、何もせずにハンドルで I<Statement> 属性を
設定した perl の C<prepare()> コードに依存することです。
...
従って、次のように、関数の先頭付近にコードが必要かもしれません:
...
示したいという場合、そして B<DBI> はハンドルが破壊されたときに
コードを呼ぶことができます。
