...
C<URI> オブジェクトは、mms スキームのうち、ユーザー情報と
...
news スキームに所属する C<URI> オブジェクトは共通、汎用、
サーバメソッドをサポートします。
...
C<URI> オブジェクトは、rtsp スキームのうち、ユーザー情報と
クエリ関連の副コマンドを除く共通、汎用、サーバメソッドに対応します。
...
C<URI> オブジェクトは、sip スキームのうち、パス関連の
...
I<news> スキームを参照してください。
その文法は news と同じですが、デフォルトポートは違います。
