...
Net::IRC::Connection - IRCコネクションのオブジェクト指向インターフェース
...
Net::IRC::Connectionはそのインスタンスが一つのIRCサーバに対する個々のコ
ネクションを表すクラスを定義します．複数のNet::IRC::Connectionオブジェ
クトは一つのNet::IRCオブジェクトで同時に扱うことができます．
