...
多くの UNIX のユーティリティとは違って、Perl はデータの大きさに恣意的な制限を
設けません -- つまり Perl では、メモリさえ十分にあれば、ファイルを丸ごと
一つの文字列に入れてしまうこともできます。
再帰の深さにも制限がありません。
...
L<perlembed>, L<perlxstut>, L<perlxs>, L<perlxstypemap>,
L<perlcall>, L<perlguts>, L<xsubpp> に記述しています。
...
L<perlobj> と L<perlootut> に記述しています。
