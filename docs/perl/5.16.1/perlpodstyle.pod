...
B<注意: ごく稀にだけ使ってください。>
強調のために太字を I<使わない> でください;
それは斜体を使うところです。
太字は B<NOTE:> や B<WARNING:> のような注意に制限してください。
しかし、プログラムの引数とオプション -- ただし名前は I<含みません>! -- は
B<-f> コマンドラインオプションを C<-f> ファイルテスト演算子を区別するために
(BE<lt>E<gt> を使って)太字で書かれます。
...
I<これ> のように、文章を強調するには斜体を使ってください。
関数名は伝統的に斜体(IE<lt>E<gt>)で表記されます;
関数を function() として記述した場合、Pod::Man がこの処理を行います。
文書化しているプログラム名を含むプログラム名は、
通常のローマ文字テキストの中にあっても、
慣習的に (IE<lt>E<gt> を使って) 斜体で書かれます。
...
リテラルコードは CE<lt>E<gt> で記述されます。
しかし、メタ構文的なプレースホルダは、
CE<lt>IE<lt>E<gt>E<gt> のようにさらに斜体を重ねます。
CE<lt>accept(IE<lt>NEWSOCKETE<gt>, E<lt>GENERICSOCKETE<gt>)E<gt> は
C<accept(I<NEWSOCKET>, I<GENERICSOCKET>)> のようにレンダリングされます。
...
相対または絶対のファイル名は、FE<lt>E<gt> マークアップで指定します。
これはイタリックでレンダリングされますが、追加の意味を持ちます。
...
他のマニュアルページへの参照は "manpage(section)" または
"C<LE<lt>manpage(section)E<gt>" の形式で記述すると、
Pod::Man が自動的に適切に書式設定します。
両方とも I<manpage>(section) としてレンダリングします。
2 番目の形式は LE<lt>E<gt> で、POD フォーマッタが可能であれば
マニュアルページへのリンクを作成するように要求するために使用されます。
例外として、モジュール文書を参照する場合、
通常はセクションを省略します; デフォルトではセクション 3 ですが、
全てのシステムがここに置くわけではないからです。
代わりにモジュール参照には C<LE<lt>Module::NameE<gt> を使えますが、
これはオプションです;
トランスレータは、$foo のような変数参照で行われるのと同様、
pod 中のモジュール参照を認識すると想定されるからです。
...
乱雑にならないように注意してください。
man ページ参照として提供されていない他のプログラムへの参照は、
IE<lt>E<gt> によってイタリックで囲む必要があります。
...
トランスレータは全ての大文字をスモールキャピタルに変換することが
想定されます。
通常は大文字と小文字が混在しています。
...
この行には、CE<lt>E<gt> や IE<lt>E<gt> などのマークアップを
使用しないでください。
関数は C<()> などで修飾しないでください。
...
使用しないようにしてください。
ここと L<Pod::Man> で説明されているように、Perl の変数、モジュール名、
関数名、man ページ
参照などをマークアップで装飾されないままにしておくことは安全であり、
POD トランスレータがそれを全て理解してくれます;
...
あなたの文書が正しく見えるかどうかをチェックするには、次を実行します:
...
I<groff> をインストールしていれば、次のようにしてより良い見た目を得られます:
...
全てがうまくいっているかを見るために結果の Postscript ファイルを見てください。
