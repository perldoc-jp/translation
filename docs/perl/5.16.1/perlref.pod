...
原則のオーバーライドが一つあるだけです: Perl はリファレンス(referencing)や
デリファレンス(defreferencing)を暗黙に行うことはありません。
...
perl のバージョン 5.001 から、中かっこに囲まれたシンボリックリファレンスは
ちょうどそれが文字列の中にあるかのように、識別子や変数名を式の残りから
単純に分離するという働きができました。
例えば、
...
5.001 では、囲まれるダブルクォートがなくても同じように動作するように
一般化されたので
