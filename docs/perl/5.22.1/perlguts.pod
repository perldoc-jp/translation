...
これは、例によって最もよく示されます。
通常、copy-on-write は、オペレータからの置換がこのハックを使用するのを
防ぎますが、copy-on-writeが不可能な文字列を作成できる場合は、
それが機能していることを確認できます。
現在の実装では、文字列バッファの最後のバイトがcopy-on-write
参照カウントとして使用されます。
バッファが十分に大きくない場合、copy-on-writeはスキップされます。
まず、空の文字列を見てみましょう。
(TBR)
...
最後に、C<LOGOP>、つまり論理OPがあります。
C<LISTOP>と同様に、これには1つ以上の子がありますが、
C<op_last>フィールドはありません。
したがって、最後の子を見つけるには、C<op_first>に続いて
C<OpSIBLING>チェーン自体をたどる必要があります。
代わりに、C<op_other>フィールドがあります。
これは、後述のC<op_next>フィールドに相当し、代替実行パスを表します。
C<and>、C<or>、C<?>などの演算子はC<LOGOP>です。
一般に、C<op_other>はC<LOGOP>の直接の子を指すことはできないことに
注意してください。
(TBR)
...
バージョン5.21.2から、実験的な定義C<-DPERL_OP_PARENT>を使用して構築された
perlsは、各opに追加のブールフラグC<op_moresib>を追加します。
設定されていない場合は、これがC<OpSIBLING>チェーン内の最後のopで
あることを示します。
これにより、最後のsiblingのC<op_sibling>フィールドが解放され、
親opをポイントします。
この構築では、そのフィールドもC<op_sibparent>に名前が変更され、
ジョイントロールが反映されます。
マクロC<OpSIBLING(o)>は、この特別な動作をラップし、最後のsiblingで
常にNULLを戻します。
この構築では、C<op_parent(o)>関数を使用して、任意のopの親を検索できます。
したがって、下位互換性のためには、C<op_sibling>に直接アクセスするのではなく、
常にC<OpSIBLING(o)>マクロを使用する必要があります。
(TBR)
...
(EBCDICプラットフォームでは、Perlは代わりにUTF-EBCDICを使用します。
これは、EBCDICプラットフォームに適合したUTF-8の形式です。
以下では、UTF-8について説明します。
UTF-EBCDICはUTF-8に似ていますが、詳細は異なります。
マクロは違いを隠します。
以下に示す特定の数値とビットパターンは、UTF-EBCDICでは異なることに
注意してください。)
(TBR)
...
(上記の例では、不変文字をテストする必要がないことに注意してください。
この関数は、整形式のUTF-8入力に対して動作します。
単に、不要な場合に関数のオーバーヘッドを回避する方が速いだけです。
)
(TBR)
...
完全な汎用性のためには、L<perlapi/DO_UTF8>マクロを使用して、SVの文字列が
UTF-8としてI<処理>されるかどうかを確認します。
これは、XS関数の呼び出しがL<S<C<use bytes>>|bytes>のスコープ内から
行われている場合を考慮します。
その場合、UTF-8文字列を構成する基礎となるバイトが、それらが表す
文字ではなく、公開されます。
しかし、このプラグマは、実際にはデバッグと、おそらくバイトレベルでの
低レベルのテストにのみ使用されるべきです。
したがって、ほとんどのXSコードはこれに関与する必要はありませんが、
perlコアのさまざまな領域がこれをサポートする必要があります。
(TBR)
...
これだけではありません。
Perl v5.12以降、UTF-8でエンコードされていない文字列も、さまざまな条件下で
Unicodeとして扱われるようになりました
(L<perlunicode/ASCII Rules versus Unicode Rules>を参照)。
これは実際には、序数が128から255の間にある文字に対してのみ問題になります。
また、ASCIIルールとUnicodeルールでは、文字の動作がコードで考慮する
方法によって異なります(L<perlunicode/The "Unicode Bug">を参照)。
これに対処するための公開されたAPIはなく、変更される可能性がありますが、
F<pp.c>のC<pp_lc>のコードを参照して、現在どのように行われているかを
例として確認できます。
(TBR)
...
L<perlapi/sv_cmp> と L<perlapi/sv_cmp_flags> は、二つの SV の文字比較を行い、
UTF-8 の性質を適切に処理します。
ただし、Unicode では、L<Unicode::Collate> モジュールを介して使用できる、
より高度な照合メカニズムが指定されていることに注意してください。
(TBR)
