...
また、L<perlport> を注意深く研究して、オペレーティングシステム、
文字集合、ファイルシステムなどに関する誤った仮定を避けるようにしてください。
...
C<'A'> は安全に言うことはできますが、C<0x41> はできません。
C<'\n'> は安全に言うことはできますが、C<\012> はできません。
However, you can use
macros defined in F<utf8.h> to specify any code point portably.
C<LATIN1_TO_NATIVE(0xDF)> is going to be the code point that means
LATIN SMALL LETTER SHARP S on whatever platform you are running on (on
...
C でのニーモニック名がない文字の文字列表現が必要な場合は、
それを F<regen/unicode_constants.pl> の一覧に追加すると、
Perl は現在のプラットフォームを基にした C<#define> を作成します。
...
(Note that special handling of ranges in
regular expression patterns makes it appear to Perl
code that the aforementioned ranges are all unbroken.)
(TBT)
...
F<utf8.h> と F<utfebcdic.h> で同じ名前(ただし定義が異なる)を持つマクロは、
呼び出し側のコードにそのようなエンコーディングが一つしかないと
認識させるために使われます。
これはほとんど常に C<utf8> と呼ばれていますが、EBCDIC 版も意味します。
繰り返しますが、コード内のコメントは、コード自体が正しい場合でも
間違っている可能性があります。
たとえば、UTF-8 の C<invariant characters> の概念は
ASCII と EBCDIC で異なります。
...
glibc の C<printf()>, C<sprintf()> などは、glibc バージョン 2.17 より前は
バグがあることに注意してください。
プログラムの現在の基となるロケールが UTF-8 の場合、
正当な UTF-8 でない文字列を作るための、精度付きの
C<%.s> 形式は許されません。
何が起きるかというと、C<%s> およびそのオペランドは何の注意もなく
単に読み飛ばされます。
L<https://sourceware.org/bugzilla/show_bug.cgi?id=6530>.
...
前述の例において、C<Perl_pp_add> の探し方が分かりましたが、
あちこちでこれを複数回呼び出していたり、
探しているのがどの op か分からない場合は?
...
このための一つの方法は、探しているものの近くのどこかに珍しい呼び出しを
挿入することです。
例えば、メソッドの前に C<study> を追加します:
...
それから、探しているものに当たるまでステップ実行します。
これは、ループ中で特定の条件のときにだけブレークしたいときに使えます:
...
コードをパース/構文解析するときに perl が何をしているのかを見たい場合、
C<BEGIN {}> を使えます:
...
C<if> ブロックのようなものの中でパーサ/構文解析器が何をしているのかを見たい場合、
もう少しトリッキーなことをする必要があります:
...
ファイルに記録され、デフォルトでは出力はインラインで表示されます。
...
C99 より前のコンパイラでは、C<bool> は C<char> の等価物として
定義されています。.
従って、それよりも大きな型を C<bool> に代入するのは安全ではなく、
切り詰められることがあります。
C<cBOOL> マクロは、これを正しくキャストするために存在します。
...
On those platforms and compilers where 
C<bool> が本当のブール値であるプラットフォームとコンパイラ (C++, C99) の場合、
キャストを忘れるのは簡単です。
C<-Accflags=-DPERL_BOOL_AS_CHAR> 付きでコンパイルすることで、
C<bool> を C<char> 強制できます。
また、次のようなもの で C<Configure> を実行するか:
...
コンパイラで等価なことをすることで、安全でない切り詰めを明らかにするのが
簡単になります。
