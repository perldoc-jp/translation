...
(これは S<C<use v5.12>> 以上を使うと自動的に選択されます。)
...
これと同様のことを行うには S<C<pack('U0', ...)>> と
S<C<pack('C0', ...)>> を参照してください)。
...
非正準分解型の文字にマッチングします。
正準分解は前述の L</Extended Grapheme Clusters (Logical characters)> 節で
説明しました。
しかし、多くの文字は異なる種類の分解を持ち、
一般的に「互換」分解あるいは「非正準」分解と呼ばれます。
これらの分解を形成する並びは合成済文字への正準等価ではないと考えられます。
例えば、C<"SUPERSCRIPT ONE"> です。
これは普通の数字 1 のようなものですが、正確ではありません;
これの数字 1 への分解は
「互換」分解と呼ばれ、特に「スーパー」("superscript" から)分解と呼ばれます。
このような互換分解(L<https://www.unicode.org/reports/tr44>を参照)は
いくつかあります。
S<C<\p{Dt: Non_Canon}>> は、これら全ての和集合を一つの名前で参照するために
使う Perl 拡張です。
...
(非正準分解の一つは "compat" という名前で、おそらく
"miscellaneous" という名前の方がよかったものです。
これは、Unicode がよりよい名前を見つけられなかったものを
含んでいます。)
...
あります。
これらは L<perlrecharclass/POSIX Character Classes> に記述されています。
...
(正規表現集合機能 L<perlre/(?[ ])> はより複雑な定義を可能にする選択肢を
提供します。)
...
サブルーチンはフラグの値に依存して異なった値を返すことがあります。
しかし、サブルーチンは各フラグ値(0 対 0以外)に対して 2 回以上
呼び出されることはありません。
返り値は保存され、再びサブルーチンを呼び出す代わりに使われます。
パターンのコンパイル時にサブルーチンが定義されている場合は、
その時点で呼び出されます;
さもなければ、実行中に(そのフラグの)値が初めて
必要になったときに呼び出されます。
...
UTF-8 データの場合は、バイト 0xC0 および 0xC1 を番兵文字として使用できます;
これらは整形式の UTF-8 では決して現れないからです。
(UTF-EBCDIC にも同等のものがあります)。
Unicode 符号位置を整数変数に格納し、負の値を番兵として
使用することもできます。
...
「コマンドラインインターフェース」(そしてどちらの?)は Unicode をどのように
扱うのでしょうか?
...
使うのが最良です。)
S<C<use v5.12>> 以上を使うと、C<unicode_strings> は自動的に選択されることに
注意してください。
