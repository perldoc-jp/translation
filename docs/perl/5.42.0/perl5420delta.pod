...
(グローバル名の区切り文字としてのアポストロフィは無効に出来るようになりました)
...
(Switch とスマートマッチング演算子は、機能として維持されます)
...
([CVE-2024-56406] tr// のヒープバッファオーバーフロー脆弱性)
...
([CVE-2025-40909] ファイル操作が意図しないパスをターゲットにした場合に Perl スレッドに作業ディレクトリの競合状態がある)
...
(eval なしの関数のための含まれる関数への参照が削除されました)
