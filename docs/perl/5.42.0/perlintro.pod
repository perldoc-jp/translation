...
前述のおかしな文字列 <$name> に気付いたかもしれません。
これは変数を意味し、一般的には "name" と発音します。
ドル記号は「印」(sigil) と呼ばれ、"name" が変数であることを示します。
変数が典型的には英字と数字からなる単なる文字列であるその他の多くの
コンピュータ言語と異なり、Perl の全ての変数には前に印が付きます。
...
Perl は主な変数型が三つあります: スカラ、配列、ハッシュです。
変数名に使われた特定の印がその変数の型を示します。
私たちは既に C<$name> で印として使われているドル記号を見ています。
ドル記号は変数がスカラであることを示します。
...
Perl では、スカラは常に C<$> 印で参照されます。
従って、先程、C<@animals> 配列の中にある 0 番目のスカラ変数を得るために
C<$animals[0]> と書きました。
大かっこ C<[...]> は、これが私たちが値を得ようとしている元が
配列であることを示しています。
これは Perl の初心者の大きな混乱点です。
単一の値がほしい時は、例え値がより大きな集合の一部であっても、
C<$> を使う必要があります。
...
これは「配列スライス」(array slice) と呼ばれます。
私たちは複数の値がほしいので、ここでは C<@> 印を使います;
C<$> は単一の値を参照するために予約されています。
...
再び、単一のハッシュ要素にアクセスするために C<$> 印を使うことに
注意してください。
中かっこ C<{...}> は、私たちが値を得ようとしている元が
ハッシュであることを示しています。
...
再び、複数の値では C<@> を使い、単一の値では C<$> を使います。
...
C<$name>, C<@name>, C<%name> は三つの異なる変数であることに
注意してください。
異なる変数を表現するために同じ英数字並びを使うのは混乱させる
かもしれませんが、状況によっては明確化するにつながることもあるでしょう;
例えば、配列 C<@name> の中の、今注目している特定の要素を表現するのに
C<$name> を使うなどです。
名付けの選択はあなた次第です; Perl インタプリタは気にしません。
複数の変数が同じ名前を持っている場合、大かっこや中かっこを使うことで
どちらを意味しているかをインタプリタに伝えます。
...
前述の C<$> はドル記号にマッチングしないことに注意してください。
同様に、C<.>, C<\>, C<[>, C<]>, C<(>, C<)>, C<^> は、
あなたが想定している文字とマッチングしません。
これらは「メタ文字」(metacharacters) と呼ばれます。
一方、例えば C<a>, C<e>, C<i>, C<o>,  C<u> のような文字は
メタ文字ではありません。
これらはリテラルにその文字自身にマッチングします。
メタ文字は通常そのリテラルな値ではない何かにマッチングします。
さらにいくつかのメタ文字は前述しています。
一部の量指定子は後述し、完全な一覧は
L<perlre/Metacharacters> にあります。
...
メタ文字がそのリテラルな値にマッチングするように、
逆スラッシュを前置することで「エスケープ」(escape)
(あるいは「クォート」(quote))します。
従って、C<\$> はドル記号にマッチングし、
C<\\> はリテラルな逆スラッシュにマッチングします。
...
また、例えば文字列 C<\s> は、逆スラッシュと文字 C<s> に
マッチングしないことに注意してください。
この場合、非メタ文字 C<s> に逆スラッシュを前置すると、
リテラルな値にマッチングしない何かに変わりますう。
このような並びは「エスケープシーケンス」(escape sequence) と呼ばれます。
L<perlrebackslash> には現在あるものの全てを文書化しています。
...
メタ文字ではなく、現在定義されているエスケープシーケンスの一部でもない
文字をエスケープすると警告が発生します。
...
マッチングしたいモノがいくつあるのかを、量指定子(quantifier)を使うことで
指定できます (ここで「モノ」とは、リテラルな文字、前述した構造の一つ、
かっこ内にあるこれらのどれかのグループ、の一つです)。
