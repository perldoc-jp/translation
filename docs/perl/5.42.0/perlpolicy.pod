...
彼は Perl というプログラミング言語のあらゆる部分について、変える、
変えないの最終決定権を持っています。
最近は、Larry はほとんどの時間を Raku に費やしており、
Perl 5 は、各リリースに何を含めるかを決定し、リリースが定期的に
リリースされることを保証する責任を持つ、porters の運営委員会が
率いています。
...
正しいことになっています。
つまり、コア機能については Larry が最終的な拒否権を持っています。
...
わかりましたね?
Larry は常に正しいのです; たとえ過去の彼が間違っていたとしても。
...
Perl コアに加えられる変更は全て寄付によるものです。
この寄付は、典型的にはコミュニティに属する個人がコードや時間を
提供するという形で行われています。
ときどき、企業、団体が特定個人ないしプロジェクトに出資するという
形をとることもあります。
...
ボランティアの集団として、私たちの活動は、
Perl に貢献しなければならない義務があるわけではない
個人の厚意や努力に大きく依存しています。
...
もっとも、私たちも Perl の安定性や安全性は大切に考えているので、
昔から、より広い意味での Perl コミュニティに属するみなさんとの
暗黙の了解事項として、
過去にリリースされた Perl のサポートや保守もしてきました。
...
この文書は、Perl コミュニティが Perl の開発陣に期待できるサポートや
保守の約束を成文化するためのものです:
...
可能ならばそのようなパッチを関係する -maint ブランチにもあてる努力はしますが、
バージョンをあげてリリースをしたり「公式」パッチを提供する選択を
行うかどうかはわかりません。
...
つらい停滞期にもつながりかねません。
しかし、失敗をなかったことにするのはとてもとても難しいことです。
わざわざユーザを傷つけるようなことをせずに行うのはほとんど不可能です。
...
歴史的に、私たちは後方互換性よりもはるかに高い基準 -- 後方バグ互換性 -- を
守ってきました。
...
非常に大切なことです。
エンドユーザ側の開発者がバグを一機能として認めてしまったのであれば、
私たちもそれをそのように扱う必要があるのです。
...
認めるものです。
Perl 開発者が遵守すべき規範を文書化しようという試みです。
Perl 開発者同士尊重しあおうという大まかなガイドラインを書き下ろす試みです。
...
Perl コアとともに配布すべきだとされることもあります。
これは、そのモジュールの作者が明示的に同意し、
そのモジュールが Perl 自身と同じライセンスで配布されることになることも明確に
認識していない限り、すべきではありません。
...
Perl コアに入っているバージョンもまだ原作者の作品であるとみなすべきです。
...
Perl の文書はユーザにとって重要なリソースです。
Perl の文書が無理のない範囲で首尾一貫していることや、
現在の実装を正確に反映していることは本当に大切なことです。
...
プログラムを用いる場合もあります。
それよりも、「重要な」部分のみを含むコード片を例とする方が多いです。
何がどう「重要」かはコード片によって異なります。
C<use strict> や C<use warnings> を宣言し、すべての変数を初期化して、
あらゆる例外を完璧に捕捉することが重要な場合もあります。
しかし、たいていの場合、これらは例示しようとしているレッスンの内容を
わかりにくくしてしまいます。
...
多々あります。
アメリカ式/イギリス式/その他の綴り方の選択はその文書の著者に任されています。
