...
C<head5> と C<head6> は 2020 年に導入され、2020 年 10 月にリリースされた
L<Pod::Simple> 3.41 で対応されたので、あなたがお使いの Pod パーサは
対応していないかもしれないことに注意してください。
...
( 1) 点にも数値にも見えないようなもの、2) そのように見えるもの、の両方を
含んでいるリストの場合、
C<ZE<lt>E<gt>> で点または数値のように見えるものを前置するべきです。
例については後述する
L<ZE<lt>E<gt>|/ZE<lt>E<gt> -- a null (zero-effect) formatting code> を
参照してください。)
...
これは 2024 年に新しく追加されたもので、全てのプロセッサや
出力フォーマットで利用できないかもしれません。
さらに、一部の出力フォーマットでは、斜体と下線のフォーマットを同時に
出力できないので、例え両方とも認識していても、見た目としてそれらを
区別できないかもしれないことに注意してください。
...
もう一つの使用法は、C<=item ZE<lt>E<gt>I<stuff...>> の
I<stuff> が点や数字と認識されないようにするためです。
例えば、C<ZE<lt>E<gt>> なしの場合、次のような行
...
もう一つの使用法は、C<=item> 行の間の目に見えるスペースを保守することです。
次のように指定すると:
...
それを成し遂げるために C<ZE<lt>E<gt>> を使えます:
