...
構文チェックが成功すると、終了コード 0 で終了し、
C<I<yourprogram> syntax OK> と返します。
失敗時は、perl は検出したエラーを表示し、非 0 終了コードで終了します。
...
デバッガに示すために環境変数 C<PERL5DB_THREADED=1> が設定されます。
...
L<-e|/-e commandline> と同様に振る舞いますが、暗黙に全てのオプション機能と
組み込み関数を(main コンパイル単位で)有効にします。
L<feature> と L<builtin> を参照してください。
...
B<-m>I<module> はプログラムの実行前に S<C<use I<module> ();>> を
実行します。
...
B<-M>I<module> はプログラムの実行前に S<C<use I<module>;>> を実行します。
...
Perl バージョン 5.39.8 から、C<-M> オプションとモジュール名を
別のコマンドライン引数として渡すことができます:
...
整数が設定されると、この値は、
明示的な C<srand()> 呼び出しがなかったり、引数なしでの明示的な
C<srand()> 呼び出しが行われたとき、
C<rand()> のための perl 内部の乱数生成器の種として使われます。
...
通常、C<srand()> の前に C<rand()> を呼び出したり、引数なしで明示的に
C<srand()> を呼び出した場合、
比較的高品質な乱数の種によって生成器の状態に
種を設定するために、乱数生成器は「ベストエフォート」を行います。
この環境変数が設定されると、使われる種は、
アプリケーションプロセスとそのフォークやスレッドがそれぞれ固有の
ユニークな種を持ち続けれるけれども、
プログラムが 2 回実行されても、C<rand()> に関しては (他の全てが同じなら)
同じ結果になるような方法で、決定論的に計算されます。
...
PERL_RAND_SEED は性能計測とデバッグを意図したもので、
安定したテストを意図したものでは明示的に異なります。
保証されることは、特定の perl バイナリが 2 回同じ結果を
出力することだけであり、
perl リリースが異なったりアーキテクチャがことなったりしたときの
結果は保証しません。
...
perl が setuid または setgid で実行されると、これは無視されます。
