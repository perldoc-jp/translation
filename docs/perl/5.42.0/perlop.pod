...
右側は、C<&> 印を前置したサブルーチン名も取れます。
これはレキシカルメソッド起動のように見えるものを作ります;
ここでメソッドサブルーチンはオブジェクトクラスのパッケージの中を
検索することによる名前ではなく、レキシカルに解決されます。
この解決は完全にコンパイル時に行われ、実行時には通常のサブルーチン呼び出しと
同様に実行されます。
...
X<cmp>
(L<前述|/Relational Operators> の関係演算子と同様です: C<gt> と C<< > >>、
C<lt> と C<< < >> など。)
...
C<スマートマッチング> (smartmatch) 機能は、新しいコードでは非推奨で、
後方互換性のために残されています。
...
5.10.0 で導入されたスマートマッチング演算子は、5.10.1 で
大きく変更されました。
これはデフォルトおよび、5.40 までの全ての機能の束で有効です。
これより後の機能の束でスマートマッチングを使うには、
明示的に有効にする必要があります:
...
C<&&> は (0 か 1 を返す C の C<&&> と異なり) 最後に評価して値を返します。
...
Perl では、制御フローに使う場合の C<&&> の代替策として、
C<and> 演算子が用意されています (L<後述|/Logical And> を参照)。
短絡の動作は全く同じです。
しかし、C<"and"> の優先順はかなり低くしてあるので、
リスト演算子の後に使う場合にも、かっこなしで安全に使うことができます。
...
Perl では、フロー制御に使う場合の C<||> の代替策として、
C<or> 演算子が用意されています (L<後述|/Logical or and Exclusive Or> を参照)。
短絡の動作は全く同じです。
しかし、C<"or"> の優先順位はかなり低くしてあるので、
リスト演算子の後に使う場合にも、かっこなしで安全に使うことができます:
...
この場合、終端子は識別子で、単語 "WHIMPER" です。
最も一般的には、単に目立たせるために、識別子を大文字にしますが、
これは必要ではなく、単なる慣習です。
...
後に詳述するように、終端子はクォートで囲むことができますが、
それなしの場合、ヒヤドキュメントのテキストは、ダブルクォートが使われたときと
全く同じように振る舞います。
...
この節は L</C-style Logical Or, Xor, and Defined Or> で置き換えられました。
この見出しは、既に存在している外部からのリンクが壊れるのを防ぐためだけに
残っています。
...
この節は L</Simpler Quote-Like Operators> で置き換えられました。
