...
I<後方参照> (backreferences) C<\1>, C<\2> …です。
...
よい理由は、以下の例で示すように、特定の文字列をマッチングするために単純な
パターンを使うところです:
...
最後の二つの修飾子 C<//g> と C<//c> は複数回マッチングに関連するものです。
...
マッチングに成功した場合には C<s///> は置換が行われた数を返し、失敗した
場合には偽を返します。
幾つか例を挙げましょう:
...
最後の例では、文字列全体がマッチングしていましたが、シングルクォートに
...
完全なリストは L<perluniprops> を参照してください。
...
構成する文字クラスの並びの略記です。
これは、一つの文字のように見えるけれども、内部では複数で表現される一つの
「論理文字」を表現します。
...
C<xdigit> があり、さらに二つの拡張である C<word> (\w にマッチングする
...
=end original
    ...
    /\s+[abc[:digit:]xyz]\s*/;  # a,b,c,x,y,z, または数字にマッチング
    /^=item\s[[:digit:]]/;      # スペースと数字が後続する '=item'
                                # にマッチング
    use charnames ":full";
    /\s+[abc\p{IsDigit}xyz]\s+/;  # a,b,c,x,y,z, または数字にマッチング
...
マッチングの失敗は捕捉バッファに保存されたり結果を生成したりしないので、
...
パターンは文字のサブセットにマッチングするクラスで始まります。
これがマッチングするときはいつでも、C<$count{'a'}++;> のような文が実行され、
文字のカウンタをインクリメントします。
それから C<(*FAIL)> がその名の通りのことを行い、
