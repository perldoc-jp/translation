...
B<注意:> perl 5.10.1 から、-C スイッチが #! 行で使われると、
...
perl が C<-Accflags=-DPERL_MEM_LOG> で設定されていると、環境変数
C<PERL_MEMLOG> を設定することでデバッグメッセージのログ出力を有効にします。
値は C<< <number>[m][s][t] >> の形式で、C<number> は書き込みたい
ファイル記述子番号、文字の組み合わせはメモリ (m) と SV(s) で、
オプションとしてタイプスタンプ (t) です。
例えば、C<PERL_MEMLOG=1mst> は全てのログ情報を標準出力に出力します。
