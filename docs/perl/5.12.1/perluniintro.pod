...
I<書記素クラスタ> (grapheme cluster) という概念を、それから
I<拡張書記素クラスタ> (extended grapheme cluster) を作りました。
...
1 つの「文字」は、1 つの Unicode の符号位置です。
...
B<Perl 5.8.0 から、C<use utf8> の使用は、遥かに限られた状況でのみ
必要になりました。>
...
データは(ほぼ)透過的に Unicode に昇格されます。
ここには問題があります -- L<perlunicode/The "Unicode Bug"> を
参照してください。
...
正規表現の文字クラス(C</[a-z]/>)内と、C<tr///>(C<y///> としても知られる)
...
気にする必要はないはずです。
しかし、今のところ範囲 128 から 256 の文字の意味論は、文字列が Unicode で
