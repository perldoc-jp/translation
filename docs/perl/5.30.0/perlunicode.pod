...
前述の説明は一部の詳細を省略しています;
UAX#24 "Unicode Script Property": L<http://www.unicode.org/reports/tr24> を
参照してください。
...
Perl はパターンを C<(?iaa: ...)> でラッピングします。
これは、ASCII 以外では、このリリースで利用可能な Unicode 特性値に
マッチングするものがなく、大文字と小文字を区別せずにマッチングする
必要があるためです。
パターンに構文エラーがある場合は、自分で指定していなくても、この
ラッピングがエラーメッセージに表示されます。
このことを知らないと混乱する可能性があります。
...
安全のために、前述の例では、C<Hebrew_Braile> のようなものに
...
([2] C<\p{...}> C<\P{...}>。この要求は最小限の特性の一覧に対するものです。Perl はこれらおよび、R2.7 によるその他全ての Unicode 文字特性に対応しています (上述の L</"Unicode Character Properties"> 参照)。)
...
(Perl は C<\d> C<\D> C<\s> C<\S> C<\w> C<\W> C<\X> C<[:I<prop>:]> C<[:^I<prop>:]> に加えて、L<http://www.unicode.org/reports/tr18/#Compatibility_Properties> で指定されている全ての特性を持ちます。これらは前述の L</Other Properties> に記されています。)
