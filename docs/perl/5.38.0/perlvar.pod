...
5.37.7 の時点で、C<@INC> フックが実行される時、フックを保持する
C<@INC> 配列のインデックスはC<$INC>変数にローカライズされます。
フックが整数を返す時、その値の継承はチェックされる C<@INC> の
次のインデックスを決定するために使用されます。
...
通常、C<@INC> 配列のトラバースは最初から最後まで(C<0.$#INC>) であり、
C<@INC> 配列がフックによって変更された場合、イテレータは新しく
追加されたエントリがスキップされる状態のままになる可能性があります。
この値を変更すると、C<@INC> フックが C<@INC> 配列を書き換え、
後でどこに進むかをPerlに指示することができます。
...
この変数は、許可される C<eval EXPR>/C<BEGIN> または C<require>/C<BEGIN>
ブロックのネストの最大数を決定します。
...
C<0> に設定すると、C<eval EXPR> または C<require EXPR> 内の
C<BEGIN> ブロックは完全に禁止され、コンパイルを終了する
例外をトリガします。
C<require> の場合は例外をスローし、C<eval> の場合は通常どおり
C<$@> にエラーを返します。
(TBR)
...
C<f()> の各呼び出しはかなりのCスタックを消費し、この変数は、C スタックを
使い果たして segfault を引き起こすのではなく、このようなコードを
死なせるために使用されます。
このようなコードが珍しいことは言うまでもありませんが、実際に
設定を上げる必要はありません。
...
その理由は、BEGIN ブロックが完了した直後に実行されるためです。
...
再開されますが、フックルーチン自体が C<goto &sub>、ループ終了、
...
C<$SIG{__DIE__}> は C<'IGNORE'> に対応していません;
...
存在することが文書化されていないキーを変更しようとしたり、コード参照や
undef 以外のものをハッシュに保存しようとしたりすると、Perl は
エラーをスローします。
フックを実装するためにオブジェクトを使用したい場合は、カリー化を
使用して、オブジェクトを匿名のコード参照に埋め込むことができます。
(TBR)
...
C<${^HOOK}{require__before}> で示されるルーチンは、C<require> B<before> に
よって呼び出され、C<%INC> をチェックし、C<@INC> を検索し、
INC フックを呼び出し、または任意のコードをコンパイルします。
このルーチンは単一の引数で呼び出され、アイテムのファイル名が
要求されます(パッケージ名はパスに変換されます)。
このファイル名を変更して、ロードされるファイルを変更できます。
実行中にフックが停止すると、require の実行がブロックされます。
(TBR)
...
coderef を返すことがあります。
この coderef は、require が正常に完了したか、例外がスローされたかに
かかわらず実行されます。
このコードは、要求されたファイル名で呼び出されます。
%INC をチェックして、require が成功したかどうかを判断できます。
C<require__before> フックからのその他の戻り値は、黙って無視されます。
(TBR)
...
C<require__before> フックは FIFO 順に呼び出され、フックがコード参照を
返す場合、それらのコード参照は FILO 順に呼び出されます。
つまり、A が B を必要とし、B が C を必要とする場合、C<require__before> が
最初に A に対して呼び出され、次に B, C の順に呼び出され、アクション後の
コード参照が最初に C に対して実行され、次に B、最後に A の順に実行されます。
(TBR)
...
以前にインストールされたフックが呼び出され、コード参照の場合は
...
C<require> B<after> によって呼び出されます。
このルーチンは単一の引数で呼び出され、アイテムのファイル名が
要求されます(パッケージ名はパスに変換されます)。
このルーチンは、C<require> が完了したときに、例外または
require ステートメントの完了によって実行されます。
...
C<require__after> フックは、必要なファイルごとに FILO の順序で
呼び出されます。
つまり、A が B を必要とし、B が C を必要とする場合、C<require__after> が
最初に C に対して呼び出され、次に B、次に A に対して呼び出されます。
(TBR)
...
正規表現変数を使用すると、プログラマは現在の動的スコープ内で最後に一致した
I<successful> regex の状態にアクセスできます。
(TBR)
...
変数自体はグローバルでスコープ指定されませんが、変数がアクセスする
データは動的スコープ指定変数と同様にスコープ指定されます。
つまり、一致が成功するたびに、グローバル状態オブジェクトが
現在のブロックまたはファイルスコープにローカライズされたかのように
動作します (動的スコープ指定と C<local> キーワードの詳細については、
...
I<successful match> には、C<m//> 演算子によって実行されたものだけでなく、
...
まったく同じスコープ内で一致が成功するたびに、以前の成功した一致の
一致コンテキストが上書きされますが、失敗した一致は
上書きされないことに注意してください。
また、内側のネストされたスコープでは、外側の動的スコープからの
以前の状態は、別の成功した一致によって上書きされるまで持続しますが、
内側のネストされたスコープが終了すると、内側の成功した一致が終了する
前に有効であった一致コンテキストが復元されます。
(TBR)
...
C<goto LABEL> が動的にスコープ指定された一致コンテキストとうまく
...
含まれる要素の数は、どの取得バッファが実際に一致したかに関係なく、
パターン内の取得バッファの数よりも1つ多くなります。
これを使用して、パターン内に取得バッファがいくつあるかを判断できます。
(C<@->とは対照的に、要素の数は少なくなります。)
(TBR)
...
含まれる要素の数は、実際に何かに一致した最も高いキャプチャバッファ
(サブグループとも呼ばれる)の数よりも1つ多くなります
(要素の数が少ないC<@+>とは対照的です)。
(TBR)
...
C<IO::Handle> クラスを読み込むのはコストの高い操作なので、
...
非推奨化の後、このマジックは Perl v5.10.0 で取り除かれ、警告を
引き起こします: C<$# is no longer supported>.
...
これを使うと警告を引き起こします: C<$* is no longer supported>。
