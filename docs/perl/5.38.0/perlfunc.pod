...
2 番目の引数を指定せずに C<bless> を使用すると、オブジェクトは
現在のパッケージ内に作成されます。
派生クラスが C<bless> を実行するメソッドを継承する可能性がある場合は、
2 番目の引数を常に指定する必要があります。
これはバグの原因となる可能性があるため、引数が一つの C<bless> は
推奨されません。
(TBR)
...
L<C<bless>|/bless REF,CLASSNAME> は、最初の引数である指定された
参照を関数の値として返します。
...
組み込み型とプラグマにのみ使用されるため、C<CLASSNAME>は
...
これにより、L<C<ref>|/ref EXPR> の結果を(誤って)チェックして、参照が
C<bless> されているかどうかを確認するコードが失敗する原因となります。
これは、偽の値である「0」が返されるためです。
(TBR)
...
実装する指定された名前空間にあるものとして宣言します。
これは、新しく作成されたパッケージがクラスとして動作することを
...
参照してください。
(L<C<open>|/open FILEHANDLE,MODE,EXPR> を入出力 I<両用> にパイプすることは
...
コマンドへの引数となります。
パイプモードではない L<C<open>|/open FILEHANDLE,MODE,EXPR> での
...
1 引数 または 2 引数の形の L<C<open>|/open FILEHANDLE,MODE,EXPR>) で
(C<-|> や C<|-> というふうに) C<-> というコマンドにパイプを開くと、
暗黙の L<C<fork>|/fork> が行なわれるので、
L<C<open>|/open FILEHANDLE,MODE,EXPR> は 2 回返ります;
...
L<C<open>|/open FILEHANDLE,MODE,EXPR> は安全です。
...
実際、C<bareword_filehandles> 機能が無効になっている場合は
ファイルハンドルに裸の単語を使うことはエラーです。
この機能は、C<use v5.36.0> 以降のスコープ内ではデフォルトでは
無効になっています。
...
1 引数 と 2 引数の形の L<C<open>|/open FILEHANDLE,MODE,EXPR> に渡された
...
L<C<open>|/open FILEHANDLE,MODE,EXPR> の I<magic> と I<3 引数> 形式を誠実に
選択するべきです。
...
L<C<open>|/open FILEHANDLE,MODE,EXPR> とは違うファイルモードを持ちます)。
...
確認しなければなりません。
あるいは、
L<C<isa> operator|perlop/"Class Instance Operator"> は、
...
フックを使用して C<@INC> 配列を書き直すこともできます。
これは奇妙に聞こえるかもしれませんが、これを行うことが非常に
有用な状況があります。
このようなフックは通常undefを返すだけで、フィルタリングと
C<@INC> 修正を混合しません。
古いバージョンの perl では、フック修正 C<@INC> を持つことは
問題をはらんでおり、segfaultやassert障害を引き起こす
可能性さえありましたが、5.37.7の時点で、ロジックは
はるかに堅牢になり、フックはループの反復を
制御できるようになりました。
(TBR)
...
require 中に実行される C<@INC> 配列トラバースのイテレータを
制御する機能が追加されました。
C<$INC>変数は、現在実行中のフックのインデックスで初期化されます。
フックが返されると、チェックされる C<@INC> の次のスロットは、
C<$INC> の値の整数の後継になります(undef の場合は -1 になります)。
たとえば、次のコードは
(TBR)
...
は C<@INC> に sub をインストールし、フックとして実行されると
(例えば、存在しないファイルのrequireによって)、フックは
C<@INC> から自分自身をスプライスし、誰かが C<@INC> 検索を
必要とする require 操作をしたときに警告し、すぐにそのフックを
実行する新しい sub を先頭に追加します。
(TBR)
...
フックをすぐに使用したり、イテレーターの左側にある C<@INC> の
変更された項目を検査したりする方法がなかったため、警告は
2 番目の呼び出しでのみ生成されました。
最近の perl では、C<$INC> を定義していない最後のステートメントが
存在すると、perl は C<@INC> 配列のトラバースを最初から再開し、
新しくインストールされた sub をすぐに実行します。
(TBR)
...
フックの存続期間中にC<$INC>に加えられた変更は、フックが終了した
後に展開され、その値はフックの実行直後にのみ意味を持ちます。
したがって、C<require> を実行する前にC<$INC>を何らかの値に
...
C<require()> をラップして新しいスタックフレームを注入すると、
しばしば問題が発生します。
それにもかかわらず、C<require> の前後にアクションを実行できる機能を
持つことは非常に便利です。
たとえば、C<Devel::TraceUse> のようなトレースユーティリティや、
ロード時間と require グラフのメモリ消費を測定する機能などです。
5.37.10年に C<require()> ラッパーを安全に作成することが
困難であったため、新しいメカニズムを導入しました。
(TBR)
...
5.37.10 では、他のアクションが実行される前に、C<require> は
C<${^HOOK}{require__before}> に coderef が含まれているかどうかをチェックし、
含まれている場合は、ロードされるアイテムのファイル名形式で呼び出されます。
フックは C<$_[0]> を変更して別のファイル名をロードすることも、致命的な
例外をスローして require を失敗させることもできます。
これは、要求されたコード自体が例外をスローしたかのように扱われます。
(TBR)
...
この場合、コード参照はrequireが完了した後に実行されます
(パラメータとしてfilnameを使用したevalで)。
コンパイルがどのように完了したかに関係なく、また、require が
致命的な例外をスローした場合でも実行されます。
関数はC<%INC>を参照して、require が失敗したかどうかを判断します。
たとえば、次のコードは、すべての C<require> ステートメントの
前後に診断を出力します。
この例には、複数のシグナルが連携できるようにシグナルを
チェーンするロジックも含まれています。
正常に動作する C<${^HOOK}{require__before}> ハンドラは、
...
C<INCDIR> フックとは異なり、すべての C<require> ステートメントに
対して実行され、require 内の他の特別な動作よりも先に実行されます。
C<${^HOOK}{require__before}> の最初のフックは、eval 内では
実行*されない*ことに注意してください。
例外をスローすると、その後の処理は停止しますが、返される可能性のある
after フックは eval 内で実行され、スローされた例外は静かに
無視されます。
...
require が完了した後に起動する同様のフックC<${^HOOK}{require__after}>が
あります。
これは、各 require ステートメントが完了した後に、例外または正常終了の
いずれかによって呼び出されます。
最後に実行された require ステートメントのファイル名を使用して呼び出されます。
...
ラップされたコードが終了した場合に STDOUT を復元します。
違いは、前者では、元の STDOUT は C<print> ステートメント
(C<print STDOUT ...>など)で明示的に使用することでアクセスできるのに対し、
...
負の整数で呼び出すと、 L<C<sleep>|/sleep EXPR> はスリープせず、
...
このバージョンの F<perl> では、C<sort> 関数はマージソートアルゴリズムで
実装されています。
...
プロセスの起動時に C<PERL_RAND_SEED> 環境変数が負でない
整数に設定されている場合、引数を指定せずに C<srand()> を呼び出すと、
引数を指定せずに明示的に呼び出されたか、C<rand()> を
使用して暗黙的に呼び出されたかに関係なく、Perl 乱数ジェネレータが
呼び出されるたびに一貫したシードで初期化されます。
特定のC<PERL_RAND_SEED>が生成する正確なシードは意図的に
指定されていませんが、C<PERL_RAND_SEED>に異なる値を使用すると、
異なる結果が生成されます。
これはデバッグとパフォーマンス分析を目的としており、他の
すべての要素が等しい場合に、同じコードを実行する同じ Perl
実行可能ファイルの呼び出し間で一貫した結果が生成されることのみが
保証されています。
環境変数はプロセスの起動時に1回だけ読み取られ、プログラム
...
同様に、指定された Perl のバージョンが 5.35.0 以上の場合、
L<warnings> が有効になります。
後から使った C<use VERSION> は先の
C<use VERSION> の全ての振る舞いを上書きするので、
それによって追加された C<strict>, C<feature>, C<feature> を
削除することがあります。
C<use VERSION> は F<feature.pm>, F<strict.pm>, F<warnings.pm> ファイルは
読み込みません。
...
現在の実装では、明示的に C<use strict> や C<no strict> を使うと、例え先に
