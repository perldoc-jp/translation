...
スマートマッチングは失敗した実験として見られていて、
Perl 5.37.10 で廃止予定としてマークされました。
これは、C<when> と C<given> キーワードおよび、
スマートマッチング演算子 C<~~> を含みます。
この機能は、Perl 5.42.0 製品版リリースで完全に削除されます。
...
Perl では、パッケージやグローバル変数の名前の部分を置き換えるために
C<::> の代わりに C<'> を使えます;
例えば C<A::B> と C<A'B> は等価です。
...
Perl 5.42 から、C<'> はもはや名前区切り文字として認識されません。
...
この振る舞いは Perl 5.40 で削除される予定です; このような引き続く要求は
コンパイル時エラーになる予定です。
...
Perl 5.38 から、L<Pod::Html::Util> からのみ要求に応じてインポート可能に
なります。
...
これらのマクロは Perl 5.32 以前より一つ追加の引数が必要になりました。
...
Perl 5.30 から、:utf8 ハンドルでバイトを読み書きするために
