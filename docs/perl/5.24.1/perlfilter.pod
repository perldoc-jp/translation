...
ソースフィルタの本来の目的は、カジュアルな盗み見を防ぐためにプログラム
ソースを暗号化するためでした。
...
必要があります。
また、生の C<?...?> 区切り文字で指定された正規表現が存在すると不思議なエラーが
起きることがあります。
回避策は代わりに C<m?...?> を使うことです。
L<http://search.cpan.org/perldoc?Switch#LIMITATIONS> を参照してください。
