...
完全な正確性すらも目的にしていません。
場合によっては、一般的な考え方を得るというゴールのために正確性を犠牲にしています。
...
Perl 5.10 から、case/switch 文もあります (C<given>/C<when> と綴ります)。
さらなる詳細については L<perlsyn/"Switch statements"> を参照してください。
...
CPAN ( L<http://www.cpan.org/> ) からダウンロードできます。
