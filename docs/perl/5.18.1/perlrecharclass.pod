...
C<(?s)> を使います。
(後述する C<\N> 逆スラッシュシーケンスでは、I<単一行> 修飾子に
...
Perl 5.18 から、実験的に、垂直タブ C<\cK> にもマッチングします。
...
v5.18 での変更は実験的と考えられています; つまり、
これがあまりに多くの既存のコードを壊すことが分かった場合は、
v5.20 や v5.22 で元に戻すかもしれません。
この変更があなたに不利に働いた場合は、
C<perlbug@perl.org> にメールしてください; あなたに有利に働いた場合は、
C<perlthanks@perl.org> にメールしてください。
それまでの間は、C<[^\S\cK]> は(ひっそりと)C<\s> が伝統的に
...
これが起きるためには、その文字は明示的に指定され、複数文字範囲の一部
(たとえその端でも)でない必要があります。
...
下ではある種の文字が複数の文字並びにマッチングするということを
Perl は無視します。
...
C<\p{SpacePerl}> と C<\p{Space}> は、Perl v5.18 からは同じように
マッチングします。
以前のバージョンでは、これらの違いは、非ロケールマッチングでは
C<\p{SpacePerl}> は垂直タブ C<\cK> にもマッチングしないということだけです。
...
全ての一覧に加えて各 ASCII の範囲の特性にマッチングする全ての文字は、
は L<perluniprops/Properties accessible through \p{} and \P{}> に
あります。
...
C<word> はロケールが何かに関わらず、プラットフォームのネイティブな
下線文字を使います。
...
これらの例の中の空白に注意してください。
この構文では、常に C<E<sol>x> 修飾子がオンになっています。
...
すべての二項演算子は左結合で、同等の優先順位を持ちます。
単項演算子は右結合で、より高い優先順位を持ちます。
デフォルトの関連付けを上書きするにはかっこを使います。
私たちが受け取ったいくつかのフィードバックは、交差が結合よりも
高い優先順位を持つことを望んでいることを示しています。
これは、フィールドからのフィードバックによって、
将来のリリースで変更される可能性があるものです;
この機能が実験的であるとみなされなくなるまで、コードにそのような変更が
影響しないように、かっこをたくさん入れた方がよいでしょう。
...
C<E<sol>x> は、かっこで囲まれた文字クラス内でも有効になりますが、
コメントを含めることはできません。
したがって:
...
は、リテラル文字 "#" にマッチングします。
リテラル空白文字を指定するには、次のように逆スラッシュでエスケープします:
...
すべてここでも受け入れられますが、通常のクラスで警告を生成する
...
これらのクラス要素からのすべての警告は致命的であり、
現在警告していないいくつかのプラクティスも同様です。
例えば次のようにはできません:
...
この構文は、C<use locale> (または C<E<sol>l> 正規表現修飾子)の
スコープ内では使用できません。
...
このクラス内の C<E<sol>x> 処理は拡張された形式です。
通常の C</x> 処理で空白と見なされる文字の他に、Unicode 標準で
推奨されている五つの文字があります:
...
しかし、これは、コードを読んでいる人が期待するような効果はありません;
なぜなら、この交差は C<\p{Thai}> だけに適用され、ラオス語には
適用されないからです。
このような落とし穴は、コンポーネントをかっこで囲むことで回避できます:
...
バランスを取る必要がある場合があります。
もし何か例を見つけたら、C<perlbug@perl.org> まで送ってください。
