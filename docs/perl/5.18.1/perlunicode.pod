...
の場合、Perl は現在のロケールに関連づけられたセマンティクスを使います。
...
CPAN モジュール L<Unicode::Casing> があります。
...
マッチングする値 (Left、Right など) の両方を指定する必要があります。
...
Unicode 特性に対して 非 Unicode 符号位置 (つまり 0x10FFFF より大きいもの) を
マッチングさせようとした場合の結果は未定義です。
現在の所、警告が発生して、マッチングは失敗します。
場合によっては、次の両方が失敗するというように、これは直感的ではありません:
...
例えば、"KATAKANA-HIRAGANA DOUBLE HYPHEN" は日本語の二つの用字
...
(これらは自動的に非 Unicode 符号位置を除外して、それらの一つと
マッチングしようとすると警告が発生するという点で、
公式の Unicode 特性と異なることに注意してください。)
...
CPAN モジュール L<Unicode::Casing> はこの機能が持っていた欠点なしに
よりよい機能を提供します。
...
(バグ付きで)行うことに注意してください:
例えば U+1F88 は単なる U+1F80 ではなく U+1F00 U+03B9 と等価です。
...
U+000B (C では \v), FF (\f), CR (\r), CRLF
(\r\n), NEL (U+0085), LS (U+2028), PS (U+2029) で
^ と $ を行います;
<>, $., スクリプトの行番号に影響を与えません;
CRLF の中の行を分割しません (つまり \r と \n の間に空行はありません)。
CRLF については、C<:crlf> 層 (L<PerlIO> 参照) を試してください。
...
UAX#14 "Unicode Line Breaking Algorithm" の行分割一致は
Unicode::LineBreaking モジュールによって可能です。
...
Perl で使われる UTF-8/UTF-EBDDIC は U+10000 から
U+10FFFF だけでなく U+10FFFF を超える値も認めません
...
渡された場合、常に C<utf8::upgrade($string)> を呼び出すか
標準モジュール L<Encode> を使うことです。
