...
集合です(単一の文字ではありません)。
文字クラスはブラケット C<[...]> で表現され、マッチングする可能性のある文字の
集合はその内側に置かれます。
以下にいくつか例を挙げます:
...
C<\p{Script:Greek}>, C<\P{script=katakana}> と等価に書けます (大文字小文字は
C<{}> の中では無意味です)。
...
これはデンマーク語では A の上に丸がついたものに変換され、オングストロームという
単語で使われます。
...
パターンマッチング操作 S<C<$kwds =~ /\b($command\w*)/g>> は
同時に複数のことを行います。
