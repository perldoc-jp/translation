...
リストアップされます; あるいは Config モジュールから
取得することもできます。
名前を得るための番号によって添え字づけされる @signame というリストと
番号を得るための名前によって添え字づけされる %signo というハッシュ
テーブルをセットアップします。
...
一時的にシグナルを無視するための戦略はそのハッシュ要素に local() 文を
使うというもので、これはその local() 文を囲むブロックから出たときに、
自動的に元の状態に復帰します。
動的スコープの local() による値は呼び出し元のブロックの内側から呼び出された
関数に「引き継がれる」(inherited) ことを忘れないでください。
...
ものなので、関数が“ぶっ壊れた”ようなシステムで実行するような不運な立場に
なるかもしれません; つまり、より新しくて信頼性のある
...
従って、シグナルハンドラ (例では C<&REAPER>) が呼び出されます。
...
終了する前にアラームをクリアします。
停止したときは、他の言語では longjmp() or throw() を使うように、ブロックから
出るために die() を使います。
...
意味します。
この問題の解決策は、もし利用可能であれば、C<POSIX> シグナルハンドラを
使うことです; これは振る舞いが明確です。
...
Perl 5.7.3 以前では、シグナルを取り扱う Perl プログラムを
...
Perl 5.7.3 以降では、「保留」シグナルによってこの問題を回避します。
...
本当にこういったことをしたいのなら、C<IPC::Open2> モジュールの
open2() というライブラリ関数を使うことで、パイプの両方の端点を
得ることができます。
三方向(tridirectional)の入出力のための C<IPC::Open3> の open3() もあるので、
...
一部の定数に対してハードコードされた値を使っていて、移植性を大きく損なっています。
明示的に C<$AF_INET = 2> のようなことをしているコードを見たなら、大きな
問題に遭遇していることになります。
まず最初にとるべき手法は C<Socket> モジュールで、これにより様々な定数と
...
IO::Socket モジュールはリリース 5.004 の標準 Perl 配布の一部として
含まれています。
あなたが以前のバージョンの Perl を使っているのであれば(この場合、
どうやってこの man ページを読んでるの?)
CPAN から IO::Socket を入手します; そこでは、以下に挙げるようなシステムに
