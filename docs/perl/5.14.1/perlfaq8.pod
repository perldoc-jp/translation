...
例外処理そのものに興味があるのなら、C<Try::Tiny> のような、例外を
...
CPAN からダウンロードできる C<local::lib> でしょう。
...
同じことをするのに C<local::lib> も使えます。
C<local::lib> の設定を使ってモジュールをインストールしてから、プログラムで
そのモジュールを使います:
...
さらなる詳細については C<local::lib> の文書を参照してください。
