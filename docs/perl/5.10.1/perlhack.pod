...
C<-DPERL_MEM_LOG> でコンパイルした場合、Perl コア内のすべての
Newx() と Renew() の割り当てと Safefree() はロギング関数を通過します;
これはブレークポイントの設定に便利です。
C<-DPERL_MEM_LOG_STDERR> でもコンパイルした場合、割り当てと解放は、これらの
ロギング関数内の STDERR (より正確にはファイル記述子 2)に、呼び出し元の
ソースコードファイルと行番号(および C コンパイラでサポートされている場合は
C 関数名)とともにロギングされます。
...
このロギングは C<-Dm> といくらか似ていますが、C<-DDEBUGGING> とは
独立しており、より高いレベルにあります(C<-Dm> は直接 C<malloc()> の
ポイントにあり、C<PERL_MEM_LOG> は C<New()> のレベルにあります)。
...
メモリの割り当てに加えて、C<-Dm> と同様に SV の割り当てもログに記録されます。
ただし、ログには PerlIO が使われないため、すべての SV の割り当てがログに
...
次に、C<PERL_MEM_LOG> または C<PERL_SV_LOG> を 0 以外の値に明示的に設定して、
メモリまたは SV の割り当てのロギングを有効にする必要があります。
