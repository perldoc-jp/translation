...
掘り下げた技術的な使用法の詳細については、デバッガ自体の文書である
L<perl5db.pl> を参照してください。
...
プログラムではありません。
その代わりに、-d フラグによって、コンパイラがインタプリタに渡すパース木に
ソース情報を埋め込むようにしています。
これは、ソースがデバッガ上で動作できるためには、
...
L<Devel::Ptkdb> GUI 経由で与えられたプログラムをデバッグする。
...
Perl が C<-d> オプション付きで呼び出された場合、
変数 C<$^P> は真の値を保持します。
コードがデバッガ上で実行されているかどうかを知る必要がある場合に有用です:
...
この変数に関するさらなる情報については L<perlvar/$^P> を参照してください。
...
表示される変数を制限できます。
バージョン 0.08 以降の L<PadWalker> モジュールが必要です;
...
以前のコマンドを再実行します(number が省略されると、直前のコマンドを
実行します)。
...
cmd をサブプロセスとして実行します(DB::IN から読み込み、DB::OUT に
書き出します)。
S<C<O shellBang>> も参照してください。
...
これはデバッガを終了する唯一の方法ですが、C<exit> を 2 回
入力しても動作します。
...
これは対話的、環境変数、rc ファイルで設定できます。
ファイル名は Unix では F</dev/tty> で F<./.perldb> または
F<~/.perldb>、それ以外では F<perldb.ini> です。
...
デバッガのようなクライアントエディタやビジュアルデバッガと相互作用するために
使われる機構です。
...
デバッガの起動時に読み込まれ、(セッションをまたいで永続させるために)終了時に
保管されるヒストリ (L<Term::ReadLine> バックエンドが利用可能であることを
仮定しています) のファイルのパス。
Bash の C<.bash_history> ファイルと同じ考え方です。
...
ヒストリに保管される行数 (上述の C<HistFile> を仮定します)。
...
これは L<Term::ReadLine> 地震を使っているプログラムをデバッグするのに
便利です。
...
C<use> 文) があれば、それらはデバッガによってI<止めることができません>;
C<require> と INIT ブロックは可能です; また、コンパイル時実行文は
C<PERLDB_OPTS> で C<AutoTrace> オプションを設定することでトレースできます。
しかし、以下のような文を自分で Perl コードに含めれば、
デバッガに制御を渡すことができます; この文は、デバッガを
起動していないときには、何もしません:
...
等価になります; 1 を設定すると C<s> コマンドとなります。
...
しかし、(L<Term::ReadLine::Gnu>, L<Term::ReadLine::Perl>, ... のように)
CPAN から L<Term::ReadKey> と C<Term::ReadLine::*> の一つのモジュールを
...
基本的なパッケージ単位のメモリ使用量ダンプ; L<PadWalker> モジュールが
インストールされているなら現在のスコープのレキシカル変数も含みます。
...
Readline 対応なしでは、矢印キーやバックスペースキーを使うと
"^[[A", "^[[C", "^[[B", "^[[D"", "^H" のような表示を見るかもしれません。
...
GNU 版の B<emacs> がシステムにインストールされている場合は、
...
最近のバージョンの Emacs には B<emacs> を Perl の文法(の一部)を解釈する文法指向の
エディタとして振舞わせるためのスタートファイルを同梱しています。
L<perlfaq3> を参照してください。
...
Perl の実行に代替デバッガを使いたい場合は、B<-d> オプションに
コロンとパッケージからなる引数を付けてスクリプトを起動してください。
Perl 用代替デバッガは Perl プロファイラを含む 
L<Devel::DProf> で、CPAN 配布として別に利用可能です。
...
Perl には自身のメモリ使用状況を報告するための内部機能がありますが、
...
C<use strict> と C<use warnings> は有効にしていますよね?
...
デバッガは現在のところ B<-W> コマンドラインスイッチと同時に使うことは
できません; これ自身が警告から逃れられないからです。
...
(キーボードやソケットからの C<wait>, C<accept>, C<read>などの)遅い
システムコールを実行中で、独自の C<$SIG{INT}> ハンドラを設定していない場合、
デバッガに戻ってくるために CTRL-C を使うことはできません;
これは、デバッガ自身の C<$SIG{INT}> ハンドラが遅いシステムコールから
longjmp(3) で出るための例外を発生させる必要性を理解しないからです。
