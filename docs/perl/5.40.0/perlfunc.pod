...
L<C<ref>|/ref EXPR> の結果をチェックするコードが失敗する原因となります;
偽の値である「0」が返されるためです。
...
生成します。
ハンドルに対して明示的に close を呼び出すと、この警告が表示されなくなります。
(TBR)
...
非常に古いマシンの中には C<PID != 0> をサポートしていないものがあり、
C<PID != 0> の場合に例外をスローします。
(TBR)
...
L<C<my>|/my VARLIST>、L<C<state>|/state VARLIST>、および
L<C<our>|/our VARLIST> と同様に、L<C<local>|/local EXPR> は、
(文字列内の補間を除いて)式内の任意の場所で変数を操作できます。
他の宣言とは異なり、L<C<local>|/local EXPR> の効果は実行時に発生するため、
同じステートメント内であっても、宣言後に実行される同じ変数の追加の
使用に適用されます。
これには、変数がローカライズされるときに変数に割り当てられた
式の中での使用は含まれないことに注意してください。
割り当てられた式はローカライズの前に評価されるからです。
(TBR)
...
L<C<state>|/state VARLIST>、L<C<local>|/local EXPR>、および
L<C<our>|/our VARLIST>と同様に、L<C<my>|/my VARLIST>は、(文字列内の
補間を除いて)式に現れる任意の場所で変数を操作できます。
この宣言は、次のステートメントまで同じ変数の追加使用には適用されません。
つまり、同じステートメント内でその変数を追加使用すると、
その宣言が発生する前と同じように動作するか、厳密な'vars'エラーが発生します。
(TBR)
...
L<C<my>|/my VARLIST>、L<C<state>|/state VARLIST>、および
L<C<local>|/local EXPR>と同様に、L<C<our>|/our VARLIST>は、
(文字列内の補間を除いて)式に現れる任意の場所で変数を操作できます。
この宣言は、次のステートメントまで、同じ変数の追加使用には適用されません。
つまり、同じステートメント内でのその変数の追加使用は、
その宣言が行われる前と同じように動作します。
ただし、そのスコープでまだ宣言されていない場合は、厳密な
「vars」を満たし、その変数を新たにエイリアスされたパッケージ変数として
解釈します。
(TBR)
...
フィールドイニシャライザ式などの同様の場所で呼び出されると、
呼び出し側インスタンスのクラスの名前を返します。
基本的にはC<ref($self)>と同じですが、インスタンスが完全に
構築される前に、フィールドイニシャライザで追加的に使用して
...
基本クラスでは、この値はL<C<__PACKAGE__>|/__PACKAGE__>と同じになります。
サブクラスが構築されると、実際のコードが属するパッケージ名だけでなく、
構築されるインスタンスのクラス名が表示されます。
(TBR)
...
L<C<my>|/my VARLIST>、L<C<local>|/local EXPR>、および
L<C<our>|/our VARLIST>と同様に、L<C<state>|/state VARLIST>は、
(文字列内の補間を除いて)式内の任意の場所で変数を操作できます。
この宣言は、次のステートメントまで、同じ変数の追加使用には
適用されません。
つまり、同じステートメント内でその変数を追加使用すると、
その宣言が発生する前と同じように動作するか、
厳密な'vars'エラーが発生します。
(TBR)
...
L<C<use builtin>|builtin> のように、対応するバージョンバンドルとともに
字句的にインポートされます。
(TBR)
...
下位バージョンでは、C<use VERSION> は以前の C<use VERSION> の
ほとんどの動作を上書きし、追加された C<warnings> および
C<feature> の影響を削除する可能性があります。
この動作は非推奨であり、将来の perl リリースでは、一度宣言された
バージョンを変更することはできなくなります。
また、5.11 より小さいバージョンの C<use VERSION> は、
5.11 より大きいバージョンの C<use VERSION> の後に使用することはできません。
(TBR)
...
C<use VERSION>は、F<feature.pm>、F<strict.pm>、F<warnings.pm>、または
