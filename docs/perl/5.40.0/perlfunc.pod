...
Use of C<use VERSION> while another is in effect is not allowed with a
C<use v5.39;> or greater version.
より低いバージョンでは C<use VERSION> は先の
C<use VERSION> のほとんどの振る舞いを上書きするので、
それによって追加された C<feature>, C<feature> の効果を
削除することがあります。
...
F<builtin.pm> ファイルは
読み込みません, but instead implements the equivalent functionality
directly.。
(TBT)
